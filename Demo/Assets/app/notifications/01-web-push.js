...
(() => {
  ...
  const VISITOR_PUSH_PROMPT_SEEN_KEY = "triplem_push_visitor_prompt_seen_v2";
  ...
  function accountPromptSeenKey(userId = currentUserId()) {
    return `${USER_PUSH_PROMPT_SEEN_PREFIX}${safe(userId || "unknown").trim() || "unknown"}`;
  }

  function getAccountPromptSeen(userId = currentUserId()) {
    try { return localStorage.getItem(accountPromptSeenKey(userId)) === "1"; } catch (_) { return false; }
  }

  function setAccountPromptSeen(userId = currentUserId()) {
    try { localStorage.setItem(accountPromptSeenKey(userId), "1"); } catch (_) {}
  }

  function getVisitorPromptSeen() {
    try { return localStorage.getItem(VISITOR_PUSH_PROMPT_SEEN_KEY) === "1"; } catch (_) { return false; }
  }

  function setVisitorPromptSeen() {
    try { localStorage.setItem(VISITOR_PUSH_PROMPT_SEEN_KEY, "1"); } catch (_) {}
  }
  ...
  function supported() {
    // Demo mode is deliberately backend-free and must never register production push workers.
    if (document.documentElement.classList.contains("triplem-demo-boot")) return false;
  ...
  async function disableVisitorNotifications() {
    ...
    try {
      ...
      if (subscription) {
        ...
        await unregisterVisitorEndpoint(endpoint).catch(() => false);
        await subscription.unsubscribe().catch(() => false);
      }
    } catch (_) {}
  ...
  async function toggleVisitorNotifications({ optimisticEnabled = null } = {}) {
    let subscription = null;
    try { subscription = supported() ? await currentSubscription() : null; } catch (_) {}
    const currentlyOn = getVisitorPreference() === "on" && !!subscription && Notification.permission === "granted";
    if (currentlyOn || optimisticEnabled === false) return disableVisitorNotifications();
    return enableVisitorNotifications();
  }
  ...
  function setVisitorPushControlStatus({ enabled = false, disabled = false, title = "" } = {}) {
    [document.getElementById("visitorPushQuickToggleBtn"), document.getElementById("visitorPushMobileToggleBtn")].filter(Boolean).forEach(btn => {
      btn.disabled = disabled;
      btn.classList.toggle("is-on", enabled);
      btn.setAttribute("aria-checked", enabled ? "true" : "false");
      btn.setAttribute("aria-label", enabled ? "Turn visitor notifications off" : "Turn visitor notifications on");
      btn.title = title || (enabled ? "Visitor notifications on" : "Visitor notifications off");
    });
    const label = document.getElementById("visitorPushStatusLabel");
    if (label) label.textContent = enabled ? "Alerts On" : "Alerts Off";
  }

  async function refreshVisitorControl() {
    if (!document.getElementById("visitorPushQuickToggleBtn") && !document.getElementById("visitorPushMobileToggleBtn")) return;
    if (!supported()) {
      setVisitorPushControlStatus({ disabled: true, title: "Web notifications are not supported by this browser." });
      return;
    }
    if (Notification.permission === "denied") {
      setVisitorPushControlStatus({ enabled: false, disabled: true, title: "Notifications are blocked in browser/site settings." });
      return;
    }
    let subscription = null;
    try { subscription = await currentSubscription(); } catch (_) {}
    const enabled = getVisitorPreference() === "on" && Notification.permission === "granted" && !!subscription;
    setVisitorPushControlStatus({ enabled, disabled: false });
  }
  ...
  function bindVisitorToggle() {
    [document.getElementById("visitorPushQuickToggleBtn"), document.getElementById("visitorPushMobileToggleBtn")].filter(Boolean).forEach(btn => {
      ...
      btn.addEventListener("click", event => {
        ...
        const appearsOn = btn.classList.contains("is-on");
        const optimisticEnabled = !appearsOn;
        stateLocal.togglePending = true;
        setVisitorPushControlStatus({ enabled: optimisticEnabled, disabled: true, title: optimisticEnabled ? "Enabling notifications…" : "Turning notifications off…" });
        Promise.resolve(toggleVisitorNotifications({ optimisticEnabled }))
          .catch(error => alert(error?.message || "Visitor notifications could not be updated."))
          .finally(() => {
  ...
  async function maybePromptVisitor() {
    if (stateLocal.visitorPromptShown || getVisitorPromptSeen() || !supported() || !landingIsVisible()) return false;
  ...
  async function requestSecurityRecoveryPush(requestToken) {
    const token = String(requestToken || "").trim();
    if (token.length < 32) return { ok: false, queued: false };
    try { return await invoke("security_recovery_request", { request_token: token }); }
    catch (error) { console.warn("Security recovery push could not be dispatched.", error); return { ok: false, queued: false }; }
  }
  ...
  function bind() {
    ...
    if (!stateLocal.bound) {
      ...
      navigator.serviceWorker?.addEventListener?.("message", event => {
        ...
        if (typeof refreshAdminCommsBadges === "function") refreshAdminCommsBadges().catch(() => {});
        if (type === "security_recovery_request") {
          const requestId = msg.payload?.data?.request_id || "";
          if (typeof loadAdminNotificationsDropdown === "function" && typeof isAdminCommsDropdownOpen === "function" && isAdminCommsDropdownOpen("admin-notify")) loadAdminNotificationsDropdown().catch(() => {});
          setTimeout(() => window.openTrustedRecoveryApprovalById?.(requestId), 120);
          return;
        }
  ...
  window.TriplemPush = {
    ...
    disableVisitorNotifications,
    toggleVisitorNotifications,
    syncVisitorSubscription,
    syncExistingSubscription,
    refreshControl,
    refreshVisitorControl,
    startClientPresence,
    requestLiveChatAgentPush,
    requestMessagePush,
    requestSecurityRecoveryPush,
    openAdminPushModal,
