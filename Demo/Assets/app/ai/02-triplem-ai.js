...
(() => {
  ...
  function resetTriplemAiState({ keepMessages = false } = {}) {
    ...
    try { setTriplemAiPageLock(false); } catch (_) {}
  }
  ...
  function isAiOriginRecord(module, recordId = "", groupId = "") {
    ...
    return (ai?.origins || []).some(origin => {
      ...
      if (rid) return String(origin?.record_id || "") === rid;
  ...
  function allowedCurrencyOptions(selected) {
    let currencies = window.TriplemCurrencyRegistry?.codes?.() || [];
    try {
  ...
  function syncTriplemAiViewport() {
    ...
    if (available > 0) {
      const viewportHeight = `${Math.max(240, available)}px`;
      section.style.setProperty("--triplem-ai-viewport-height", viewportHeight);
      panel.style.setProperty("--triplem-ai-viewport-height", viewportHeight);
    }
  }
  ...
  async function prepareTriplemAiWorkspace() {
    ...
    if (!ai?.access) return;
    renderTriplemAiWorkspace();
    if (!ai.draftsLoaded && !ai.draftsLoading) {
      void loadAiDrafts().then(() => renderTriplemAiWorkspace());
    }
  }

  function setTriplemAiPageLock(active) {
    document.documentElement.classList.toggle("triplem-ai-workspace-active", !!active);
    document.body.classList.toggle("triplem-ai-workspace-active", !!active);
    if (active) requestAnimationFrame(() => syncTriplemAiViewport());
  }
  ...
  window.setTriplemAiPageLock = setTriplemAiPageLock;
