...
let expenseTransferEditState = null;

function resetTransferOptionalDetails(form){
  if (!form) return;
  form.querySelectorAll("details.expense-optional-details").forEach(details => { details.open = false; });
}

function fillTransferOptionalDetails(form, details = {}){
  if (!form || typeof expenseDetailFieldsFor !== "function") return;
  const normalized = typeof normalizeExpenseDetails === "function" ? normalizeExpenseDetails(details || {}) : (details || {});
  for (const field of expenseDetailFieldsFor("transfer")) {
    const input = form.querySelector(`[name="${field.form}"]`);
    if (input) input.value = normalized[field.key] == null ? "" : String(normalized[field.key]);
  }
}

function collectTransferOptionalDetails(fd){
  return typeof expenseDetailsFromFormData === "function"
    ? expenseDetailsFromFormData(fd, "transfer")
    : {};
}

function openTransferModal(fromGroupId, fromWalletName, currency, options = {}) {
  ...
  if (!options.editing) expenseTransferEditState = null;
  els.transferModalTitle.textContent = options.editing ? "Edit Transfer" : "Transfer Money";
  els.transferModalDesc.textContent = options.editing
    ? `Revise this transfer without changing its wallet route.`
    : `Move money from ${fromWalletName} to another wallet.`;
  els.transferForm.reset();
  resetTransferOptionalDetails(els.transferForm);
  defaultDateInputs(els.transferForm);
  els.transferFromWallet.disabled = false;
  els.transferToWallet.disabled = false;
  const submitBtn = els.transferForm.querySelector('button[type="submit"]');
  if (submitBtn) submitBtn.textContent = options.editing ? "Update Transfer" : "Transfer Money";
  ...
  // Bind once. Repeated modal opens must not accumulate handlers.
  if (els.transferForm.dataset.transferListenersBound !== "1") {
    els.transferForm.dataset.transferListenersBound = "1";
...
function expenseTransferUserNote(event){
  const clean = cleanExpenseNote(event?.notesExpense || "");
  if (!clean) return "";
  const detailed = clean.match(/^Transfer to .*?\([^)]*Rate:[^)]*\)\s*(?:-\s*)?(.*)$/i);
  if (detailed) return String(detailed[1] || "").trim();
  const simple = clean.match(/^Transfer to [^:]+(?::\s*(.*))?$/i);
  return simple ? String(simple[1] || "").trim() : clean;
}

function buildExpenseTransferNotes(fromAccount, toAccount, amount, receivedAmount, conversionRate, notes){
  const cross = fromAccount.currency !== toAccount.currency;
  if (cross) {
    const stem = `${amount} ${fromAccount.currency} → ${receivedAmount.toFixed(2)} ${toAccount.currency} (Rate: ${conversionRate})`;
    return {
      expense: `Transfer to ${toAccount.person_name}: ${stem}${notes ? ` - ${notes}` : ""}`,
      topup: `Transfer from ${fromAccount.person_name}: ${stem}${notes ? ` - ${notes}` : ""}`
    };
  }
  return {
    expense: `Transfer to ${toAccount.person_name}${notes ? `: ${notes}` : ""}`,
    topup: `Transfer from ${fromAccount.person_name}${notes ? `: ${notes}` : ""}`
  };
}

function openTransferEditModal(recordId){
  const event = typeof expenseTransferEventById === "function" ? expenseTransferEventById(recordId) : null;
  if (!event?.expenseEntry || !event?.topupEntry) {
    alert("The complete paired transfer could not be found, so it cannot be edited safely.");
    return;
  }
  const accounts = getExpenseAccounts({ applyUiFilters: false });
  const fromAccount = accounts.find(a => a.group_id === event.expenseEntry.group_id);
  const toAccount = accounts.find(a => a.group_id === event.topupEntry.group_id);
  if (!fromAccount || !toAccount) { alert("Transfer wallets could not be found."); return; }
  expenseTransferEditState = {
    recordId: String(event.expenseId),
    event,
    expenseEntry: event.expenseEntry,
    topupEntry: event.topupEntry,
    fromAccount,
    toAccount
  };
  openTransferModal(fromAccount.group_id, fromAccount.person_name, fromAccount.currency, { editing: true });
  els.transferFromWallet.value = fromAccount.group_id;
  els.transferToWallet.innerHTML = accounts.filter(a => a.currency !== "BTC" && a.group_id !== fromAccount.group_id).map(a =>
    `<option value="${escapeHtml(a.group_id)}" data-currency="${escapeHtml(a.currency || "")}" ${a.group_id === toAccount.group_id ? "selected" : ""}>${escapeHtml(a.person_name)} (${escapeHtml(formatReportAmount(a.balance, a.currency))}) - ${escapeHtml(a.currency)}</option>`
  ).join("");
  els.transferToWallet.value = toAccount.group_id;
  syncCurrencySelectFonts(els.transferToWallet);
  updateTransferCurrencyIndicators();
  els.transferForm.querySelector('input[name="amount"]').value = String(Number(event.amtOut || 0));
  els.conversionRateInput.value = String(Number(event.rate || 1));
  els.transferForm.querySelector('input[name="received_amount"]').value = Number(event.amtIn || 0).toFixed(2);
  els.transferForm.querySelector('input[name="date"]').value = String(event.date || "");
  els.transferForm.querySelector('input[name="notes"]').value = expenseTransferUserNote(event);
  fillTransferOptionalDetails(els.transferForm, event.details || {});
  els.transferFromWallet.disabled = true;
  els.transferToWallet.disabled = true;
  calculateReceivedAmount();
}
window.openTransferEditModal = openTransferEditModal;

async function updateExpenseTransfer(form){
  const edit = expenseTransferEditState;
  if (!edit?.expenseEntry || !edit?.topupEntry) throw new Error("Transfer edit session expired. Please reopen the transfer.");
  const fd = new FormData(form);
  const amount = Number(fd.get("amount") || 0);
  const conversionRate = Number(fd.get("conversion_rate") || 1);
  const date = String(fd.get("date") || "");
  const notes = String(fd.get("notes") || "").trim();
  const details = collectTransferOptionalDetails(fd);
  const { fromAccount, toAccount, expenseEntry, topupEntry } = edit;
  if (!Number.isFinite(amount) || amount <= 0) throw new Error("Please enter a valid amount greater than zero.");
  if (!date) throw new Error("Please select a date.");
  const available = Number(fromAccount.balance || 0) + Number(expenseEntry.action_amount || 0);
  if (amount > available) throw new Error(`Insufficient balance. Available including this transfer: ${formatReportAmount(available, fromAccount.currency)}`);
  const cross = fromAccount.currency !== toAccount.currency;
  if (cross && (!Number.isFinite(conversionRate) || conversionRate <= 0)) throw new Error("Please enter a valid conversion rate for this cross-currency transfer.");
  const rate = cross ? conversionRate : 1;
  const receivedAmount = amount * rate;
  const transferNotes = buildExpenseTransferNotes(fromAccount, toAccount, amount, receivedAmount, rate, notes);
  const oldExpenseMeta = expenseMetaFromNotes(expenseEntry.notes);
  const oldTopupMeta = expenseMetaFromNotes(topupEntry.notes);
  const expenseNotes = upsertExpenseMetaInNote(transferNotes.expense, { ...oldExpenseMeta, rowType: "EXPENSE", expenseType: "Transfer", details });
  const topupNotes = upsertExpenseMetaInNote(transferNotes.topup, { ...oldTopupMeta, rowType: "TOPUP", expenseType: "Transfer", details });
  const updatedExpense = { ...expenseEntry, action_amount: amount, action_date: date, notes: expenseNotes };
  const updatedTopup = { ...topupEntry, action_amount: receivedAmount, action_date: date, notes: topupNotes };
  state.entries = state.entries.map(row => row.id === expenseEntry.id ? updatedExpense : row.id === topupEntry.id ? updatedTopup : row);
  if (!isBackupMode()) {
    queueDatabasePatch(expenseEntry.id, { action_amount: amount, action_date: date, notes: expenseNotes }, "Transfer", updatedExpense);
    queueDatabasePatch(topupEntry.id, { action_amount: receivedAmount, action_date: date, notes: topupNotes }, "Transfer", updatedTopup, { silent: true });
  }
  expenseTransferEditState = null;
  els.transferFromWallet.disabled = false;
  els.transferToWallet.disabled = false;
  ...
  renderAll();
  if (typeof showEntryConfirmation === "function") showEntryConfirmation("Transfer updated successfully.", "success");
}


function renderTransferAccountMini(targetId, account) {
  const host = document.getElementById(targetId);
  if (!host) return;
  const html = account && typeof expenseAccountTransferMiniHtml === "function"
    ? expenseAccountTransferMiniHtml(account)
    : "";
  host.innerHTML = html;
...
function updateTransferCurrencyIndicators() {
  ...
  renderTransferAccountMini("transferFromAccountMini", fromAccount);
  renderTransferAccountMini("transferToAccountMini", toAccount);
  
  // Update conversion rate field visibility and help text
  if (fromAccount && toAccount) {
    const isSameCurrency = fromAccount.currency === toAccount.currency;
    const conversionField = document.getElementById("transferConversionField");
    if (conversionField) conversionField.classList.toggle("hide", isSameCurrency);
    els.conversionRateInput.style.display = "block";
...
async function saveTransfer(form) {
  if (expenseTransferEditState) return updateExpenseTransfer(form);
  const fd = new FormData(form);
  ...
  // Create paired transfer records — await sync so both wallet legs remain atomic in the UI.
  const generatedNotes = buildExpenseTransferNotes(fromAccount, toAccount, amount, receivedAmount, isCrossCurrency ? conversionRate : 1, notes || "");
  const transferNote = generatedNotes.expense;
  const receiveNote = generatedNotes.topup;
  ...
  const expensePayload = {
    ...
    action_date: date,
    notes: upsertExpenseMetaInNote(transferNote, { rowType: "EXPENSE", expenseType: "Transfer", details })
  };
  ...
  const topupPayload = {
    ...
    action_date: date,
    notes: upsertExpenseMetaInNote(receiveNote, { rowType: "TOPUP", expenseType: "Transfer", details })
  };
...
function installmentCsvRowHasSourceIdentity(entry){
  ...
  const planType = normalizeInstallmentPlanType(entry.installment_plan_type || (direction === "given" ? "sold" : "bought"));
  ...
  // A given-direction row is accepted only when it carries explicit Sold installment identity.
  if (direction === "given" && planType !== "sold" && !hasDomainIdentity) return false;
...
function ensureEntryTagsForSection(entry, scope){
  ...
  if (scope === LEDGER_SCOPE_INSTALLMENTS) {
    row.installment_plan_type = normalizeInstallmentPlanType(row.installment_plan_type || (String(row.direction || "").toLowerCase() === "given" ? "sold" : "bought"));
    row.direction = row.installment_plan_type === "sold" ? "given" : "taken";
    row.installment_details = normalizeInstallmentDetails(row.installment_details);
...
function expandWalletsOverview() {
  ...
  els.toggleWalletsBtn.setAttribute("aria-expanded", "true");
  els.toggleWalletsBtn.title = "Collapse Wallets Overview";
  els.toggleWalletsBtn.setAttribute("aria-label", "Collapse Wallets Overview");
}
...
function collapseWalletsOverview() {
  ...
  els.toggleWalletsBtn.setAttribute("aria-expanded", "false");
  els.toggleWalletsBtn.title = "Expand Wallets Overview";
  els.toggleWalletsBtn.setAttribute("aria-label", "Expand Wallets Overview");
}
...
function expandMainOverview() {
  ...
  if (els.toggleMainOverviewBtn) els.toggleMainOverviewBtn.setAttribute("aria-expanded", "true");
  setMainOverviewHeading(getActiveTabKey() === "goods" ? "inventory" : "loans");
  if (els.toggleMainOverviewBtn) els.toggleMainOverviewBtn.setAttribute("aria-label", els.toggleMainOverviewBtn.title || "Collapse overview");
}
