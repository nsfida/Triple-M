 ...
 */
function restorePortaledFixedMenuDropdown(panel){
  if (!panel || !panel.__triplemFixedMenuHome) return;
  const home = panel.__triplemFixedMenuHome;
  try { panel.__triplemFixedMenuObserver?.disconnect?.(); } catch (_) {}
  panel.__triplemFixedMenuObserver = null;
  panel.removeAttribute("data-fixed-menu-portaled");
  ...
  panel.style.minWidth = home.minWidth || "";
  ...
  panel.__triplemFixedMenuHome = null;
}
...
function positionFixedMenuDropdown(panel, anchorEl, options = {}){
  ...
  const shouldPortal = options.portal === true;

  // Backdrop-filter / transformed glass cards can create fixed-position containing
  // blocks in Safari and Chromium. Installment menus opt into a temporary body
  // portal so they always paint in the viewport top layer, never behind the card.
  if (shouldPortal && panel.parentNode !== document.body) {
    panel.__triplemFixedMenuHome = { parent: panel.parentNode, nextSibling: panel.nextSibling, minWidth: panel.style.minWidth };
    document.body.appendChild(panel);
    panel.setAttribute("data-fixed-menu-portaled", "true");
    try { panel.__triplemFixedMenuObserver?.disconnect?.(); } catch (_) {}
    const observer = new MutationObserver(() => {
      if (!panel.classList.contains("open")) restorePortaledFixedMenuDropdown(panel);
    });
    observer.observe(panel, { attributes: true, attributeFilter: ["class"] });
    panel.__triplemFixedMenuObserver = observer;
  ...
  // Inline fixed positioning wins over legacy absolute rules used by older modules.
  ...
  const panelWidth = Math.min(Math.max(measured.width || panel.offsetWidth || 0, minWidth), Math.max(minWidth, viewportWidth - margin * 2));
  const panelHeight = measured.height || panel.offsetHeight || 0;
  ...
  // Glass/transform ancestors can establish a containing block for fixed descendants.
  // First neutralize coordinates, then compensate to the desired viewport position.
  panel.style.top = "0px";
  panel.style.left = "0px";
  const fixedOrigin = panel.getBoundingClientRect();
  let adjustedTop = top - fixedOrigin.top;
  let adjustedLeft = left - fixedOrigin.left;
  panel.style.top = `${Math.round(adjustedTop)}px`;
  panel.style.left = `${Math.round(adjustedLeft)}px`;

  // A second tiny correction removes residual offsets introduced by transforms/zoom.
  const placed = panel.getBoundingClientRect();
  const residualX = left - placed.left;
  const residualY = top - placed.top;
  if (Math.abs(residualX) > 0.75 || Math.abs(residualY) > 0.75) {
    adjustedLeft += residualX;
    adjustedTop += residualY;
    panel.style.left = `${Math.round(adjustedLeft)}px`;
    panel.style.top = `${Math.round(adjustedTop)}px`;
  }
}
...
function overviewAvailableLine(amountHtml, balance = 0, usdEquivalent = ""){
  ...
  let usdLine = "";
  if (usdEquivalent) {
    usdLine = `<div class="summary-line summary-line-one" style="margin-top: 2px;"><span class="summary-line-one-label"></span><span class="summary-line-one-value" style="color: var(--muted); font-size: 0.74rem; font-weight: 600;">≈ ${currencySymbolHtml("USD")}${usdEquivalent}</span></div>`;
  }
...
function overviewWatermarkWallet(walletName, currency, customLogoUrl = ""){
  ...
  return `
    <div class="summary-watermark summary-watermark-wallet-logo" aria-hidden="true">
      <img src="${escapeLogoSrc(logoPath)}" alt="${escapeHtml(walletName)} logo"
           style="width: 100%; height: 100%; object-fit: contain; opacity: 0.34;"
...
function walletLogoPhotoMenuHtml(groupId, walletName = "Wallet", options = {}){
  ...
  return `
    <div class="menu-wrap wallet-logo-menu-wrap" data-wallet-logo-menu="${gid}">
      ...
      <div class="menu-dropdown wallet-logo-menu-dropdown" data-wallet-logo-panel="${gid}" role="menu" aria-label="Wallet photo options">
        <button type="button" class="menu-item walletLogoMenuAction" data-action="view" data-group-id="${gid}" role="menuitem">View</button>
        <button type="button" class="menu-item walletLogoMenuAction" data-action="change" data-group-id="${gid}" role="menuitem">Change</button>
        <button type="button" class="menu-item walletLogoMenuAction wallet-logo-default-action${hasCustom ? "" : " hide"}" data-action="default" data-group-id="${gid}" role="menuitem">Default</button>
      </div>
    </div>
  `;
}
...
function overviewWatermarkFloatingWalletLogos(accounts){
  ...
  for (let i = 0; i < logoCount; i++) {
    ...
    // Stable pseudo-random placement prevents visual jumps whenever an expense
    // mutation repaints the overview. The motion remains organic, but the same
    // wallet always resumes from the same layout recipe instead of teleporting.
    const seed = `${account.group_id || name}:${i}`;
    const dur = (14 + hash01(`${seed}:duration`) * 18).toFixed(1); // 14..32s
    const delay = (-2 - hash01(`${seed}:delay`) * 13).toFixed(1); // -2..-15s
    const scale = (0.72 + hash01(`${seed}:scale`) * 0.48).toFixed(2); // 0.72..1.20
    const left = (4 + hash01(`${seed}:left`) * 88).toFixed(2); // 4..92%
    const top = (5 + hash01(`${seed}:top`) * 86).toFixed(2); // 5..91%
    const animVariant = Math.floor(hash01(`${seed}:variant`) * 4) % 4;
...
function renderInventoryOverviewCards(){
  ...
  els.statsGrid.innerHTML = `
    ...
    <div class="summary currency-summary goods-overview">
      ...
      ${overviewOneLine("Balance due:", `<strong>${balanceTotalHtml}</strong>`)}
      <div class="overview-card-actions" style="margin-top: 8px; display: flex; gap: 6px; flex-wrap: wrap;">
        <button class="tiny ghost" onclick="downloadGoodsPDF()"><i class="fa-solid fa-file-pdf"></i></button>
      </div>
    </div>
  `;
}
    ...
    return `
      <div class="summary currency-summary">
        ...
        <div class="overview-card-actions" style="margin-top: 8px; display: flex; gap: 6px; flex-wrap: wrap;">
          ...
          <button class="tiny ghost" onclick="downloadCurrencyPDF('${currency}')"><i class="fa-solid fa-file-pdf"></i></button>
        </div>
      </div>
    `;
  }).join("");
...
function normalizeInstallmentPlanType(value){
  return String(value || "").trim().toLowerCase() === "sold" ? "sold" : "bought";
}

function installmentPlanTypeOf(value){
  if (!value) return "bought";
  const direct = value.plan_type || value.installment_plan_type;
  if (direct) return normalizeInstallmentPlanType(direct);
  return String(value.direction || "").toLowerCase() === "given" ? "sold" : "bought";
}

function normalizeInstallmentDetails(value){
  const src = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const clean = key => String(src[key] || "").trim().slice(0, key === "address" || key === "terms" ? 500 : 180);
  return {
    party_type: clean("party_type"), phone: clean("phone"), email: clean("email"),
    address: clean("address"), reference: clean("reference"), item: clean("item"),
    tax_id: clean("tax_id"), guarantor: clean("guarantor"), guarantor_contact: clean("guarantor_contact"),
    terms: clean("terms"), document_ref: clean("document_ref")
  };
}
...
function getInstallmentPlanGroup(groupId){
  ...
  return {
    ...
    plan_type: installmentPlanTypeOf(principal),
    details: normalizeInstallmentDetails(principal.installment_details),
    downPayment,
