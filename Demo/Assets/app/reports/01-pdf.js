...
const EXPENSE_PDF_FLOW_MARKERS = Object.freeze({
  in: "\u2062",
  out: "\u2063"
});
const EXPENSE_PDF_FLOW_MARKER_PATTERN = /[\u2062\u2063]/g;
const EXPENSE_PDF_FLOW_MARKER_TEST_PATTERN = /[\u2062\u2063]/;
const EXPENSE_PDF_FLOW_SIGN_COLORS = Object.freeze({
  in: [22, 163, 74],
  out: [220, 38, 38]
});

function hasExpensePdfFlowMarkers(value){
  return EXPENSE_PDF_FLOW_MARKER_TEST_PATTERN.test(String(value ?? ""));
}

function stripExpensePdfFlowMarkers(value){
  return String(value ?? "").replace(EXPENSE_PDF_FLOW_MARKER_PATTERN, "");
}

function stripPdfRenderMarkers(value){
  return stripExpensePdfFlowMarkers(stripPdfCurrencyMarkers(value));
}

function pdfTextContainsCurrencyMarkers(text){
  if (Array.isArray(text)) return text.some(line => hasPdfCurrencyMarkers(line) || hasExpensePdfFlowMarkers(line));
  return hasPdfCurrencyMarkers(text) || hasExpensePdfFlowMarkers(text);
}
...
function splitPdfMarkedTextToSize(doc, text, maxWidth){
  ...
  words.forEach(word => {
    ...
    const visibleCandidate = stripPdfRenderMarkers(candidate);
...
function tokenizePdfCurrencyText(text){
  const markerFonts = Object.fromEntries(Object.entries(PDF_CURRENCY_CODES_BY_MARKER || {}).map(([marker, code]) => [marker, code]));
  const flowColors = {
    [EXPENSE_PDF_FLOW_MARKERS.in]: EXPENSE_PDF_FLOW_SIGN_COLORS.in,
    [EXPENSE_PDF_FLOW_MARKERS.out]: EXPENSE_PDF_FLOW_SIGN_COLORS.out
  };
  ...
  for (let i = 0; i < raw.length; i += 1){
    ...
    const flowColor = flowColors[ch];
    if (font || flowColor){
      if (buffer){
        tokens.push({ text: buffer, font: null, color: null });
        buffer = "";
      }
      const markedCharacter = raw[i + 1] || "";
      if (markedCharacter){
        tokens.push({
          text: markedCharacter,
          font: font || null,
          color: flowColor || null,
          flow: flowColor
            ? (ch === EXPENSE_PDF_FLOW_MARKERS.in ? "in" : "out")
            : null
        });
        i += 1;
      }
  ...
  if (buffer) tokens.push({ text: buffer, font: null, color: null });
  return tokens;
}
...
function installPdfCurrencyTextRenderer(doc){
  ...
  function restoreTextColor(color){
    if (!color) return;
    try { doc.setTextColor(color); }
    catch { doc.setTextColor(0, 0, 0); }
  }

  function drawMarkedLine(line, x, y, options = {}){
    const raw = String(line ?? "");
    if (!hasPdfCurrencyMarkers(raw) && !hasExpensePdfFlowMarkers(raw)){
      return originalText(raw, x, y, options);
    }

    const baseFont = doc.getFont();
    const baseTextColor = typeof doc.getTextColor === "function" ? doc.getTextColor() : "#000000";
    const visibleText = stripPdfRenderMarkers(raw);
    ...
    tokenizePdfCurrencyText(raw).forEach(token => {
      ...
      if (Array.isArray(token.color)) doc.setTextColor(token.color[0], token.color[1], token.color[2]);
      else restoreTextColor(baseTextColor);

      // jsPDF's built-in Helvetica does not contain U+2212 (MINUS SIGN) and can
      // encode it as a quotation-mark glyph. Keep the semantic source character
      // as the true Unicode minus, but render that one glyph as a vector minus so
      // it is always visually correct. Incoming "+" rendering is intentionally
      // left untouched.
      if (token.flow === "out" && token.text === "−"){
        const signWidth = Math.max(doc.getTextWidth("+"), 1.6);
        const fontHeight = Math.max(doc.getFontSize() / doc.internal.scaleFactor, 1);
        const lineY = y - (fontHeight * 0.31);
        const inset = signWidth * 0.16;
        const previousDrawColor = typeof doc.getDrawColor === "function" ? doc.getDrawColor() : null;
        const previousLineWidth = typeof doc.getLineWidth === "function" ? doc.getLineWidth() : null;
        if (Array.isArray(token.color)) doc.setDrawColor(token.color[0], token.color[1], token.color[2]);
        doc.setLineWidth(Math.max(0.18, fontHeight * 0.075));
        doc.line(cursorX + inset, lineY, cursorX + signWidth - inset, lineY);
        if (previousDrawColor != null){
          try { doc.setDrawColor(previousDrawColor); } catch {}
        }
        if (previousLineWidth != null){
          try { doc.setLineWidth(previousLineWidth); } catch {}
        }
        cursorX += signWidth;
      } else {
    ...
    restoreFont(baseFont);
    restoreTextColor(baseTextColor);
    return doc;
  }
...
function isPdfMoneyLike(value){
  const text = stripPdfRenderMarkers(Array.isArray(value) ? value.join(" ") : value);
  if (!/\d/.test(text)) return false;
  const symbols = (window.TriplemCurrencyRegistry?.symbols?.() || []).filter(Boolean)
    .map(symbol => String(symbol).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
  if (!symbols.length) return false;
  const symbolPattern = `(?:${symbols.join("|")})`;
  return new RegExp(`(^|[\\s(:+\\-])${symbolPattern}\\s*[-+]?\\d|[-+]?\\d[\\d,]*(?:\\.\\d+)?\\s*${symbolPattern}`).test(text);
}
...
function expensePdfFlowDirection(direction){
  return String(direction || "out").toLowerCase() === "in" ? "in" : "out";
}

function expensePdfFlowText(amount, currency, direction){
  const flow = expensePdfFlowDirection(direction);
  const sign = flow === "in" ? "+" : "−";
  return `${EXPENSE_PDF_FLOW_MARKERS[flow]}${sign} ${formatPdfAmount(Math.abs(Number(amount || 0)), currency)}`;
}

function expensePdfFlowExistingText(value, direction){
  const raw = stripExpensePdfFlowMarkers(String(value == null ? "" : value)).trim();
  if (!raw || raw === "—" || raw === "-") return raw || "—";
  const flow = expensePdfFlowDirection(direction);
  const sign = flow === "in" ? "+" : "−";
  const clean = raw.replace(/^[+−-]\s*/, "");
  return `${EXPENSE_PDF_FLOW_MARKERS[flow]}${sign} ${clean}`;
}
    ...
    const rows = spendRows
      ...
      .map(({ row, account }) => {
        ...
        return [
          ...
          expensePdfFlowText(Number(row.action_amount || 0), account.currency, "out"),
          tax.tax ? expensePdfFlowText(tax.tax, account.currency, "out") : "-",
          "—",
          typeof expensePdfDetailsText === "function"
            ? expensePdfDetailsText(meta.details || {}, "expense", cleanExpenseNote(row.notes))
...
async function exportSectionPDF(searchKey){
  ...
  const tableHead = expensePdf
    ? [["Item", "Date", "Notes / Full Detail", "Wallet / Type", "Wallet", "VAT", "Amount"]]
    : [["Member", "Date", "Type", "Notes", "Amount", "Remaining"]];
  const expenseSectionLogoRows = expensePdf && typeof expensePdfWalletLogos === "function"
    ? (() => report.rows.map(row => row[3] || "Wallet"))()
    : [];
  const expenseSectionLogoMap = expensePdf && typeof expensePdfWalletLogos === "function"
    ? await expensePdfWalletLogos(expenseSectionLogoRows)
    : new Map();
  const expenseSectionLogos = expenseSectionLogoRows.map(name => expenseSectionLogoMap.get(String(name).trim()) || null);
  ...
  doc.autoTable({
    ...
    margin: { top: 42, bottom: 32 },
    ...(expensePdf && typeof expensePdfWalletCellHooks === "function" ? expensePdfWalletCellHooks(doc, expenseSectionLogos, 4) : {}),
    didDrawPage: () => drawPdfHeaderAndFooter(doc, logoData, title, subtitle, false)
  });
...
async function loadCustomFontsForPdf(doc){
  ...
  try {
    const customCurrencies = (window.TriplemCurrencyRegistry?.all?.() || []).filter(currency =>
      currency?.font?.file && currency?.font?.family && currency.font.family !== "inherit"
    );
    for (const currency of customCurrencies) {
      try {
        const response = await fetch(currency.font.file);
        if (!response.ok) continue;
        const bytes = new Uint8Array(await response.arrayBuffer());
        let binary = "";
        const chunkSize = 0x8000;
        for (let i = 0; i < bytes.length; i += chunkSize) binary += String.fromCharCode(...bytes.subarray(i, i + chunkSize));
        const sourceName = String(currency.font.file).split(/[/?#]/).filter(Boolean).pop() || `${currency.code}.ttf`;
        const vfsName = `TriplemCurrency_${currency.code}_${sourceName}`;
        doc.addFileToVFS(vfsName, binary);
        doc.addFont(vfsName, currency.code, "normal");
      } catch (fontError) {
        console.log(`Failed to load ${currency.code} PDF currency font:`, fontError);
      }
    }
    doc.__tripleMFontsLoaded = true;
  } catch (e) {
    console.log("Failed to load custom currency fonts:", e);
  } finally {
    applyProfessionalPdfDefaults(doc);
  }
}
