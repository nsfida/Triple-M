...
(function demoModeBootstrap(global) {
  ...
  function sampleAiResponse(message) {
    const q = String(message || "").toLowerCase();
    const references = [{ source: "Demo workspace", detail: "Local fictional Triplem VIP records used only inside this Demo page" }];

    if ((q.includes("draft") || q.includes("record")) && q.includes("expense")) {
      const draft = {
        id: id("ai-draft-office-supplies"),
        module: "expenses",
        record_type: "expense",
        status: "draft",
        created_at: DEMO_NOW,
        updated_at: DEMO_NOW,
        payload: { item_name: "Office supplies", amount: 125, currency: "AED", date: "2026-09-12", wallet_name: "Office Cash", expense_type: "Office", notes: "Prepared by the local Demo assistant" }
      };
      return {
        answer: "I prepared an isolated expense draft for AED 125. Review the wallet, date, category and amount before finalizing it. Nothing changes in balances until you approve the draft.",
        records: [],
        references,
        drafts: [draft]
      };
    ...
    if (q.includes("vat")) {
      return {
        answer: "The sample UAE VAT activity is concentrated in office rent and cloud infrastructure. Rent includes AED 600 VAT and cloud services include AED 90 VAT, so these two sample expenses contribute AED 690 of input VAT before any accounting-document VAT is considered.",
        records: [
          { module: "expenses", record_type: "expense", record_id: id("expense-rent"), group_id: id("wallet-enbd"), title: "Office Rent", record_date: "2026-09-01", details: { wallet: "Emirates NBD", currency: "AED", amount: 12600, vat: 600, status: "Recorded" } },
          { module: "expenses", record_type: "expense", record_id: id("expense-cloud"), group_id: id("wallet-enbd"), title: "Cloud Infrastructure", record_date: "2026-09-03", details: { wallet: "Emirates NBD", currency: "AED", amount: 1890, vat: 90, status: "Recorded" } }
        ],
        references: [{ source: "Expenses", detail: "VAT-tagged sample expense records" }, { source: "Accounting Tax", detail: "Posted accounting documents remain separate from operating expense records" }],
        drafts: []
      };
    }

    if (q.includes("inventory") && (q.includes("remain") || q.includes("stock after") || q.includes("left in stock"))) {
      return {
        answer: "Remaining sample stock is 12 iPhone 16 Pro units, 240 Premium Mineral Water bottles and 7 Dell XPS 14 units. These figures reflect completed sample sales against the original purchased quantities.",
        records: [
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-iphone-p"), group_id: id("inventory-iphone"), title: "iPhone 16 Pro 256GB", record_date: "2026-07-05", details: { item: "iPhone 16 Pro 256GB", currency: "AED", purchased: 24, sold: 12, remaining: 12, purchase_price: 3450, status: "In Stock" } },
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-water-p"), group_id: id("inventory-water"), title: "Premium Mineral Water 500ml", record_date: "2026-08-10", details: { item: "Premium Mineral Water 500ml", currency: "AED", purchased: 360, sold: 120, remaining: 240, purchase_price: 1.15, status: "In Stock" } },
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-dell-p"), group_id: id("inventory-dell"), title: "Dell XPS 14", record_date: "2026-06-18", details: { item: "Dell XPS 14", currency: "USD", purchased: 10, sold: 3, remaining: 7, purchase_price: 1480, status: "In Stock" } }
        ],
        references: [{ source: "Inventory", detail: "Remaining quantities from sample purchases and completed sales" }],
        drafts: []
      };
    }
    ...
    if (q.includes("inventory")) {
      return {
        answer: "By sell-through rate, iPhone 16 Pro is the fastest-moving sample item: 12 of 24 units sold, leaving 12 in stock. Premium Mineral Water has sold 120 of 360 units, while Dell XPS 14 has sold 3 of 10. The iPhone line therefore deserves the closest replenishment attention.",
        records: [
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-iphone-p"), group_id: id("inventory-iphone"), title: "iPhone 16 Pro 256GB", record_date: "2026-07-05", details: { item: "iPhone 16 Pro 256GB", currency: "AED", purchased: 24, sold: 12, remaining: 12, purchase_price: 3450, status: "In Stock" } },
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-water-p"), group_id: id("inventory-water"), title: "Premium Mineral Water 500ml", record_date: "2026-08-10", details: { item: "Premium Mineral Water 500ml", currency: "AED", purchased: 360, sold: 120, remaining: 240, purchase_price: 1.15, status: "In Stock" } },
          { module: "inventory", record_type: "inventory_item", record_id: id("inventory-dell-p"), group_id: id("inventory-dell"), title: "Dell XPS 14", record_date: "2026-06-18", details: { item: "Dell XPS 14", currency: "USD", purchased: 10, sold: 3, remaining: 7, purchase_price: 1480, status: "In Stock" } }
        ],
        references: [{ source: "Inventory", detail: "Sample purchases and completed sales grouped by product" }],
        drafts: []
      };
    }
    ...
    if (q.includes("expense")) {
      return {
        answer: "September AED expenses total AED 22,625 in the sample workspace. Rent is the largest category at AED 12,600, followed by Marketing at AED 7,350 and Software at AED 1,890. Logistics contributes AED 475 and Office contributes AED 310.",
        records: [
          { module: "expenses", record_type: "expense", record_id: id("expense-rent"), group_id: id("wallet-enbd"), title: "Office Rent", record_date: "2026-09-01", details: { wallet: "Emirates NBD", currency: "AED", amount: 12600, status: "Recorded" } },
          { module: "expenses", record_type: "expense", record_id: id("expense-ads"), group_id: id("wallet-enbd"), title: "Digital Campaign", record_date: "2026-09-05", details: { wallet: "Emirates NBD", currency: "AED", amount: 7350, status: "Recorded" } },
          { module: "expenses", record_type: "expense", record_id: id("expense-cloud"), group_id: id("wallet-enbd"), title: "Cloud Infrastructure", record_date: "2026-09-03", details: { wallet: "Emirates NBD", currency: "AED", amount: 1890, status: "Recorded" } }
        ],
        references: [{ source: "Expenses", detail: "September sample transactions, kept in their original currencies" }],
        drafts: []
      };
    }
    ...
    if (q.includes("loan")) {
      return {
        answer: "The sample workspace has receivables from Ahmad Khan and Olivia Carter and a payable to Skyline Capital. Ahmad has AED 16,000 outstanding after two repayments, Olivia has USD 7,000 outstanding, and Skyline Capital has AED 50,000 remaining after a principal repayment.",
        records: [
          { module: "loans", record_type: "loan", record_id: id("loan-given-ahmad-p"), group_id: id("loan-given-ahmad"), title: "Ahmad Khan", record_date: "2026-05-10", details: { person: "Ahmad Khan", direction: "given", currency: "AED", principal: 25000, outstanding: 16000, status: "Open" } },
          { module: "loans", record_type: "loan", record_id: id("loan-taken-skyline-p"), group_id: id("loan-taken-skyline"), title: "Skyline Capital", record_date: "2026-04-18", details: { person: "Skyline Capital", direction: "taken", currency: "AED", principal: 60000, outstanding: 50000, status: "Open" } }
        ],
        references: [{ source: "Loans", detail: "Sample principal and repayment records" }],
        drafts: []
      };
    }

    if (q.includes("financial") || q.includes("how am i doing") || q.includes("this month") || q.includes("audit")) {
      return {
        answer: "The sample workspace is operationally active across several currencies. AED spending is led by rent and marketing, inventory remains in stock across all three sample products, and open loan balances are still material. Review each source record before making a real financial decision.",
        records: [],
        references: [{ source: "Dashboard", detail: "Local sample balances and activity summaries" }, { source: "Audit", detail: "Cross-module review is available from the Financial Audit section" }],
        drafts: []
      };
    }

    return {
      answer: "I can explain the local sample dashboard, expenses, accounting, inventory, loans, installments, assets, notes and Bitcoin data. I can also prepare isolated drafts when you explicitly ask to record something, but the Demo never sends your prompt to a remote service.",
      records: [],
      references,
      drafts: []
    };
  ...
  global.fetch = async function demoFetch(input, init = {}) {
    ...
    if (/demo\.triplem\.local|\/functions\/v1\/triplem-ai/i.test(url)) {
      ...
      if (url.includes("triplem-ai")) {
        ...
        if (action === "chat") {
          const sample = sampleAiResponse(body.message);
          return jsonResponse({ ok: true, ...sample, model: "gemini-3.8-flash" });
        }
