/* Triplem VIP Demo Guided Chapters · overlay-stability fix 2026-09-12
...
(function triplemDemoGuidedChapters(global) {
  ...
  let activeStepNarration = Promise.resolve();
  let cachedVoices = [];
  ...
  // Routine navigation, typing labels and obvious button instructions stay visual-only.
  function usefulNarration(text, kind = "step") {
    ...
    if (kind === "action" || kind === "field") {
      const actionOnly = /^(?:open|choose|select|click|press|type|enter|set|return|close|expand|add|pick|switch|start|send|save|run|move)\b/i.test(clean);
      const teachingSignal = /\b(?:because|so that|which|while|keeps?|shows?|records?|creates?|calculates?|groups?|narrows?|connects?|preserves?|protects?|separates?|applies?|updates?|reduces?|increases?|confirms?|summarizes?|reveals?|tracks?|links?|lets?|allows?)\b/i.test(clean);
      const wordCount = clean.split(/\s+/).filter(Boolean).length;
      if (actionOnly && !teachingSignal && wordCount <= 20) return "";
      if (kind === "field" && !teachingSignal && wordCount <= 16) return "";
    }

    let chosen = sentences.slice(0, kind === "intro" ? 4 : kind === "exercise" ? 3 : kind === "lecture" ? 3 : 2).join(" ").trim();
    if (kind === "step" || kind === "action" || kind === "field") {
      chosen = chosen
    ...
    const limit = kind === "intro" ? 62 : kind === "exercise" ? 44 : kind === "lecture" ? 50 : 40;
  ...
  function cancelNarration() {
    narrationToken += 1;
    activeStepNarration = Promise.resolve();
    if (!speechAvailable) return;
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      #triplemDemoTourFocusRingV3{position:fixed;left:0;top:0;z-index:2147483000;pointer-events:none;opacity:0;border:3px solid color-mix(in srgb,var(--primary,#2563eb) 82%,white);border-radius:12px;box-shadow:0 0 0 8px color-mix(in srgb,var(--primary,#2563eb) 18%,transparent);transition:left .12s ease,top .12s ease,width .12s ease,height .12s ease,opacity .14s ease,border-radius .12s ease}
      #triplemDemoTourFocusRingV3.is-visible{opacity:1}
      ...
      @media(prefers-reduced-motion:reduce){#triplemDemoTourCursorV3{transition:opacity .15s ease}#triplemDemoTourFocusRingV3{transition:opacity .12s ease}}
    `;
  ...
  function installOverlays() {
    ...
    if (!focusRing) {
      ...
      focusRing.id = "triplemDemoTourFocusRingV3";
      focusRing.setAttribute("aria-hidden", "true");
      doc.body.appendChild(focusRing);
    }
  }
  ...
  function visualFocusElement(el) {
    if (!el || !el.isConnected) return null;
    // Full-screen modal/overlay roots must never be restyled by the tour. Highlight their
    // visible dialog/content panel instead, while retaining the real element as the action target.
    if (el.matches?.('.modal,.transfer-success-overlay,.money-added-success-overlay,.guest-restriction-overlay,.fraud-alert-overlay')) {
      return firstVisible('.modal-dialog,.transfer-success-content,.money-added-success-content,[role="dialog"]', el) || el;
    }
    return el;
  }
  ...
  function syncFocusRing() {
    if (!focusRing) return;
    const visual = visualFocusElement(highlight);
    if (!visual || !visible(visual)) {
      focusRing.classList.remove("is-visible");
      return;
    }
    const r = visual.getBoundingClientRect();
    const pad = 5;
    const left = clamp(r.left - pad, 2, Math.max(2, global.innerWidth - 4));
    const top = clamp(r.top - pad, 2, Math.max(2, global.innerHeight - 4));
    const right = clamp(r.right + pad, left + 2, global.innerWidth - 2);
    const bottom = clamp(r.bottom + pad, top + 2, global.innerHeight - 2);
    const radius = Math.max(10, Math.min(20, parseFloat(global.getComputedStyle(visual).borderRadius) || 10));
    focusRing.style.left = `${Math.round(left)}px`;
    focusRing.style.top = `${Math.round(top)}px`;
    focusRing.style.width = `${Math.max(2, Math.round(right - left))}px`;
    focusRing.style.height = `${Math.max(2, Math.round(bottom - top))}px`;
    focusRing.style.borderRadius = `${radius}px`;
  ...
  async function waitUntil(test, timeout = 3600, interval = 90) {
    const started = Date.now();
  ...
  function overlayContextFor(el) {
    if (!el?.closest) return null;
    return el.closest('.modal:not(.hide),.transfer-success-overlay:not(.hide),.money-added-success-overlay:not(.hide),.settings-sheet:not(.hide),[role="dialog"]');
  }

  function scrollableAncestorWithin(el, boundary) {
    let node = el?.parentElement || null;
    while (node && node !== boundary?.parentElement) {
      const style = global.getComputedStyle(node);
      const overflowY = String(style.overflowY || style.overflow || "");
      if (/(auto|scroll|overlay)/i.test(overflowY) && node.scrollHeight > node.clientHeight + 4) return node;
  ...
  async function ensureInView(el) {
    ...
    const overlay = overlayContextFor(el);

    // Fixed modal/dialog content belongs to its own scrolling context. Calling window.scrollBy
    // while explaining an overlay makes the page behind it jump, and scrollIntoView can move
    // both the dialog and the document. Scroll only the modal's internal body when necessary.
    if (overlay) {
      const overlayStyle = global.getComputedStyle(overlay);
      const isFixedRoot = overlay === el && overlayStyle.position === "fixed";
      if (isFixedRoot) return;
      if (r.top < topSafe || r.bottom > bottomSafe || horizontallyOff) {
        const host = scrollableAncestorWithin(el, overlay);
        if (host && host !== el) {
          const hr = host.getBoundingClientRect();
          const desiredTop = hr.top + Math.max(18, (hr.height - Math.min(r.height, hr.height)) / 2);
          const delta = r.top - desiredTop;
          try { host.scrollBy({ top: delta, behavior: reduced() ? "auto" : "smooth" }); } catch (_) { host.scrollTop += delta; }
          await sleep(isMobile() ? 360 : 280);
  ...
  async function focusTarget(target, remark, options = {}) {
    ...
    if (!el) {
      // Never speak about a control that is not actually visible. Keep only the on-screen guidance.
      await showRemark(remark || "Continue with the next part of this workflow.", { ...(options.narration || {}), narrate: false });
      activeStepNarration = Promise.resolve();
    ...
    activeStepNarration = Promise.resolve(showRemark(remark, options.narration || {})).catch(() => {});
    highlight = el;
    syncFocusRing();
    ...
    syncFocusRing();
    if (!options.syncAction) await activeStepNarration;
    await sleep(options.hold ?? pace("settle"));
    return el;
  }

  async function finishStepNarration() {
    const pending = activeStepNarration;
    activeStepNarration = Promise.resolve();
  ...
  async function clickTarget(target, remark, options = {}) {
    const focusOptions = { ...options, syncAction: true, narration: { kind: "action", ...(options.narration || {}) } };
    let el = await focusTarget(target, remark, focusOptions);
    if (!el) return null;

    for (let pass = 0; pass < 4; pass += 1) {
      el = resolve(target) || el;
      if (!el || !visible(el)) { await finishStepNarration(); return null; }
      await ensureInView(el);
      await waitForStableRect(el);
      syncFocusRing();
      el = resolve(target) || el;
      ...
      // Keep speech and the visible action synchronized, then allow the UI to settle.
      await finishStepNarration();
      await sleep(options.after ?? Math.max(pace("click"), isMobile() ? 700 : 560));
      return el;
    }
  ...
  async function ensureExpanded(target, remark, desired = true) {
    ...
    if (!el) { currentStep += 1; await showRemark(remark, { narrate: false }); await sleep(90); return null; }
  ...
  async function typeTarget(target, value, remark, options = {}) {
    const focusOptions = { ...options, syncAction: true, narration: { kind: "field", ...(options.narration || {}) } };
    const el = await focusTarget(target, remark, focusOptions);
    if (!el) return null;
    ...
    if (direct) {
      ...
      await finishStepNarration();
      await sleep(Math.min(pace("select"), 480));
      return el;
    }
  ...
  async function selectTarget(target, wanted, remark, options = {}) {
    const focusOptions = { ...options, syncAction: true, narration: { kind: "action", ...(options.narration || {}) } };
    const el = await focusTarget(target, remark, focusOptions);
    if (!el || !(el instanceof HTMLSelectElement)) { await finishStepNarration(); return null; }
  ...
  async function lecture(target, remark, ms) {
    await focusTarget(target, remark, { hold: 160, narration: { kind: "lecture" } });
    await sleep(ms ?? Math.round(pace("lecture") * .82));
  }

  async function pressEnter(target, remark) {
    const el = await focusTarget(target, remark, { syncAction: true, narration: { kind: "action" } });
    if (!el) return;
    ...
    await finishStepNarration();
    await sleep(Math.min(pace("click"), 520));
  }
  ...
  async function clickNav(key) {
    ...
    if (btn && !btn.classList.contains("active")) {
      await clickTarget(btn, `Opening ${AREA_LABELS[key] || key}.`, { narration: { narrate: false, kind: "navigation" }, after: 360 });
      await sleep(260);
    }
  }
  ...
  const GUIDES = {
    ...
    goods: {
      label: "Inventory",
      chapters: [
        chapter("Inventory overview", "Read stock health, then narrow the catalog with practical search and status controls.", 8, async () => {
          ...
          await lecture("#statsGrid", "The overview summarizes inventory value and movement before you open an individual category or product.");
          await lecture("#goodsList", "The main catalog is grouped into category cards, so large inventories stay readable without mixing unrelated product families.");
          await typeTarget("#searchGoods", "iPhone", "Search for iPhone.", { narration: { narrate: false } });
          await lecture("#goodsList", "Search narrows the visible catalog immediately while preserving the underlying stock records.");
          await clearInput("#searchGoods");
          await selectTarget("#inventoryStatusFilter", "In Stock", "Keep the view on products that still have stock available.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#goodsList .inventory-section-card[data-inventory-section]'), "Each category card shows item count, brand count and current stock before you open the detailed product view.");
        }),
        chapter("Add purchased stock", "Build a purchase line, verify totals and understand the optional wallet deduction before saving.", 10, async () => {
          await clickNav("goods");
          await clickTarget("#inventoryActionsBtn", "Open Inventory actions.", { narration: { narrate: false } });
          await clickTarget("#openGoodsBoughtBtn", "Choose Add item.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#goodsBoughtForm .inventory-purchase-line'), "A purchase line keeps the product identity, quantity, cost, selling price, tax treatment and currency together.");
          await typeTarget("#goodsBoughtForm .inventory-purchase-line .goods-buy-name", "USB-C Travel Hub", "Enter the sample product name.", { narration: { narrate: false } });
          await typeTarget("#goodsBoughtForm .inventory-purchase-line .goods-buy-qty", "3", "Enter quantity three.", { direct: true, narration: { narrate: false } });
          await typeTarget("#goodsBoughtForm .inventory-purchase-line .goods-buy-price", "85", "Enter the unit purchase cost.", { direct: true, narration: { narrate: false } });
          await typeTarget("#goodsBoughtForm .inventory-purchase-line .goods-buy-selling", "129", "Enter the intended selling price.", { direct: true, narration: { narrate: false } });
          await lecture("#goodsBoughtTotalAmount", "The purchase total recalculates from the line values, so the amount being added to inventory is visible before saving.");
          await lecture("#goodsPurchaseWalletSelect", "Wallet deduction is optional. Selecting a wallet links the stock purchase with the corresponding cash movement; skipping it keeps only the inventory record.");
          await clickTarget('#goodsBoughtForm button[type="submit"]', "Save Purchase records this sample stock line in the current Demo session.", { after: 900 });
        }),
        chapter("Create a sale", "Build an invoice line, choose a customer and see how payment and balance values remain connected.", 10, async () => {
          await clickNav("goods");
          await clickTarget("#inventoryActionsBtn", "Open Inventory actions.", { narration: { narrate: false } });
          await clickTarget("#openGoodsSoldBtn", "Choose Create sale.", { narration: { narrate: false } });
          await selectTarget(() => firstVisible('#goodsSoldForm .goods-sale-item'), "iPhone 16 Pro 256GB", "Select the iPhone stock line for this sample invoice.", { narration: { narrate: false } });
          await selectTarget("#goodsCustomerSelect", "Blue Horizon LLC", "Choose the customer record.", { narration: { narrate: false } });
          await typeTarget(() => firstVisible('#goodsSoldForm .goods-sale-qty'), "1", "Set quantity to one.", { direct: true, narration: { narrate: false } });
          await typeTarget(() => firstVisible('#goodsSoldForm .goods-sale-price'), "4099", "Set the unit selling price.", { direct: true, narration: { narrate: false } });
          await typeTarget("#goodsSalePaidAmount", "4099", "Enter the amount received.", { direct: true, narration: { narrate: false } });
          await lecture("#goodsSaleGrandTotal", "Invoice total, paid amount and outstanding balance update together, keeping the sale and receivable position consistent.");
          await lecture("#goodsSaleWalletSelect", "An optional wallet top-up can connect the receipt with a financial account without changing the invoice logic.");
          await closeVisibleModal();
        }),
        chapter("Saved carts and proforma", "Open temporary carts and see how a sale can be prepared without reducing stock.", 6, async () => {
          await clickNav("goods");
          await clickTarget("#openInventoryDraftsBtn", "Open saved carts.", { narration: { narrate: false } });
          await lecture("#inventoryDraftsView", "Saved carts act as temporary or proforma sales. Stock is not reduced until a cart is finalized into a real sale.");
          await lecture("#inventoryDraftsList", "Existing carts keep their customer, items and draft totals together for later continuation.");
          await clickTarget("#inventoryNewDraftFromListBtn", "Start a new draft cart.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.modal:not(.hide), [role="dialog"]'), "The draft workspace lets you assemble items first, then save or finalize only when the sale is ready.");
          await closeVisibleModal();
          await clickTarget(() => firstVisible('#inventoryDraftsView .inventoryBackToStockBtn'), "Return to Inventory.", { narration: { narrate: false } });
        }),
        chapter("Customers, invoices and balances", "Search a customer, open the record and inspect invoice, settlement and statement controls.", 8, async () => {
          await clickNav("goods");
          await clickTarget("#openInventoryCustomersBtn", "Open Customers.", { narration: { narrate: false } });
          await lecture("#inventoryOutstandingList", "Customer view combines contact information with invoice totals, amounts paid and outstanding balances.");
          await typeTarget(() => firstVisible('#inventoryOutstandingList .inventoryOutstandingSearchInput'), "Blue Horizon", "Search for Blue Horizon.", { narration: { narrate: false } });
          await clickTarget(() => firstVisible('#inventoryOutstandingList .inventoryOutstandingCustomerOpenBtn'), "Open the customer record.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#inventoryCustomerBody,.modal:not(.hide) .inventory-customer-record,.modal:not(.hide)'), "The customer record keeps invoice history and payment position together, so outstanding amounts can be traced to specific sales.");
          await lecture(() => firstVisible('.modal:not(.hide) .inventoryCustomerInvoicePdfBtn,.modal:not(.hide) .inventoryCustomerInvoiceEditBtn,.modal:not(.hide) button'), "Invoice-level controls provide PDF, edit and settlement actions where the record permits them.");
          await closeVisibleModal();
          await clickTarget(() => firstVisible('#inventoryCustomersView .inventoryBackToStockBtn'), "Return to Inventory.", { narration: { narrate: false } });
        }),
        chapter("Product details and invoice history", "Open a category, inspect one SKU and expose its invoice history and item actions.", 9, async () => {
          await clickNav("goods");
          await clickTarget(() => firstVisible('#goodsList .inventorySectionActionBtn[data-action="open"]'), "Open the first inventory category.", { narration: { narrate: false } });
          await lecture("#inventorySectionModalBody", "The category overlay separates individual products while preserving brand, variant, stock and pricing context.");
          await clickTarget(() => firstVisible('#inventorySectionModalBody .inventorySkuHistoryBtn'), "Open invoice history for the first product.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#inventorySectionModalBody .inventory-section-item-history:not(.hide)'), "Invoice history joins purchases and sales chronologically, making stock movement auditable at product level.");
          await clickTarget(() => firstVisible('#inventorySectionModalBody [data-section-sku-menu]'), "Open the product action menu.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#inventorySectionModalBody .sectionSkuActionBtn[data-action="details"],#inventorySectionModalBody .sectionSkuActionBtn[data-action="pdf"],#inventorySectionModalBody .sectionSkuActionBtn[data-action="edit-bought"]'), "The product menu keeps details, PDF, restock, edit and lifecycle actions close to the exact SKU they affect.");
          await clickTarget(() => firstVisible('#inventorySectionModalBody .sectionSkuActionBtn[data-action="details"]'), "Open full product details.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.modal:not(.hide) .inventory-item-detail,.modal:not(.hide) .inventory-detail,.modal:not(.hide)'), "Full details expose the selected product without losing the category context behind it.");
          await closeVisibleModal();
          await closeVisibleModal();
        }),
        chapter("Categories, brands and variants", "Inspect the catalog hierarchy used to organize products without rewriting transaction history.", 6, async () => {
          await clickNav("goods");
          await clickTarget("#openInventoryBrandsBtn", "Open Brands and variants.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.modal:not(.hide), [role="dialog"]'), "Catalog management separates category, brand, product line and variant structure from purchase and sale transactions.");
          await lecture(() => firstVisible('.modal:not(.hide) .inventoryCatalogActionBtn,.modal:not(.hide) [data-catalog-menu],.modal:not(.hide) button'), "Edit and delete controls apply to catalog definitions, while historical invoices retain the transaction information they were created with.");
          await lecture(() => firstVisible('.modal:not(.hide) input,.modal:not(.hide) select'), "These controls let a large inventory remain consistently classified as new brands and variants are introduced.");
          await closeVisibleModal();
          await lecture("#inventoryGridSortSelect", "The Arrange control can sort category grids by custom order, name or stock condition without changing any stock quantity.");
        }),
        chapter("Product barcodes", "Open barcode records, search a product and understand label and scanner integration.", 6, async () => {
          await clickNav("goods");
          await clickTarget("#openInventoryBarcodesBtn", "Open Product Barcodes.", { narration: { narrate: false } });
          await lecture("#inventoryBarcodesList", "Each stock product can keep its own barcode for lookup and compact price-label printing, including sold or out-of-stock items.");
          await typeTarget("#inventoryBarcodeSearch", "iPhone", "Search the barcode list for iPhone.", { narration: { narrate: false } });
          await lecture("#inventoryBarcodesList", "Barcode search narrows labels to the matching product before printing or scanning.");
          await lecture("#inventoryScannerFromBarcodesBtn", "The Scanner uses the same product identity to add or find stock quickly when camera access is available on the device.");
          await clickTarget(() => firstVisible('#inventoryBarcodesView .inventoryBackToStockBtn'), "Return to Inventory.", { narration: { narrate: false } });
        }),
        chapter("Inventory reports and exports", "Locate full inventory PDF, CSV and audit exports without triggering file downloads during the lesson.", 6, async () => {
          await clickNav("goods");
          await clickTarget("#inventoryActionsBtn", "Open Inventory actions.", { narration: { narrate: false } });
          await lecture("#downloadInventoryFullPdfBtn", "The full PDF combines inventory summary, brands, items, stock and detailed product information into a readable report.");
          await lecture(() => firstVisible('[data-section-csv-download="inventory"]'), "CSV export provides the inventory data in a spreadsheet-friendly format for further analysis.");
          await lecture("#downloadInventoryAuditReportBtn", "Audit Report exports a deeper spreadsheet-oriented inventory audit for review and reconciliation.");
          await lecture("#inventoryActionsBtn", "These export controls are demonstrated without downloading files, so the guided lesson remains uninterrupted.");
          closeTransientMenus();
        })
      ]
    },
    ...
    "triplem-ai": {
      label: "Triplem AI",
      chapters: [
        chapter("AI workspace orientation", "Understand the private chat workspace, suggestions, composer and verified-source pattern before asking a question.", 6, async () => {
          await clickNav("triplem-ai");
          await lecture(() => firstVisible('#triplemAiRoot .triplem-ai-console-head'), "Triplem AI stays inside the authenticated workspace and keeps the current model and key-security state visible at the top of the console.");
          await lecture(() => firstVisible('#triplemAiRoot .triplem-ai-secure-pill'), "The secured-key indicator represents the live product pattern where the connected API credential is protected rather than displayed back in full.");
          await lecture(() => firstVisible('#triplemAiRoot .triplem-ai-suggestions'), "Prompt suggestions are starting points only. You can ignore them and ask a normal finance or workspace question in your own words.");
          await lecture("#triplemAiInput", "The composer supports ordinary language questions and follow-ups, so you do not need to learn special commands to inspect your workspace.");
          await lecture("#triplemAiThread", "Answers remain in one conversation thread, preserving the context needed for meaningful follow-up questions.");
        }),
        chapter("Expense analysis with sources", "Ask for a September expense summary, then inspect the answer, references and linked source records.", 8, async () => {
          await clickNav("triplem-ai");
          let input = firstVisible("#triplemAiInput");
          if (!input) { await sleep(520); input = firstVisible("#triplemAiInput"); }
          const before = doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length;
          await typeTarget(input, "Summarize September expenses and the largest categories.", "Type the expense-analysis question.", { narration: { narrate: false }, delay: Math.min(42, pace("type")) });
          await pressEnter(input, "Send the question.");
          await waitUntil(() => doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length > before, 3200);
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai')).at(-1), "The answer summarizes the sample AED expense total and identifies the largest categories instead of merely repeating every transaction.");
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-reference-box')).at(-1), "References show which workspace areas and records support the answer, making the response easier to verify.");
          await clickTarget(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-record-card')).at(-3) || firstVisible('#triplemAiThread .triplem-ai-record-card'), "Open one linked source record.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.modal:not(.hide), [role="dialog"]'), "Linked records open through the normal Triplem VIP detail interface, so AI analysis remains traceable to the underlying transaction.");
          await closeVisibleModal();
        }),
        chapter("Inventory intelligence", "Ask which inventory items are moving fastest and verify the product records behind the conclusion.", 7, async () => {
          await clickNav("triplem-ai");
          const input = firstVisible("#triplemAiInput");
          const before = doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length;
          await typeTarget(input, "Which inventory items are moving fastest?", "Type an inventory-performance question.", { narration: { narrate: false }, delay: Math.min(42, pace("type")) });
          await pressEnter(input, "Send the inventory question.");
          await waitUntil(() => doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length > before, 3200);
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai')).at(-1), "The Demo compares purchased, sold and remaining quantities and explains why the iPhone line has the strongest sell-through rate.");
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-records')).at(-1), "Product cards beneath the answer expose the exact inventory records used for the comparison.");
          await clickTarget(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-records')).at(-1)?.querySelector('.triplem-ai-record-card'), "Open the first verified inventory record.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.modal:not(.hide), [role="dialog"]'), "The linked product opens in its native detail view without forcing you to leave the AI conversation.");
          await closeVisibleModal();
        }),
        chapter("Follow-up context", "Continue the same conversation and see how a second question can build on the workspace topic.", 5, async () => {
          await clickNav("triplem-ai");
          const input = firstVisible("#triplemAiInput");
          const before = doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length;
          await typeTarget(input, "For inventory, what remains in stock after those sales?", "Type a follow-up question in the same thread.", { narration: { narrate: false }, delay: Math.min(42, pace("type")) });
          await pressEnter(input, "Send the follow-up.");
          await waitUntil(() => doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length > before, 3200);
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai')).at(-1), "Follow-ups remain inside the same thread, so the user can refine an analysis instead of restarting from a separate screen.");
          await lecture("#triplemAiThread", "The conversation remains scrollable and keeps earlier questions, answers, records and references together for review.");
        }),
        chapter("Draft-first financial actions", "Ask AI to prepare an expense draft, review its fields and confirm that nothing changes until finalization.", 8, async () => {
          await clickNav("triplem-ai");
          const input = firstVisible("#triplemAiInput");
          const before = doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length;
          await typeTarget(input, "Prepare a draft expense for AED 125 for office supplies.", "Ask Triplem AI to prepare an expense draft.", { narration: { narrate: false }, delay: Math.min(42, pace("type")) });
          await pressEnter(input, "Send the draft request.");
          await waitUntil(() => doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai').length > before && firstVisible('#triplemAiThread .triplem-ai-draft-card'), 3200);
          await lecture(() => Array.from(doc.querySelectorAll('#triplemAiThread .triplem-ai-message.is-ai')).at(-1), "Triplem AI prepares an isolated draft and explicitly keeps balances unchanged until the user reviews and finalizes it.");
          await clickTarget(() => firstVisible('#triplemAiThread .triplem-ai-draft-card'), "Open the prepared AI Draft.", { narration: { narrate: false } });
          await lecture(() => firstVisible('.triplem-ai-draft-modal form,.modal:not(.hide) form'), "The review form exposes the draft amount, date, category and wallet fields before any financial record can be finalized.");
          await lecture(() => firstVisible('.modal:not(.hide) .triplem-ai-draft-actions,.modal:not(.hide) .modal-footer'), "Finalize and delete remain explicit user actions. The automatic Demo does not finalize this draft.");
          await closeVisibleModal();
        }),
        chapter("AI settings and privacy boundary", "Inspect the protected integration settings without exposing or changing a real API secret.", 7, async () => {
          await clickNav("triplem-ai");
          await clickTarget(() => firstVisible('#triplemAiRoot [data-triplem-ai-settings]'), "Open Triplem AI settings.", { narration: { narrate: false } });
          await lecture(() => firstVisible('#triplemAiSettingsTitle'), "Settings separate AI access from normal financial features and show the connected integration state in one place.");
          await lecture(() => firstVisible('#triplemAiSettingsModal input,#triplemAiSettingsModal .triplem-ai-settings-field input,.triplem-ai-settings-modal input'), "The credential field follows a masked pattern; the Demo never requests or stores a real Gemini key.");
          await lecture(() => firstVisible('#triplemAiModel,.triplem-ai-settings-modal select'), "Model selection is independent from the finance data, so changing the model does not rewrite workspace records.");
          await lecture(() => firstVisible('.triplem-ai-settings-notice'), "The safety notice reinforces the draft-first rule: AI may prepare supported records, but financial changes remain reviewable before finalization.");
          await closeVisibleModal();
          await lecture("#triplemAiThread", "At any point, you can verify an AI answer through its linked records or inspect the source module directly.");
        })
      ]
    },
  ...
  async function startChapter(areaKey, index = 0) {
    ...
    try {
      ...
      if (outcome === "complete" && !cancelled) {
        ...
        currentStep = 1;
        totalSteps = 1;
        await showRemark(`${AREA_LABELS[areaKey] || GUIDES[areaKey]?.label || "This section"} guided chapter series is complete.`, { narrate: false });
        await sleep(520);
      }
    } finally {
      running = false;
  ...
  let focusRingFrame = 0;
  function requestFocusRingSync() {
    if (!highlight || !focusRing) return;
    if (focusRingFrame) global.cancelAnimationFrame?.(focusRingFrame);
    focusRingFrame = global.requestAnimationFrame?.(() => { focusRingFrame = 0; syncFocusRing(); }) || 0;
  }
  global.addEventListener?.("resize", requestFocusRingSync, { passive: true });
  doc.addEventListener?.("scroll", requestFocusRingSync, { passive: true, capture: true });
