...
(() => {
  ...
  function cleanSemanticAnalysis(raw) {
    ...
    const allowed = new Set([
      ...
      "reports","accounting","chart_accounts","journals","receivables_payables","bank_reconciliation","tax_accounting","accounting_reports","accounting_controls",
      "loans","installments","assets","notes","bitcoin","company_team","platforms",
    ...
    return {
      ...
      currency: (() => { const code = safeText(raw.currency).toUpperCase(); const allowed = new Set(window.TriplemCurrencyRegistry?.billingCurrencies?.().map(row => row.code) || []); return code === "UNKNOWN" || allowed.has(code) ? (code === "UNKNOWN" ? "unknown" : code) : "unknown"; })(),
      intents
    };
  }
