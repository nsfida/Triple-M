...
function personalMessageUnreadCount(item, admin = isAppAdminSession()){
  if (!isPersonalMessageThread(item)) return 0;
  ...
  return Math.max(0, Number(admin ? item.unread_for_admin : item.unread_for_user) || 0);
}

function isIncomingPersonalMessage(item, admin = isAppAdminSession()){
  if (!isPersonalMessageThread(item) || personalMessageUnreadCount(item, admin) <= 0) return false;
  const role = String(item.last_message_role || "").toLowerCase();
  return admin ? (role === "user") : (role === "admin");
}
...
async function fetchPersonalMessageThreads(limit = 300, status = null){
  if (isAppAdminSession()) {
    ...
    return (Array.isArray(result?.items) ? result.items : []).filter(isPersonalMessageThread);
  }
...
function floatingHistoryRowsHtml(messages){
  const admin = isAppAdminSession();
...
function ensureFloatingMessageCard(thread){
  ...
  if (!card) {
    ...
    card.innerHTML = `
      <div class="message-float-card-head">
        ...
        <div class="message-float-card-controls">
          <button type="button" class="message-float-card-avatar-toggle" data-float-collapse="${escapeHtml(id)}" data-float-card-avatar aria-label="Minimize quick chat" title="Minimize chat">${escapeHtml(identity.initial)}</button>
    ...
    if (mobileAvatar && mobileAvatar.textContent !== identity.initial) mobileAvatar.textContent = identity.initial;
  }
  return card;
}
...
function renderFloatingMessageCard(thread, options = {}){
  ...
  if (historyEl && historyEl.dataset.renderKey !== nextHistoryKey) {
    if (history) historyEl.innerHTML = floatingHistoryRowsHtml(history.messages);
  ...
  if (options.focus && input) requestAnimationFrame(() => input.focus({ preventScroll: true }));
}

function visibleFloatingThreads(){
  const mobile = isFloatingMobile();
  const activeId = String(floatingMessageState.activeId || "");
  return Array.from(floatingMessageState.visibleIds)
    .filter(id => mobile ? String(id) !== activeId : !floatingMessageState.openIds.has(String(id)))
...
function renderFloatingMessageCards(){
  ...
  syncFloatingChatPageLock(openIds.length > 0);
}
...
function renderFloatingMessageHeads(){
  ...
  const admin = isAppAdminSession();
  const threads = visibleFloatingThreads();
      ...
      return `
        <div class="message-float-head${active}" data-float-head="${escapeHtml(id)}">
          <button type="button" class="message-float-bubble${active}" data-float-thread="${escapeHtml(id)}" aria-label="Quick chat with ${escapeHtml(identity.username)}" title="${escapeHtml(identity.username)} · ${escapeHtml(identity.detail)}">
            <span class="message-float-avatar">${escapeHtml(identity.initial)}</span>
...
function syncFloatingMessageBubbles(threads){
  ...
  const allThreads = (Array.isArray(threads) ? threads : []).filter(isPersonalMessageThread);
  ...
  allThreads.forEach(thread => {
    ...
    if (genuinelyNewIncoming && dismissedMarker !== incomingMarker) {
      ...
      floatingMessageState.visibleIds.add(id);
      floatingMessageState.dismissedSignatures.delete(id);
      if (!newestChanged || new Date(thread.last_message_at || 0) > new Date(newestChanged.last_message_at || 0)) newestChanged = thread;
    }
  });
  ...
  if (newestChanged && !(getActiveTabKey() === "messages" && messagesUiState.selectedId === newestChanged.id)) {
    showFloatingMessagePeek(newestChanged);
    const id = String(newestChanged.id);
    if (floatingMessageState.openIds.has(id)) {
      renderFloatingMessageCard(floatingMessageState.threadMap.get(id), { scrollBottom: false, focus: false });
    }
  }
}
...
function appendOptimisticFloatingReply(threadId, body){
  ...
  cache.messages.push({
    id: optimisticId,
    sender_role: isAppAdminSession() ? "admin" : "user",
    sender_label: "You",
    body,
...
function endFloatingDockDrag(e){
  ...
  const stack = document.getElementById("messageFloatStack");
  const movedStackRect = stack?.getBoundingClientRect();
  ...
  if (!moved) return;
  floatingMessageState.suppressClickUntil = Date.now() + 320;

  if (shouldDismiss) {
    if (isFloatingMobile() && drag.mode === "dock" && !floatingMessageState.mobileExpanded) dismissAllFloatingThreads();
    else dismissFloatingThread(drag.threadId);
    return;
  }

  if (drag.mode === "head" && drag.head) {
    storeFloatingHeadPosition(drag.threadId, drag.head.getBoundingClientRect());
    applyFloatingMobileHeadLayout(true);
    return;
  }
  ...
  const rawBottom = movedStackRect ? window.innerHeight - movedStackRect.bottom : gap;
  floatingMessageState.position = {
    side,
...
function bindFloatingMessageDock(){
  ...
  root.addEventListener("click", e => {
    ...
    if (threadTarget) {
      ...
      if (floatingMessageState.openIds.has(id) && floatingMessageState.activeId === id) {
        ...
        // Render first so the page is locked at its exact current scroll position
        // before any history update or textarea focus can ask the browser to scroll.
        renderFloatingMessageHeads();
        loadFloatingMessageHistory(id, { scrollBottom: true, focus: true }).catch(() => {});
        setTimeout(() => refreshAdminCommsBadges().catch(() => {}), 40);
      }
    }
  });
  ...
  if (window.visualViewport) {
    ...
    window.visualViewport.addEventListener("scroll", () => applyFloatingDockPosition(false), { passive: true });
  }
}
...
async function loadAdminMessagesPreview(){
  ...
  try {
    const admin = isAppAdminSession();
    const result = admin
      ? await supabaseRpc("app_admin_list_inquiries", { p_status: null, p_limit: 80 })
      : await supabaseRpc("app_list_my_inquiries", {});
    const items = (Array.isArray(result?.items) ? result.items : []).filter(isPersonalMessageThread).slice(0, 12);
    ...
    list.innerHTML = items.map(item => {
      ...
      const who = admin
        ...
        : "Admin";
      return `
      <div class="admin-comms-item admin-comms-message-row ${unread ? "unread" : ""}" data-message-preview-row="${escapeHtml(item.id)}">
        <button type="button" class="admin-comms-message-main" data-preview-inquiry="${escapeHtml(item.id)}" title="Open floating chat">
          <span class="admin-comms-item-icon inquiry"><i class="fa-solid fa-comments"></i></span>
          <span class="admin-comms-message-copy">
            <span class="admin-comms-item-title">${escapeHtml(item.subject || "Conversation")}</span>
            <span class="admin-comms-item-body">${escapeHtml(who)} · ${escapeHtml(item.last_message_preview || item.body || "")}</span>
            <span class="admin-comms-item-meta">${escapeHtml(formatRelativeTime(item.last_message_at || item.created_at))}</span>
          </span>
        </button>
        <button type="button" class="btn ghost admin-comms-message-open-full" data-preview-open-full="${escapeHtml(item.id)}" title="Open in Messages"><i class="fa-solid fa-up-right-from-square"></i><span>Open</span></button>
      </div>`;
    }).join("");
  } catch (err) {
...
async function renderMessagesPanel(options = {}){
  ...
  if (!silent) {
    ...
    if (subtitle) {
      subtitle.textContent = admin
        ? "Private conversations with registered users. Access requests stay in Notifications."
  ...
  try {
    ...
    if (admin) {
      ...
      items = (Array.isArray(result?.items) ? result.items : []).filter(isPersonalMessageThread);
    } else {
...
async function openInquiryThread(inquiryId, options = {}){
  ...
  try {
    ...
    header.innerHTML = `
      ${admin ? `<div class="messages-thread-header-top">
        <button type="button" class="btn ghost messages-thread-back" data-messages-back aria-label="Back to conversations">
          <i class="fa-solid fa-arrow-left"></i> Back
        </button>
      </div>` : ""}
      <div class="messages-thread-header-main">
        <div>
          <h4>${admin ? escapeHtml(inquiry.subject || "Conversation") : "Triplem VIP Admin"}</h4>
          <p>${admin ? `${escapeHtml(inquiry.sender_display_name || "Guest")}${inquiry.sender_username ? ` · @${escapeHtml(inquiry.sender_username)}` : ""}${inquiry.source === "landing" ? " · Login page request" : ""}` : "Private conversation · Continue here whenever you need assistance."}</p>
          ${admin && contactBits ? `<p class="messages-thread-contact">${contactBits}</p>` : ""}
        </div>
        ${admin ? `<span class="message-status-pill ${escapeHtml(inquiry.status || "open")}">${escapeHtml(inquiry.status || "open")}</span>` : ""}
      </div>
    ...
    if (replyBar) {
      ...
      if (replyInput && !admin) replyInput.placeholder = "Write a message to Admin…";
  ...
  document.getElementById("adminNotifyList")?.addEventListener("click", async e => {
    ...
    try {
      ...
      if (row?.dataset.notificationId) {
        ...
        if (n?.kind === "inquiry" && isMemberAccessRequestNotification(n)) {
          document.querySelectorAll(".menu-dropdown.open").forEach(p => p.classList.remove("open"));
...
document.getElementById("adminMessagesPreviewList")?.addEventListener("click", async e => {
    const fullBtn = e.target.closest("[data-preview-open-full]");
    const floatBtn = e.target.closest("[data-preview-inquiry]");
    if (!fullBtn && !floatBtn) return;
    e.preventDefault();
    e.stopPropagation();
    document.querySelectorAll(".menu-dropdown.open").forEach(p => p.classList.remove("open"));
    if (fullBtn) {
      goToMessagesTab(fullBtn.dataset.previewOpenFull, { compose: false });
      return;
    }
