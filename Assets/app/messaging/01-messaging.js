...
function syncLiveChatOfferDock(notificationRows){
  ...
  if (!messagingLiveEligible()) {
    dock.classList.add("hide"); dock.replaceChildren(); liveChatOfferState.pendingIds.clear(); liveChatOfferState.signature = ""; return;
  }
...
async function openLiveChatAssignmentDetails(notification){
  ...
  const preview = String(payload.message || notification.body || "A visitor is waiting for support.");
  modal.innerHTML = `<div class="modal-backdrop" data-live-assignment-close></div>
    <div class="modal-dialog settings-sheet live-chat-assignment-sheet" role="dialog" aria-modal="true" aria-labelledby="liveChatAssignmentTitle">
      <div class="settings-sheet-head">
        <div><h3 id="liveChatAssignmentTitle"><i class="fa-solid fa-headset"></i> Live chat request</h3><p>Triplem VIP Support queue</p></div>
  ...
  modal.querySelector("[data-live-assignment-accept]").onclick = async (e) => {
    const btn = e.currentTarget;
    const err = modal.querySelector("[data-live-assignment-error]");
    try {
      btn.disabled = true;
      if (err) { err.textContent = ""; err.classList.remove("show"); }
      const accepted = unwrapRpcJson(await supabaseRpc("app_live_chat_accept_assignment", { p_inquiry_id: inquiryId })) || {};
      close();
      noteMessagingLocalMutation();
      document.querySelectorAll(".menu-dropdown.open").forEach(p => p.classList.remove("open"));
      const currentId = String(state?.sessionUser?.id || state?.sessionUser?.user_id || "");
      const acceptedInquiry = accepted?.inquiry ? { ...accepted.inquiry, source: accepted.inquiry.source || "landing", subject: accepted.inquiry.subject || "Live Chat Support", support_assignment_status: "accepted", support_assigned_to: accepted.inquiry.support_assigned_to || currentId } : null;
      if (acceptedInquiry) revealFloatingThread(acceptedInquiry, { open: true, focus: true, forceDatabase: true });
      const threads = await fetchPersonalMessageThreads(300, null).catch(() => []);
      syncFloatingMessageBubbles(threads);
      const thread = threads.find(item => String(item.id) === inquiryId) || acceptedInquiry;
      if (thread) revealFloatingThread(thread, { open: true, focus: true, forceDatabase: true });
      await refreshAdminCommsBadges().catch(() => {});
    } catch (ex) {
  ...
  try {
    ...
    list.innerHTML = adminCommsState.notifications.map(n => {
      ...
      const paymentReview = source === "admin" && n.kind === "subscription_payment" && payload.request_id
        ...
        : "";
      const assignmentOpen = !String(payload.assignment_status || "").trim() || String(payload.assignment_status || "").toLowerCase() === "pending";
