...
function isSecurityRecoveryNotification(item){
  if (!item || String(item.kind || "").toLowerCase() !== "system") return false;
  return String(notificationPayloadObject(item).type || "").toLowerCase() === "security_recovery_approval";
}
...
function notificationBelongsInBell(item){
  ...
  return NOTIFICATION_CENTER_KINDS.has(String(item.kind || "").toLowerCase())
    ...
    || isSecurityRecoveryNotification(item)
...
function isOwnedSupportThread(item){
  if (!isLandingLiveChatThread(item)) return false;
  const assigned = String(item.support_assigned_to || "");
  const current = String(state?.sessionUser?.id || state?.sessionUser?.user_id || "");
  const status = String(item.support_assignment_status || "").toLowerCase();
  return (status === "accepted" || status === "closed") && !!assigned && assigned === current;
}

function isUserVisibleMessageThread(item){
  return isPersonalMessageThread(item) || isOwnedSupportThread(item);
}
    ...
    return `
      <div class="message-float-chat-row ${mine ? "mine" : "theirs"}" data-message-id="${escapeHtml(String(m.id || ""))}">
        <div class="message-float-chat-bubble${m._optimistic ? " chat-bubble-pending" : ""}">
          <div class="message-float-chat-body" dir="auto">${escapeHtml(String(m.body || ""))}</div>
    ...
    card.innerHTML = `
      ...
      <div class="message-float-reply">
        ...
        <div class="message-float-actions">
          ...
          ${liveSupportSide && isAssignedSupportThread(thread) ? `<button type="button" class="btn ghost message-float-transfer" data-float-transfer="${escapeHtml(id)}"><i class="fa-solid fa-right-left"></i> Transfer</button><button type="button" class="btn ghost live-support-end-chat-btn message-float-end-chat" data-float-end-chat="${escapeHtml(id)}" title="End this live chat"><i class="fa-solid fa-phone-slash"></i><span>End Chat</span></button>` : ""}
...
async function endAssignedLiveChat(inquiryId){
  const id = String(inquiryId || "").trim();
  if (!id) return false;
  if (!(await appConfirmDelete("End this live support chat? The visitor will immediately see that the conversation was ended by the assigned Agent, and no further messages can be sent in this chat.", { title: "End live chat?", confirmLabel: "End Chat" }))) return false;
  const result = await supabaseRpc("app_live_chat_agent_end", { p_inquiry_id: id });
  noteMessagingLocalMutation();
  floatingMessageState.visibleIds.delete(id);
  floatingMessageState.openIds.delete(id);
  floatingMessageState.activeId = floatingMessageState.activeId === id ? null : floatingMessageState.activeId;
  renderFloatingMessageHeads();
  await refreshAdminCommsBadges().catch(() => {});
  if (getActiveTabKey() === "messages") await renderMessagesPanel({ silent: true }).catch(() => {});
  return result || true;
}
...
function bindFloatingMessageDock(){
  ...
  root.addEventListener("click", e => {
    ...
    const endChat = e.target.closest("[data-float-end-chat]");
    ...
    if (endChat) { e.preventDefault(); endAssignedLiveChat(endChat.dataset.floatEndChat).catch(err => alert(err.message || "Could not end live chat.")); return; }
    if (send) {
      e.preventDefault();
...
function startMessagingLiveSync(){
  ...
  // Warm the hidden dropdowns during idle time so the bell/chat menus feel
  // immediate on first use. Database latency is absorbed in the background.
  const warmCommsMenus = () => {
    if (!messagingLiveEligible()) return;
    loadAdminNotificationsDropdown().catch(() => {});
    loadAdminMessagesPreview().catch(() => {});
  };
  if (typeof window.requestIdleCallback === "function") window.requestIdleCallback(warmCommsMenus, { timeout: 900 });
  else window.setTimeout(warmCommsMenus, 320);
}
  ...
  try {
    ...
    if (messagingLiveState.liveChatPresenceSweepAvailable !== false
      ...
      try {
        ...
        messagingLiveState.liveChatPresenceSweepAvailable = true;
        if (Number(swept?.closed_count || 0) > 0) {
          messagingLiveState.fingerprint = null;
          messagingLiveState.fallbackFingerprint = null;
        }
      } catch (presenceErr) {
        if (/app_live_chat_presence_sweep|Could not find the function|PGRST202|schema cache/i.test(String(presenceErr?.message || presenceErr))) {
          messagingLiveState.liveChatPresenceSweepAvailable = false;
        } else {
          console.warn("Live Chat visitor presence sweep failed:", presenceErr);
        }
      }
    }
    ...
    list.innerHTML = adminCommsState.notifications.map(n => {
      ...
      const reminderPreview = n.kind === "note_reminder"
        ...
        : n.kind === "installment_due"
          ...
          : "";
      const extra = isSecurityRecoveryNotification(n)
        ? `<p class="admin-comms-item-body"><strong>Matching code ${escapeHtml(payload.display_code || "")}</strong> · Approve only from a browser you previously trusted.</p>`
      ...
      return `
        <div class="admin-comms-item ${unread ? "unread" : ""}" data-notification-id="${escapeHtml(n.id)}" data-notif-source="${escapeHtml(source)}" data-related-user="${escapeHtml(n.related_user_id || "")}">
          <div class="admin-comms-item-icon ${isSecurityRecoveryNotification(n) ? "warn" : isLiveChatOfferNotification(n) ? "inquiry" : notificationIconClass(n.kind)}">
            <i class="fa-solid ${isSecurityRecoveryNotification(n) ? "fa-shield-halved" : isLiveChatOfferNotification(n) ? "fa-headset" : notificationIcon(n.kind)}"></i>
          </div>
          <div>
...
async function loadAdminMessagesPreview(){
  ...
  try {
    ...
    list.innerHTML = items.map(item => {
      const supportThread = isOwnedSupportThread(item);
...
async function renderMessagesPanel(options = {}){
  ...
  try {
    ...
    if (!admin) {
      ...
      const canOpenOwnedThread = t => String(t?.status || "open").toLowerCase() !== "archived" || isOwnedSupportThread(t);
      const requested = messagesUiState.pendingOpenId && items.find(t => String(t.id) === String(messagesUiState.pendingOpenId) && canOpenOwnedThread(t));
      const selected = messagesUiState.selectedId && items.find(t => String(t.id) === String(messagesUiState.selectedId) && canOpenOwnedThread(t));
...
async function openInquiryThread(inquiryId, options = {}){
  ...
  try {
    ...
    const admin = isAppAdminSession();
    const supportThread = isOwnedSupportThread(inquiry);
    const supportActive = isAssignedSupportThread(inquiry);
    const supportAiMode = String(inquiry.support_ai_mode || "").toLowerCase();
    const supportRouteStatus = String(inquiry.support_assignment_status || "").toLowerCase();
    const visitorPresence = String(inquiry.visitor_presence || "").toLowerCase();
    const closedBy = String(inquiry.support_closed_by || "").toLowerCase();
    const closedByName = String(inquiry.support_closed_by_name || "").trim();
    const liveChatIdentityNote = supportRouteStatus === "closed"
      ? ` · Ended by ${closedBy === "agent" ? (closedByName || "Agent") : closedBy === "visitor" ? "Visitor" : "Triplem VIP"}`
      : supportAiMode === "human_pending"
        ? " · AI handoff requested"
        : supportAiMode === "human" ? " · Agent support" : "";
    const presenceLabel = supportRouteStatus === "closed"
      ? (String(inquiry.support_close_reason || "").toLowerCase() === "visitor_left_website" ? "Visitor left website · chat ended" : `Chat ended by ${closedBy === "agent" ? (closedByName || "Agent") : closedBy === "visitor" ? "Visitor" : "Triplem VIP"}`)
      : visitorPresence === "leaving" ? "Visitor connection closing…"
      : visitorPresence === "online" ? "Visitor online"
      : "Visitor presence unknown";
    const presenceClass = supportRouteStatus === "closed" ? "is-ended" : visitorPresence === "online" ? "is-online" : visitorPresence === "leaving" ? "is-leaving" : "is-unknown";
    ...
    header.innerHTML = `
      ...
      <div class="messages-thread-header-main">
        <div>
          ...
          ${supportThread ? `<p class="live-support-visitor-presence ${escapeHtml(presenceClass)}"><i class="fa-solid ${supportRouteStatus === "closed" ? "fa-circle-stop" : visitorPresence === "online" ? "fa-circle" : "fa-signal"}"></i><span>${escapeHtml(presenceLabel)}</span></p>` : ""}
        </div>
      ...
      <div class="messages-thread-header-actions">
        ...
        ${supportActive ? `${String(inquiry.support_transfer_status || "").toLowerCase() === "pending" ? `<span class="live-support-transfer-pending"><i class="fa-solid fa-clock"></i> Transfer pending${inquiry.support_transfer_to_name ? ` · ${escapeHtml(inquiry.support_transfer_to_name)}` : ""}</span>` : `<button type="button" class="btn ghost tiny" data-live-chat-transfer="${escapeHtml(inquiry.id)}"><i class="fa-solid fa-right-left"></i> Transfer</button>`}<button type="button" class="btn ghost tiny live-support-end-chat-btn" data-live-chat-agent-end="${escapeHtml(inquiry.id)}" title="End this live chat"><i class="fa-solid fa-phone-slash"></i><span>End Chat</span></button>` : (supportThread ? `<span class="live-support-ended-state"><i class="fa-solid fa-lock"></i> Conversation ended</span>` : (!admin ? `<button type="button" class="btn ghost danger-text" data-my-inquiry-delete="${escapeHtml(inquiry.id)}"><i class="fa-solid fa-trash"></i> Delete</button>` : ""))}
      </div>`;
    ...
    if (scroll) {
      ...
      optimisticRow.innerHTML = `
        <div class="chat-bubble ${isAppAdminSession() ? "from-admin" : "from-user"}">
          ...
          <div class="chat-bubble-body" dir="auto">${escapeHtml(body)}</div>
        </div>`;
...
function bindMessagingUi(){
  ...
  function setNotificationBadgeOptimistically(count) {
    adminCommsState.unreadNotifications = Math.max(0, Number(count) || 0);
    setHeaderBadge(document.getElementById("adminNotifyCount"), adminCommsState.unreadNotifications);
  }

  function optimisticMarkNotificationRead(notificationId) {
    const id = String(notificationId || "");
    if (!id) return;
    const item = (adminCommsState.notifications || []).find(n => String(n.id) === id);
    const wasUnread = !!item && !item.is_read;
    if (item) item.is_read = true;
    const row = document.querySelector(`#adminNotifyList [data-notification-id="${CSS.escape(id)}"]`);
    row?.classList.remove("unread");
    row?.querySelectorAll("[data-notif-read]").forEach(btn => btn.remove());
    if (wasUnread) setNotificationBadgeOptimistically(adminCommsState.unreadNotifications - 1);
  }

  function optimisticDeleteNotification(notificationId) {
    const id = String(notificationId || "");
    if (!id) return;
    const item = (adminCommsState.notifications || []).find(n => String(n.id) === id);
    const wasUnread = !!item && !item.is_read;
    adminCommsState.notifications = (adminCommsState.notifications || []).filter(n => String(n.id) !== id);
    const row = document.querySelector(`#adminNotifyList [data-notification-id="${CSS.escape(id)}"]`);
    row?.remove();
    if (wasUnread) setNotificationBadgeOptimistically(adminCommsState.unreadNotifications - 1);
    const list = document.getElementById("adminNotifyList");
  ...
  function optimisticMarkAllNotificationsRead() {
    ...
    setNotificationBadgeOptimistically(0);
  }

  function runNotificationMutationInBackground(work, failureMessage) {
    Promise.resolve().then(work).then(() => {
      ...
      refreshAdminCommsBadges().catch(() => {});
      if (isAdminCommsDropdownOpen("admin-notify")) loadAdminNotificationsDropdown().catch(() => {});
      alert(error?.message || failureMessage || "Notification action failed.");
    });
  }
  ...
  if (markAllBtn) {
    markAllBtn.addEventListener("click", e => {
      ...
      runNotificationMutationInBackground(async () => {
        const requests = [supabaseRpc("app_mark_all_my_notifications_read", {})];
        if (isAppAdminSession()) requests.push(supabaseRpc("app_admin_mark_all_notifications_read", {}));
        const results = await Promise.allSettled(requests);
        const failed = results.find(result => result.status === "rejected");
        if (failed) throw failed.reason || new Error("Could not mark notifications as read.");
  ...
  document.getElementById("adminNotifyList")?.addEventListener("click", async e => {
    ...
    try {
      ...
      if (readBtn) {
        ...
        const notificationId = readBtn.dataset.notifRead;
        optimisticMarkNotificationRead(notificationId);
        runNotificationMutationInBackground(() => source === "user"
          ? supabaseRpc("app_mark_my_notification_read", { p_notification_id: notificationId })
          : supabaseRpc("app_admin_mark_notification_read", { p_notification_id: notificationId }),
      ...
      if (delBtn) {
        ...
        const notificationId = delBtn.dataset.notifDelete;
        optimisticDeleteNotification(notificationId);
        runNotificationMutationInBackground(() => source === "user"
          ? supabaseRpc("app_delete_my_notification", { p_notification_id: notificationId })
          : supabaseRpc("app_admin_delete_notification", { p_notification_id: notificationId }),
      ...
      if (row?.dataset.notificationId) {
        ...
        if (n && !n.is_read) {
          optimisticMarkNotificationRead(n.id);
          runNotificationMutationInBackground(() => source === "user"
            ? supabaseRpc("app_mark_my_notification_read", { p_notification_id: n.id })
            : supabaseRpc("app_admin_mark_notification_read", { p_notification_id: n.id }),
          "Could not mark this notification as read.");
        }
        if (source === "user" && isSecurityRecoveryNotification(n)) {
          document.querySelectorAll(".menu-dropdown.open").forEach(p => p.classList.remove("open"));
          const opened = await window.openTrustedRecoveryApprovalById?.(notificationPayloadObject(n).request_id || "");
          if (!opened) alert("Open this request from the browser that you previously trusted for account recovery.");
        } else if (source === "user" && isLiveChatOfferNotification(n)) {
          document.querySelectorAll(".menu-dropdown.open").forEach(p => p.classList.remove("open"));
...
document.getElementById("adminMessagesPreviewList")?.addEventListener("click", async e => {
  ...
  document.getElementById("messagesThreadHeader")?.addEventListener("click", async e => {
    ...
    try {
      ...
      if (supportEnd) {
        await endAssignedLiveChat(supportEnd.dataset.liveChatAgentEnd);
        return;
      }
      if (backBtn) {
        e.preventDefault();
