...
function drawAssetPdfPageChrome(doc, logoData, title, subtitle, { cover = false, fullBleed = true } = {}){
  ...
  if (fullBleed) {
    // Soft page wash (only when starting a fresh composed page)
  ...
  doc.rect(0, 0, pageW, cover ? 46 : 34, "F");
  doc.setFillColor(...T.teal);
  doc.rect(0, cover ? 46 : 34, pageW, 2.2, "F");
  // Accent slash
  doc.setFillColor(...T.amber);
  doc.triangle(pageW - 38, 0, pageW, 0, pageW, cover ? 46 : 34, "F");

  if (logoData) {
    drawFittedPdfImage(doc, logoData, 14, cover ? 10 : 7, 36, cover ? 24 : 18, { align: "left", valign: "middle" });
  }

  doc.setTextColor(...T.white);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(cover ? 16 : 12.5);
  doc.text(title, 56, cover ? 20 : 15);
  if (subtitle) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.8);
    doc.setTextColor(186, 230, 253);
    const lines = doc.splitTextToSize(String(subtitle), pageW - 100);
    doc.text(lines, 56, cover ? 28 : 22);
  }
  ...
  doc.text(`TRIPLE M · Asset Intelligence`, 14, pageH - 7);
  doc.text(`Page ${pageNo}`, pageW - 14, pageH - 7, { align: "right" });
}
...
function drawAssetPdfKpiCards(doc, cards, x, y, width){
  ...
  cards.forEach((card, i) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(6.4);
    doc.text(String(card.label || "").toUpperCase(), cx + 6, y + 7);
    doc.setTextColor(...T.ink);
    doc.setFontSize(9.2);
    doc.text(String(card.value || "—"), cx + 6, y + 15);
  });
  return y + 26;
}
...
function drawAssetPdfPanel(doc, x, y, w, h, title){
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(String(title || ""), x + 5, y + 6);
  return { contentX: x + 4, contentY: y + 11, contentW: w - 8, contentH: h - 15 };
}
...
function assetPdfTableTheme(doc, headRgb = ASSET_PDF_THEME.ink){
  const side = assetPdfSideMargin(doc);
  ...
  return {
    ...
    alternateRowStyles: { fillColor: [248, 250, 252] },
    margin: { top: 42, left: side, right: side, bottom: 24 }
  };
}
...
async function buildAssetPdfChartPack(data){
  ...
  const performanceBars = await renderAssetPdfBars(
    ["Invested", "Revenue", "Sale", "Income", "Net"],
    [{
      label: cur,
      data: [t.expenses || 0, t.revenue || 0, t.sale || 0, t.income || 0, t.net || 0],
      backgroundColor: ["#e11d48", "#0ea5e9", "#059669", "#0d9488", (t.net || 0) >= 0 ? "#059669" : "#e11d48"],
      borderRadius: 8,
      maxBarThickness: 42
    }]
  );
...
async function downloadAssetsSummaryPDF(){
  ...
  y = drawAssetPdfKpiCards(doc, [
    { label: "Invested", value: formatPdfAmount(t.expenses || 0, cur), accent: ASSET_PDF_THEME.rose },
    { label: "Revenue", value: formatPdfAmount(t.revenue || 0, cur), accent: ASSET_PDF_THEME.teal },
    { label: "Sale proceeds", value: formatPdfAmount(t.sale || 0, cur), accent: ASSET_PDF_THEME.amber },
  ...
  doc.setFontSize(7.5);
  doc.text(`Active ${t.status?.active || 0}  ·  Sold ${t.status?.sold || 0}  ·  Disposed ${t.status?.disposed || 0}  ·  Purchase base ${formatPdfAmount(t.purchase || 0, cur)}`, L.leftX, y);
  y += 6;
  ...
  doc.autoTable({
    startY: y,
    head: [["Asset", "Type", "Status", "Owned", "Spent", "Revenue", "Net"]],
    body: (data.rows || []).map(row => [
      ...
      formatPdfAmount(row.sum?.totalExpenses || 0, cur),
...
async function downloadAssetsDetailsPDF(){
  ...
  doc.autoTable({
    ...
    body: [
      ["Total invested / spent", formatPdfAmount(t.expenses || 0, cur)],
      ["Total revenue", formatPdfAmount(t.revenue || 0, cur)],
      ["Sale proceeds", formatPdfAmount(t.sale || 0, cur)],
      ["Total income", formatPdfAmount(t.income || 0, cur)],
  ...
  doc.autoTable({
    startY: y,
    head: [["Asset", "Status", "Purchase", "Spent", "Revenue", "Sale", "Income", "Net", "Owned"]],
    body: (data.rows || []).map(row => [
      row.asset.name || "—",
      typeof assetStatusLabel === "function" ? assetStatusLabel(row.asset.status) : row.asset.status,
      ...
      formatPdfAmount(row.sum?.totalIncome || 0, cur),
      formatPdfAmount(row.sum?.net || 0, cur),
...
async function renderSingleAssetPdfPages(doc, logoData, asset, txs, sum, meta = {}){
  ...
  (window.TripleMAssetMath?.monthlyBuckets?.(txs) || []).forEach(m => {
  ...
  const perfBars = await renderAssetPdfBars(
    ["Spent", "Revenue", "Sale", "Income", "Net"],
    [{
      label: cur,
      data: [sum.totalExpenses || 0, sum.revenue || 0, sum.salePrice || 0, sum.totalIncome || 0, sum.net || 0],
      backgroundColor: ["#e11d48", "#0ea5e9", "#059669", "#0d9488", (sum.net || 0) >= 0 ? "#059669" : "#e11d48"],
      borderRadius: 8,
      maxBarThickness: 40
    }]
  );
  ...
  const L = assetPdfLayout(doc);
  let y = meta.cover ? 56 : 44;
  doc.setTextColor(...ASSET_PDF_THEME.ink);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(13);
  doc.text(`${meta.indexLabel || ""}${asset.name || "Asset"}`, L.leftX, y);
  y += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...ASSET_PDF_THEME.muted);
  doc.text(`${type} · ${status} · Purchased ${asset.purchase_date || "—"} · Owned ${sum.ownership?.label || "—"} (${sum.ownership?.days || 0} days)`, L.leftX, y);
  y += 7;
  if (asset.description) {
    const desc = doc.splitTextToSize(String(asset.description), L.width);
    doc.text(desc.slice(0, 2), L.leftX, y);
    y += Math.min(desc.length, 2) * 3.8 + 2;
  }

  y = drawAssetPdfKpiCards(doc, [
    { label: "Invested", value: formatPdfAmount(sum.totalExpenses || 0, cur), accent: ASSET_PDF_THEME.rose },
    { label: "Revenue", value: formatPdfAmount(sum.revenue || 0, cur), accent: ASSET_PDF_THEME.teal },
    { label: "Sale", value: formatPdfAmount(sum.salePrice || 0, cur), accent: ASSET_PDF_THEME.amber },
    { label: "Net", value: formatPdfAmount(sum.net || 0, cur), accent: (sum.net || 0) >= 0 ? ASSET_PDF_THEME.emerald : ASSET_PDF_THEME.rose }
  ], L.leftX, y, L.width);

  const left = drawAssetPdfPanel(doc, L.leftX, y, L.half, 70, "Cost composition");
  embedAssetPdfImage(doc, mixDonut, left.contentX, left.contentY, left.contentW, left.contentH - 2, { fit: "square" });
  const right = drawAssetPdfPanel(doc, L.rightX, y, L.half, 70, "Performance stack");
  embedAssetPdfImage(doc, perfBars, right.contentX, right.contentY, right.contentW, right.contentH - 2, { fit: "contain" });
  y += 76;

  const candlePanel = drawAssetPdfPanel(doc, L.leftX, y, L.width, 62, "Ownership equity candles (OHLC)");
  embedAssetPdfImage(doc, candleImg, candlePanel.contentX, candlePanel.contentY, candlePanel.contentW, candlePanel.contentH - 2, { fit: "contain" });

  doc.addPage();
  drawAssetPdfPageChrome(doc, logoData, title, subtitle);
  y = 44;
  y = drawAssetPdfSectionLabel(doc, "Trend & transactions", L.leftX, y);
  const monthPanel = drawAssetPdfPanel(doc, L.leftX, y, L.width, 64, "Monthly revenue & expenses");
  embedAssetPdfImage(doc, monthlyLines, monthPanel.contentX, monthPanel.contentY, monthPanel.contentW, monthPanel.contentH - 2, { fit: "contain" });
  y += 70;
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ["Total invested", formatPdfAmount(sum.totalExpenses || 0, cur)],
      ["Total revenue", formatPdfAmount(sum.revenue || 0, cur)],
      ["Sale proceeds", formatPdfAmount(sum.salePrice || 0, cur)],
      ["Total income", formatPdfAmount(sum.totalIncome || 0, cur)],
  ...
  const txStart = (doc.lastAutoTable?.finalY || y) + 8;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(...ASSET_PDF_THEME.ink);
  doc.text("Transaction history", L.leftX, txStart);
  doc.autoTable({
    startY: txStart + 3,
