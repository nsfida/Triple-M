/* Triplem VIP unified Account Security — verified recovery, trusted 2FA browsers, biometric quick sign-in, Smart PIN integration and session history and bulk trusted-browser controls (v148). */
(() => {
  "use strict";
  ...
  async function isCurrentBrowserTrustedForSmartPin() {
    const candidates=[];
    const two=readTrusted2fa(user()?.username);
    const recovery=recoveryLocal(user()?.id);
    if(two?.secret)candidates.push(two.secret);
    if(recovery?.secret && !candidates.includes(recovery.secret))candidates.push(recovery.secret);
    if(!candidates.length)return false;
    for(const secret of candidates){
      try{
        const result=await rpc("app_account_security_verify_trusted_browser",{p_device_secret:secret});
        if(result?.ok!==false && result?.trusted===true)return true;
      }catch(err){
        const msg=safe(err?.message||err);
        if(/Could not find the function|PGRST202|404/i.test(msg))return false;
      }
    }
    return false;
  }
  ...
  async function loadSecurityData(){const local2=readTrusted2fa();const recLocal=recoveryLocal();const recoverySecret=recLocal?.secret||local2?.secret||null;const [status,sessions,twoDevices,recovery,recoveryDevices]=await Promise.all([rpc("app_account_security_status",{p_two_factor_device_secret:local2?.secret||null}),rpc("app_account_security_sessions",{}),rpc("app_two_factor_trusted_devices",{p_device_secret:local2?.secret||null}),rpc("app_account_recovery_status",{p_device_secret:recoverySecret}),rpc("app_account_recovery_trusted_devices",{p_device_secret:recoverySecret})]);return{status,sessions,twoDevices,recovery,recoveryDevices};}
  ...
  function renderSecurityCenter(modal,data,options={}){
    const s=data.status||{}, r=data.recovery||{}, active=Array.isArray(data.sessions?.active)?data.sessions.active:[], history=Array.isArray(data.sessions?.history)?data.sessions.history:[], trusted=Array.isArray(data.twoDevices?.devices)?data.twoDevices.devices:[], recoveryTrusted=Array.isArray(data.recoveryDevices?.devices)?data.recoveryDevices.devices:[];
    ...
    const trustedCount=Math.max(trusted.length,recoveryTrusted.length,Number(r.trusted_device_count||0));
    const hasOtherTrustedBrowsers=trusted.some(d=>!d.current)||recoveryTrusted.some(d=>!d.current);
    const canBulkRemoveTrustedBrowsers=hasOtherTrustedBrowsers;
    ...
    body.innerHTML=`
      ...
      <section class="account-security-card account-security-card-wide"><div class="account-security-section-title"><span><i class="fa-solid fa-shield-heart"></i></span><div><strong>Trusted browsers</strong><small>A trusted browser can skip Authenticator and Smart PIN for 30 days and approve recovery requests.</small></div><span class="account-security-count">${trustedCount}</span></div><div class="account-security-inline-actions account-security-trusted-actions">${currentFullyTrusted?`<span class="account-security-trusted-current"><i class="fa-solid fa-circle-check"></i> This browser is trusted</span>`:`<button class="btn primary" id="securityTrustCurrent" type="button"><i class="fa-solid fa-plus"></i> Trust this browser</button>`}<button class="btn ghost tiny account-security-danger-action" id="securityRemoveOtherTrustedBrowsers" type="button" ${canBulkRemoveTrustedBrowsers?"":"disabled"} title="${canBulkRemoveTrustedBrowsers?"Keep this signed-in browser trusted and remove trust from every other browser.":"No other trusted browsers are currently recorded."}"><i class="fa-solid fa-shield-xmark"></i> Remove all other trusted browsers</button><button class="btn ghost tiny" id="securityRefreshTrusted" type="button"><i class="fa-solid fa-rotate"></i> Refresh</button></div><div class="account-security-device-list">${trusted.length?trusted.map(d=>`<div class="account-security-device"><span class="device-icon"><i class="fa-solid fa-laptop"></i></span><div><strong>${esc(d.label||describeUa(d.user_agent))}${d.current?` <em>Current</em>`:""}</strong><small>${esc(describeUa(d.user_agent))} · Expires ${esc(formatDate(d.expires_at))}</small></div><button class="btn ghost tiny" data-revoke-2fa-device="${esc(d.id)}" type="button">Remove</button></div>`).join(""):`<div class="account-security-empty compact"><small>${currentRecoveryTrusted?"This browser is trusted for recovery approval.":"No browser is currently trusted."}</small></div>`}</div></section>
      <section class="account-security-card account-security-card-wide"><div class="account-security-section-title"><span><i class="fa-solid fa-desktop"></i></span><div><strong>Logged-in devices</strong><small>Review active sessions and remotely revoke every other device while keeping this one signed in.</small></div><span class="account-security-count">${active.length}</span></div><div class="account-security-inline-actions account-security-session-actions">${active.filter(d=>!d.current).length?`<button class="btn ghost tiny account-security-danger-action" id="securitySignOutOthers" type="button"><i class="fa-solid fa-right-from-bracket"></i> Sign out all other devices</button>`:`<span class="account-security-trusted-current"><i class="fa-solid fa-circle-check"></i> No other active devices</span>`}</div><div class="account-security-device-list">${active.map(d=>`<div class="account-security-device"><span class="device-icon"><i class="fa-solid fa-display"></i></span><div><strong>${esc(describeUa(d.user_agent))}${d.current?` <em>Current</em>`:""}</strong><small>${esc(authMethodLabel(d.auth_method))} · Last active ${esc(formatDate(d.last_seen_at))}</small></div>${d.current?`<span class="account-security-state is-on">This device</span>`:`<button class="btn ghost tiny" data-revoke-session="${esc(d.id)}" type="button">Sign out</button>`}</div>`).join("")}</div><details class="account-security-history"><summary><span>Login history</span><span>${history.length}</span></summary><div class="account-security-history-toolbar"><small>Only the latest 15 days are retained. Older records are permanently purged automatically.</small><button class="btn ghost tiny account-security-danger-action" id="securityDeleteLoginHistory" type="button"><i class="fa-solid fa-trash-can"></i> Delete history</button></div><div class="account-security-history-list">${history.length?history.map(h=>`<div><span class="history-dot ${h.status}"></span><p><strong>${esc(describeUa(h.user_agent))}</strong><small>${esc(authMethodLabel(h.auth_method))} · ${esc(formatDate(h.created_at))}</small></p><em>${esc(h.status)}</em></div>`).join(""):`<div class="account-security-history-empty"><i class="fa-solid fa-clock-rotate-left"></i><span>No login history is stored.</span></div>`}</div></details></section>
  ...
  function bindSecurityActions(modal,data,options){const s=data.status||{};
    ...
    modal.querySelector("#securityTrustCurrent")?.addEventListener("click",async e=>{const b=e.currentTarget;try{const pw=options.currentPassword||await promptCurrentPassword("Trust this browser","This browser will be allowed to approve recovery requests and skip Smart PIN. If 2FA is enabled, it can also skip Authenticator for 30 days.");if(!pw)return;b.disabled=true;const existingRecovery=recoveryLocal(user()?.id),existingTwoFactor=readTrusted2fa(user()?.username);const secret=existingRecovery?.secret||existingTwoFactor?.secret||randomSecret(),label=existingRecovery?.label||existingTwoFactor?.label||deviceLabel();let trusted2fa=null;if(s.two_factor_enabled){trusted2fa=await rpc("app_two_factor_trust_device",{p_password:pw,p_device_secret:secret,p_label:label,p_user_agent:navigator.userAgent||"",p_days:30});writeTrusted2fa(user()?.username,secret,label,trusted2fa?.expires_at||"");}await rpc("app_account_recovery_trust_device",{p_password:pw,p_device_secret:secret,p_label:label,p_user_agent:navigator.userAgent||""});writeRecoveryLocal(secret,label,user()?.id);if(options.currentPassword)options.currentPassword=pw;startAccountSecurityBackground();await refreshSecurityCenter(modal,options);}catch(err){setError(modal,"accountSecurityCenterError",err?.message||"Could not trust this browser.");}finally{if(b?.isConnected)b.disabled=false;}});
    modal.querySelector("#securityRemoveOtherTrustedBrowsers")?.addEventListener("click",async e=>{const b=e.currentTarget;if(b.disabled)return;try{setError(modal,"accountSecurityCenterError","");const pw=await promptCurrentPassword("Remove all other trusted browsers","This signed-in browser will be securely kept as trusted. Every other trusted browser will lose Authenticator, Smart PIN and recovery trust immediately.");if(!pw)return;const localTwo=readTrusted2fa(user()?.username),localRecovery=recoveryLocal(user()?.id),secret=localRecovery?.secret||localTwo?.secret||randomSecret(),label=localRecovery?.label||localTwo?.label||deviceLabel();b.disabled=true;const result=await rpc("app_account_security_revoke_other_trusted_browsers_v2",{p_password:pw,p_device_secret:secret,p_label:label,p_user_agent:navigator.userAgent||"",p_days:30});writeRecoveryLocal(secret,label,user()?.id);if(s.two_factor_enabled)writeTrusted2fa(user()?.username,secret,label,result?.expires_at||"");else clearTrusted2fa(user()?.username);await refreshSecurityCenter(modal,options);}catch(err){const raw=safe(err?.message);const friendly=/schema cache|could not find the function/i.test(raw)?"The latest Account Security database update is not installed yet. Run migration 148, then reopen Account Security.":(raw||"Could not remove the other trusted browsers.");setError(modal,"accountSecurityCenterError",friendly);}finally{if(b?.isConnected)b.disabled=!hasOtherTrustedBrowsers;}});
    modal.querySelector("#securityRefreshTrusted")?.addEventListener("click",()=>refreshSecurityCenter(modal,options));
    modal.querySelectorAll("[data-revoke-2fa-device]").forEach(btn=>btn.onclick=async()=>{try{setError(modal,"accountSecurityCenterError","");const pw=await promptCurrentPassword("Remove trusted browser");if(!pw)return;const id=safe(btn.dataset.revoke2faDevice).trim();if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id))throw new Error("Trusted browser record is unavailable. Refresh Account Security and try again.");const wasCurrent=Array.isArray(data.twoDevices?.devices)&&data.twoDevices.devices.some(d=>safe(d.id)===id&&d.current);await rpc("app_account_security_revoke_trusted_browser_v2",{p_password:pw,p_device_id:id});if(wasCurrent)clearTrusted2fa(user()?.username);await refreshSecurityCenter(modal,options);}catch(err){const raw=safe(err?.message);const friendly=/schema cache|could not find the function/i.test(raw)?"The latest Account Security database update is not installed yet. Run migration 148, then reopen Account Security.":(raw||"Could not remove trusted browser.");setError(modal,"accountSecurityCenterError",friendly);}});
    ...
    modal.querySelector("#securitySignOutOthers")?.addEventListener("click",async e=>{const b=e.currentTarget;try{const pw=await promptCurrentPassword("Sign out all other devices","Every other active Triplem VIP session will be revoked immediately. This current browser will remain signed in.");if(!pw)return;b.disabled=true;await rpc("app_account_security_revoke_other_sessions",{p_password:pw});await refreshSecurityCenter(modal,options);}catch(err){setError(modal,"accountSecurityCenterError",err?.message||"Could not sign out the other devices.");}finally{if(b?.isConnected)b.disabled=false;}});
    modal.querySelector("#securityDeleteLoginHistory")?.addEventListener("click",async e=>{const b=e.currentTarget;try{const pw=await promptCurrentPassword("Delete login history","This permanently deletes your stored login-history records. Active sessions are not deleted or signed out.");if(!pw)return;b.disabled=true;await rpc("app_account_security_delete_login_history",{p_password:pw});await refreshSecurityCenter(modal,options);}catch(err){setError(modal,"accountSecurityCenterError",err?.message||"Could not delete login history.");}finally{if(b?.isConnected)b.disabled=false;}});
  }
  ...
  window.isCurrentBrowserTrustedForSmartPin=isCurrentBrowserTrustedForSmartPin;
