...
(() => {
  ...
  function renderPasswordRecoveryIdentity(modal) {
    const preset = safe(document.getElementById("zipUsernameInput")?.value).trim();
    modal.innerHTML = `
  ...
  function renderPasswordRecoveryVerify(modal, challengeToken, username) {
    ...
    const close = () => closeModalElement(modal);
    modal.querySelectorAll("[data-password-recovery-close]").forEach(el => el.onclick = close);
    modal.querySelector("#passwordRecoveryBack").onclick = () => renderPasswordRecoveryIdentity(modal);
  ...
  function openPasswordRecovery() {
    ...
    renderPasswordRecoveryIdentity(modal);
    modal.classList.remove("hide");
    modal.setAttribute("aria-hidden", "false");
  }
