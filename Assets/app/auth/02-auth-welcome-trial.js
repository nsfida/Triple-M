...
async function completeAuthenticatedUnlock(user, sessionToken, { remember = false, silentResume = false, biometricVerified = false } = {}){
  ...
  if (!state.trialLocked) {
    ...
    if (state.secretPinHash && !state.secretPinVerified) {
      ...
      // If it is unavailable, cancelled, or fails, the existing Smart PIN gate
      // remains the authoritative fallback. Accounts that never opted in keep
      // the exact Build 137 flow without an additional prompt.
