    ...
    <div class="modal-dialog trial-signup-dialog" role="dialog" aria-modal="true" aria-labelledby="trialSignupTitle">
      ...
      <div class="modal-body trial-signup-body">
        ...
        <div class="form-group">
          ...
          <p class="help">Select at least one currency. Choosing <strong>BTC</strong> also enables the Bitcoin tab; without BTC, the Bitcoin tab stays off.</p>
        </div>
        <div class="form-group">
...
async function completeAuthenticatedUnlock(user, sessionToken, { remember = false, silentResume = false } = {}){
  ...
  if (!state.trialLocked) {
    ...
    if (silentResume && user && (
      ...
      state.secretPinPreferenceId = null;
      // Sentinel only — never rely on a client-stored hash for verification after migration 085.
