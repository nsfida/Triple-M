...
const adminSecurityState = {
  ...
  configured: false,
  unlocked: false,
  /** True only after a successful unlock on this page load — refresh always re-locks. */
  unlockedThisPage: false,
  checking: false,
  bound: false,
  pageBootLocked: false
};
...
async function lockAdminSecuritySession({ silent = true } = {}){
  ...
  try {
    ...
    adminSecurityState.unlockedThisPage = false;
  }
}

/**
 * On every fresh page load / session resume: revoke any prior Admin unlock so
 * refresh always requires the 15-digit key again. Never stores the key on device.
 */
async function prepareAdminSecurityForPageLoad(){
  if (typeof isProtectedAdminSession === "function" && !isProtectedAdminSession()) {
    adminSecurityState.applies = false;
    adminSecurityState.unlocked = true;
    adminSecurityState.unlockedThisPage = true;
    return;
  }
  adminSecurityState.unlocked = false;
  adminSecurityState.unlockedThisPage = false;
  try {
    ...
    if (status?.unavailable || !status?.configured) return;
    await lockAdminSecuritySession({ silent: true });
    adminSecurityState.pageBootLocked = true;
  } catch (_) {
    adminSecurityState.unlocked = false;
    adminSecurityState.unlockedThisPage = false;
  }
...
async function unlockAdminSecurityWithKey(rawKey){
  ...
  // Never persist the raw key — only send to server for verify; drop local copy.
  const result = await supabaseRpc("app_admin_security_key_unlock", { p_key: key });
  // ok:false commits fail counters (must not raise/rollback on the server).
  if (result && result.ok === false) {
    throw new Error(String(result.message || "Invalid security key.").trim());
  }
...
async function saveAdminSecurityKeyFromForm(root = document){
  ...
  if (result && result.ok === false) {
    throw new Error(String(result.message || "Could not save security key.").trim());
  }
  adminSecurityState.configured = true;
  adminSecurityState.unlocked = true;
  adminSecurityState.unlockedThisPage = true;
 ...
 * Unlock is page-memory only — a refresh always shows the lock again.
 */
async function ensureAdminSecurityAccess(){
  ...
  try {
    if (!adminSecurityState.pageBootLocked) {
    ...
    // Page-load unlock only — ignore durable server unlock left from before refresh.
    if (adminSecurityState.unlockedThisPage) {
      adminSecurityState.unlocked = true;
      setAdminSecurityLockVisible(false);
      return true;
    }
...
function bindAdminSecurityUi(){
  ...
  // bfcache / soft refresh: re-lock Admin so prior unlock cannot linger.
  window.addEventListener("pageshow", (event) => {
    if (!event.persisted) return;
    adminSecurityState.unlockedThisPage = false;
    adminSecurityState.pageBootLocked = false;
    if (typeof isProtectedAdminSession === "function" && isProtectedAdminSession()) {
      lockAdminSecuritySession({ silent: true }).catch(() => {});
      const adminPanel = document.getElementById("admin");
      if (adminPanel && !adminPanel.classList.contains("hide")) {
        setAdminSecurityLockVisible(true);
      }
    }
  });
