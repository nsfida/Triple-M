...
function renderAdminUserCardHtml(user, { nested = false, memberCount = 0, activeMemberCount = null, inTeamOverlay = false } = {}){
  ...
  const seatEntitlement = Math.max(0, Number(user.team_seat_entitlement ?? user.max_team_members) || 0);
  const isCompanyAdministrator = !user.team_owner_id && String(user.account_title || "").toLowerCase() === "company administrator";
  const teamAccountBadge = (user.allow_team_members || isCompanyAdministrator || seatEntitlement > 0)
    ? `<span class="admin-badge ok">${isCompanyAdministrator ? "Company Administrator" : "Team"}${seatEntitlement ? ` · ${escapeHtml(String(seatEntitlement))} seats` : ""}</span>`
    : "";
  const teamMemberBadge = (nested || inTeamOverlay || user.team_owner_id)
    ? `<span class="admin-badge">${escapeHtml(user.team_title || user.account_title || "Team Member")}</span>`
    : "";
  ...
  const triplemAiBadge = user.triplem_ai_access ? `<span class="admin-badge triplem-ai-badge"><i class="fa-solid fa-wand-magic-sparkles" aria-hidden="true"></i> AI</span>` : "";
  ...
  const clearPinBtn = (pinEnabled && !user.is_protected && isProtectedAdminSession())
    ...
    : "";
  const showTeamBtn = !isTeamMember && (memberCount > 0 || !!user.allow_team_members || isCompanyAdministrator || seatEntitlement > 0);
  const teamBtn = "";
  ...
  const resolvedActiveMemberCount = activeMemberCount == null ? Number(user.team_active_count ?? memberCount) : Number(activeMemberCount);
  const memberHint = !isTeamMember && (memberCount > 0 || Number(user.team_total_count) > 0)
    ? `<span class="admin-user-team-count">${escapeHtml(String(Math.max(0, resolvedActiveMemberCount || 0)))} active · ${escapeHtml(String(Math.max(memberCount, Number(user.team_total_count) || 0)))} total</span>`
    : "";
      ...
      <div class="admin-user-summary" data-admin-toggle-card role="button" tabindex="0" aria-expanded="false">
        <div class="admin-user-summary-main">
          ...
          <div class="admin-user-summary-badges">
            ${statusBadge}${roleBadge}${planBadge}${paymentBadge}${pinBadge}${protectedBadge}${teamAccountBadge}${teamMemberBadge}${triplemAiBadge}${forceBadge}
          </div>
        </div>
...
function renderAdminUserTileHtml(user, memberCount = 0, activeMemberCount = null){
  const name = user.display_name || user.username || "User";
  const inactive = user.is_active === false;
  const active = activeMemberCount == null ? Number(user.team_active_count ?? 0) : Number(activeMemberCount);
  const total = Math.max(memberCount, Number(user.team_total_count) || 0);
  const teamTitle = total > 0 ? `${active} active / ${total} team members` : "";
  return `<button type="button" class="admin-user-tile${inactive ? " is-inactive" : ""}" data-admin-user-tile="${escapeHtml(user.id)}" title="Open ${escapeHtml(name)}${teamTitle ? ` · ${escapeHtml(teamTitle)}` : ""}">
    ${total > 0 ? `<span class="admin-user-tile-count" title="${escapeHtml(teamTitle)}">${escapeHtml(String(active))}</span>` : ""}
    <span class="admin-user-tile-avatar">${adminUserLogoImage(user, "admin-user-tile-logo")}</span>
...
function openAdminUserDetailsOverlay(user){
  ...
  modal.innerHTML = `<div class="modal-backdrop" data-admin-user-details-close></div>
    <div class="modal-dialog admin-user-details-dialog" role="dialog" aria-modal="true" aria-labelledby="adminUserDetailsTitle">
      ...
      <div class="modal-body admin-user-details-body">${renderAdminUserCardHtml(user,{nested:false,memberCount:members.length,activeMemberCount:members.filter(member => member.is_active !== false).length})}</div>
    </div>`;
...
function adminUserMatchesSearch(user, term){
  ...
  if (!q) return true;
  return [user?.username, user?.display_name, user?.team_title, user?.account_title, user?.company_name, user?.settings?.Company].some(value => String(value || "").toLowerCase().includes(q));
}
...
function renderAdminUsersGrid(){
  ...
  list.innerHTML = visible.map(user => {
    ...
    return renderAdminUserTileHtml(user, members.length, members.filter(member => member.is_active !== false).length);
  }).join("");
...
function openAdminTeamOverlay(owner, opts = {}){
  ...
  const seats = Math.max(0, Number(owner.team_seat_entitlement ?? owner.max_team_members) || 0);
  modal.innerHTML = `
    ...
    <div class="modal-dialog settings-sheet admin-team-overlay-sheet" role="dialog" aria-modal="true" aria-labelledby="adminTeamOverlayTitle">
      <div class="settings-sheet-head">
        <div>
          ...
          <p>${escapeHtml(companyName)} · @${escapeHtml(owner.username)} · Company Administrator · ${activeMembers} active · ${members.length} total · ${seats} paid seats</p>
        </div>
...
function adminReceiptArchiveRowHtml(row){
  ...
  const context = row.request_context === "team_seat_addition" ? "Team seats" : (row.request_context === "renewal" ? "Extension" : "Signup");
...
function openAdminSubscriptionReviewModal(request){
  ...
  const isTeamSeatAddition = request.request_context === "team_seat_addition";
  const plan = isTeamSeatAddition ? "Team seat addition" : (request.billing_period === "yearly" ? "Pro Yearly" : "Pro Monthly");
  const teamText = isTeamSeatAddition ? `+${request.team_seats} paid team seat${Number(request.team_seats)===1?"":"s"}` : (request.team_enabled ? `${request.team_seats} paid team seat${Number(request.team_seats)===1?"":"s"}` : "No paid team seats");
  const requestContext = isTeamSeatAddition ? "Additional paid team capacity" : (request.request_context === "renewal" ? "Plan extension" : "New Pro signup");
  const requestPending = String(request.status || "pending") === "pending";
  const receiptExists = request.receipt_exists !== false;
  const proposedExpiry = isTeamSeatAddition
    ? (request.proposed_expires_at ? `Seat access through ${formatTrialExpiry(request.proposed_expires_at)}` : "Company plan expiry unchanged")
    : (request.proposed_expires_at ? formatTrialExpiry(request.proposed_expires_at) : "Calculated on approval");
    ...
    <div class="modal-dialog settings-sheet admin-subscription-review-sheet" role="dialog" aria-modal="true" aria-labelledby="subReviewTitle">
      ...
      <div class="modal-body settings-sheet-body admin-subscription-review">
        ...
        <div class="admin-subscription-actions">
          ${isTeamSeatAddition ? `
            <button class="btn primary tiny" data-sub-action="approve"><i class="fa-solid fa-user-plus"></i> Approve team seats</button>
...
async function openPlanSubscriptionModal(){
  ...
  const pendingSeatAddition = sub?.request_context === "team_seat_addition";
  const pendingPlan = pendingSeatAddition ? "Team seat addition" : (sub?.plan === "yearly" ? "Pro Yearly" : "Pro Monthly");
  const pendingExpiry = pendingSeatAddition ? currentExpiry : (sub?.proposed_expires_at ? formatTrialExpiry(sub.proposed_expires_at) : currentExpiry);
  ...
  body.innerHTML = `
    ...
    <div class="plan-sub-managed plan-sub-billing-contact"><i class="fa-solid fa-envelope"></i><div><strong>Billing assistance</strong><p>For subscription, receipt or payment questions, email <a href="mailto:billing@triplem.vip">billing@triplem.vip</a>.</p></div></div>
...
function buildAdminUserFormFields(prefix, user = null){
  ...
  return `
    ...
    <div class="admin-accounting-access-block" data-admin-accounting-access>
      <div class="admin-accounting-access-copy">
        <span class="admin-accounting-access-icon"><i class="fa-solid fa-calculator" aria-hidden="true"></i></span>
        <div><strong>Accounting feature</strong><small>Show the full Accounting workspace for this user. When off, the Accounting tab is hidden and its database permissions are revoked.</small></div>
      </div>
      <label class="admin-accounting-switch">
        <input id="${prefix}AccountingAccess" type="checkbox" ${tabs.includes("accounting") ? "checked" : ""} ${user?.is_protected ? "disabled" : ""} />
        <span aria-hidden="true"></span><em>${tabs.includes("accounting") ? "On" : "Off"}</em>
      </label>
    </div>
    <div class="admin-accounting-access-block" data-admin-insurance-access>
      <div class="admin-accounting-access-copy">
        <span class="admin-accounting-access-icon"><i class="fa-solid fa-shield-heart" aria-hidden="true"></i></span>
        <div><strong>Insurance feature</strong><small>Main Admin controlled. The protected Main Admin always has access; enable or disable Insurance for other accounts here. Team members can never exceed their company owner’s Insurance access.</small></div>
      </div>
      <label class="admin-accounting-switch">
        <input id="${prefix}InsuranceAccess" type="checkbox" ${(user?.is_protected || tabs.includes("insurance")) ? "checked" : ""} ${(user?.is_protected || !isProtectedAdminSession()) ? "disabled" : ""} />
        <span aria-hidden="true"></span><em>${(user?.is_protected || tabs.includes("insurance")) ? "On" : "Off"}</em>
      </label>
    </div>
    ${user && isProtectedAdminSession() ? `
    <div class="admin-triplem-ai-access-block" data-admin-triplem-ai-access>
      <div class="admin-triplem-ai-access-copy">
        <span class="admin-triplem-ai-access-icon"><i class="fa-solid fa-wand-magic-sparkles" aria-hidden="true"></i></span>
        <div><strong>Triplem AI access</strong><small>Main Admin controlled. When enabled, this user may connect their own Gemini API key and use the private read-only financial assistant. Revoking access also destroys their stored Gemini credential.</small></div>
      </div>
      <label class="admin-triplem-ai-switch">
        <input id="${prefix}TriplemAiAccess" type="checkbox" ${user?.triplem_ai_access ? "checked" : ""} />
        <span aria-hidden="true"></span><em>${user?.triplem_ai_access ? "On" : "Off"}</em>
      </label>
    </div>` : ""}
    <div class="form-group" style="margin-top:12px">
      <label class="form-label">Allowed tabs / modules</label>
      <p class="help">Only checked tabs will appear for this user after login. Accounting is controlled separately above; Bitcoin follows its own access setting.</p>
      ${checkboxGridHtml(`${prefix}Tabs`, ADMIN_TAB_OPTIONS.filter(opt => opt.id !== "accounting").filter(opt => opt.id !== "insurance"), tabs, { disabledIds: disabledTabs })}
    </div>
    <label class="admin-inline-check">
...
async function callPhotoMaintenance(action, payload = {}){
  const cfg = getSupabaseConfig();
  if (!cfg?.supabaseUrl) throw new Error("Supabase configuration is unavailable.");
  const token = String(state.sessionToken || "").trim();
  if (!token) throw new Error("Authentication required.");
  const response = await fetch(`${cfg.supabaseUrl}/functions/v1/photo-maintenance`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || data?.error) throw new Error(data?.error || `Photo maintenance failed (${response.status}).`);
  return data;
}
...
function bindAdminAccountingAccessControl(root, prefix){
  if (!root) return;
  const input = root.querySelector(`#${prefix}AccountingAccess`);
  if (!input) return;
  const label = input.closest(".admin-accounting-switch")?.querySelector("em");
  const sync = () => { if (label) label.textContent = input.checked ? "On" : "Off"; };
  input.addEventListener("change", sync);
  sync();
}

function bindAdminInsuranceAccessControl(root, prefix){
  if (!root) return;
  const input = root.querySelector(`#${prefix}InsuranceAccess`);
  if (!input) return;
  const label = input.closest(".admin-accounting-switch")?.querySelector("em");
  const sync = () => { if (label) label.textContent = input.checked ? "On" : "Off"; };
  input.addEventListener("change", sync);
...
function bindAdminTriplemAiAccessControl(root, prefix){
  if (!root) return;
  const input = root.querySelector(`#${prefix}TriplemAiAccess`);
  if (!input) return;
  const label = input.closest(".admin-triplem-ai-switch")?.querySelector("em");
  const sync = () => { if (label) label.textContent = input.checked ? "On" : "Off"; };
  input.addEventListener("change", sync);
  sync();
}

function applyAdminAccountingAccessToTabs(root, prefix, tabs){
  const next = (Array.isArray(tabs) ? tabs : []).filter(tab => String(tab).toLowerCase() !== "accounting");
  if (root?.querySelector(`#${prefix}AccountingAccess`)?.checked) next.push("accounting");
  return Array.from(new Set(next));
}

function applyAdminInsuranceAccessToTabs(root, prefix, tabs){
  const next = (Array.isArray(tabs) ? tabs : []).filter(tab => String(tab).toLowerCase() !== "insurance");
  const input = root?.querySelector(`#${prefix}InsuranceAccess`);
  if (input?.checked) next.push("insurance");
  return Array.from(new Set(next));
}
...
function openAdminCreateUserModal(){
  ...
  modal.innerHTML = `
    ...
    <div class="modal-dialog admin-modal-dialog settings-sheet admin-settings-sheet">
      ...
      <div class="modal-body settings-sheet-body admin-user-access-scroll">
        <div id="adminCreateFormBody"></div>
        <div id="adminCreateError" class="lock-error"></div>
      </div>
      <div class="modal-footer settings-sheet-footer admin-user-access-footer">
  ...
  bindAdminAccountingAccessControl(body, "adminNew");
  bindAdminInsuranceAccessControl(body, "adminNew");
  ...
  modal.querySelector("#adminCreateSave").onclick = async () => {
    ...
    try {
      ...
      const tabs = applyAdminInsuranceAccessToTabs(body, "adminNew", applyAdminAccountingAccessToTabs(body, "adminNew", accessBits.tabs));
...
function openAdminEditUserModal(user){
  ...
  bindAdminAccountingAccessControl(body, "adminEdit");
  bindAdminInsuranceAccessControl(body, "adminEdit");
  bindAdminTriplemAiAccessControl(body, "adminEdit");
  ...
  (async () => {
    try {
      ...
      const insuranceAccessEl = modal.querySelector("#adminEditInsuranceAccess");
      if (insuranceAccessEl) {
        const freshInsuranceEnabled = fresh.is_protected === true
          || (typeof userAllowedTabs === "function" ? userAllowedTabs(fresh) : (fresh.allowed_tabs || fresh.settings?.Tabs || [])).includes("insurance");
        insuranceAccessEl.checked = freshInsuranceEnabled;
        insuranceAccessEl.disabled = fresh.is_protected === true || !isProtectedAdminSession();
        insuranceAccessEl.dispatchEvent(new Event("change"));
      }
      const aiAccessEl = modal.querySelector("#adminEditTriplemAiAccess");
      if (aiAccessEl) {
        aiAccessEl.checked = fresh.triplem_ai_access === true;
        aiAccessEl.dispatchEvent(new Event("change"));
      }
  ...
  modal.querySelector("#adminEditSave").onclick = async () => {
    ...
    try {
      ...
      const currencies = accessBits.currencies;
      const tabs = applyAdminInsuranceAccessToTabs(body, "adminEdit", applyAdminAccountingAccessToTabs(body, "adminEdit", accessBits.tabs));
      if (!currencies.length) throw new Error("Select at least one currency.");
      if (!tabs.length) throw new Error("Select at least one tab.");
      const payload = {
        p_user_id: user.id,
      ...
      const aiAccessEl = modal.querySelector("#adminEditTriplemAiAccess");
      if (aiAccessEl && isProtectedAdminSession()) {
        const nextAiAccess = aiAccessEl.checked === true;
        if (nextAiAccess !== (user.triplem_ai_access === true)) {
          await supabaseRpc("app_admin_set_triplem_ai_access", {
            p_user_id: user.id,
            p_allowed: nextAiAccess
          });
        }
      }
      ...
      if (state.sessionUser?.id === user.id) {
        try {
          ...
          if (validated?.user) {
            ...
            updateGuestModeUi();
            try { await window.refreshTriplemAiAccessState?.({ force: true }); } catch (_) {}
...
function openAccountCompanyEditModal(parentModal){
    ...
    <div class="modal-dialog settings-sheet account-settings-editor account-company-editor" role="dialog" aria-modal="true" aria-labelledby="accountCompanyEditTitle">
      ...
      <div class="modal-body settings-sheet-body account-settings-editor-body">
        <div class="settings-grid">
          ...
          <label class="settings-field settings-span-2">Company logo
            <div class="settings-logo-row account-logo-maintenance-row">
              <input id="acctCompanyEditLogoFile" class="input settings-input" type="file" accept="image/png,image/jpeg,image/jpg,image/webp,image/gif" />
              <input id="acctCompanyEditLogoUrl" type="hidden" value="${escapeHtml(snap.logo)}" />
              <img id="acctCompanyEditLogoPreview" class="settings-logo-preview ${snap.logo ? "" : "hide"}" src="${escapeHtml(snap.logo)}" alt="Company logo preview" />
              ${snap.logo ? `<button id="acctCompanyEditLogoDelete" type="button" class="btn danger tiny"><i class="fa-solid fa-trash"></i> Delete logo</button>` : ""}
            </div>
            <span id="acctCompanyEditLogoStatus" class="settings-hint">${snap.logo ? "Current logo loaded · deleting restores the Triplem VIP default logo" : "Using Triplem VIP default logo · PNG/JPG/WebP"}</span>
          </label>
        </div>
  ...
  const deleteLogoBtn = modal.querySelector("#acctCompanyEditLogoDelete");
  if (deleteLogoBtn) deleteLogoBtn.onclick = async () => {
    const ok = await requireSmartPinConfirm({
      title: "Delete company logo",
      description: "Remove the current company logo from your account and storage? Triplem VIP's default logo will be used immediately.",
      confirmLabel: "Delete Logo",
      accent: "danger"
    });
    if (!ok) return;
    const status = modal.querySelector("#acctCompanyEditLogoStatus");
    try {
      deleteLogoBtn.disabled = true;
      if (status) status.textContent = "Removing logo securely…";
      const result = await callPhotoMaintenance("clear-own-logo");
      if (result?.profile) await syncAccountSettingsSession(result.profile);
      const hidden = modal.querySelector("#acctCompanyEditLogoUrl");
      if (hidden) hidden.value = "";
      const preview = modal.querySelector("#acctCompanyEditLogoPreview");
      if (preview) { preview.src = "Assets/logo/logo.png"; preview.classList.remove("hide"); }
      deleteLogoBtn.remove();
      if (status) status.textContent = result?.storage_deleted === false
        ? "Logo removed from your account. An unreferenced storage copy can be cleaned by Admin."
        : "Logo deleted. Triplem VIP default logo is now active.";
      renderAccountSettingsSummary(parentModal);
    } catch (ex) {
      if (status) status.textContent = ex.message || "Could not delete logo.";
      alert(ex.message || "Could not delete logo.");
    } finally { if (deleteLogoBtn?.isConnected) deleteLogoBtn.disabled = false; }
  };
  const close = () => closeAccountSettingsChild(modal);
  modal.querySelectorAll("[data-account-child-close]").forEach(el => el.onclick = close);
  modal.classList.remove("hide"); modal.setAttribute("aria-hidden", "false");
...
function renderAccountSettingsSummary(modal){
  ...
  body.innerHTML = `
    ...
    ${typeof window.triplemAiAccountSettingsCardHtml === "function" ? window.triplemAiAccountSettingsCardHtml() : ""}`;
  ...
  body.querySelector("#accountTriplemAiBtn")?.addEventListener("click", () => {
    if (typeof window.openTriplemAiSettingsModal === "function") window.openTriplemAiSettingsModal();
  });
...
function openAccountSettingsModal(){
  ...
  modal.innerHTML = `
    ...
    <div class="modal-dialog settings-sheet account-settings-overview" role="dialog" aria-modal="true" aria-labelledby="accountSettingsTitle">
      <div class="settings-sheet-head"><div><h3 id="accountSettingsTitle">Account settings</h3><p>Your profile, company identity, security and private integrations in one place.</p></div><button type="button" class="btn ghost tiny" data-admin-modal-close="accountSettingsModal" aria-label="Close">✕</button></div>
  ...
  modal.classList.remove("hide"); modal.setAttribute("aria-hidden", "false");
  try {
    window.refreshTriplemAiAccessState?.().then(() => {
      if (!modal.classList.contains("hide")) renderAccountSettingsSummary(modal);
    }).catch(() => {});
  } catch (_) {}
}
...
function adminStorageChartTheme(){
  ...
  return {
    ...
    line: read("--chart-grid", read("--line", "")),
    lineStrong: read("--chart-tooltip-border", read("--line-strong", "")),
    surface: read("--chart-tooltip-bg", read("--surface-glass-strong", "")),
    tooltipText: read("--chart-tooltip-text", read("--text", "")),
    primary: read("--chart-series-1", read("--primary", "")),
    danger: read("--danger", ""),
    palette: Array.from({ length: 8 }, (_, index) => read(`--chart-series-${index + 1}`, read("--primary", "")))
  };
}
...
async function openAdminPhotoCompressModal(user, storageModal){
    ...
    <div class="modal-dialog admin-modal-dialog admin-photo-compress-dialog" role="dialog" aria-modal="true" aria-labelledby="adminPhotoCompressTitle">
      ...
      <div class="modal-body admin-photo-compress-body">
        <div class="admin-photo-compress-toolbar">
          ...
          <button type="button" class="btn ghost tiny" id="adminPhotoFindUnusedBtn"><i class="fa-solid fa-broom"></i> Find unused</button>
          <button type="button" class="btn danger tiny hide" id="adminPhotoDeleteUnusedBtn"><i class="fa-solid fa-trash"></i> Delete unused</button>
        ...
        <div class="admin-unused-photo-panel hide" id="adminUnusedPhotoPanel"></div>
      </div>
    </div>`;
  ...
  const renderUnusedPhotos = () => {
    const panel = modal.querySelector("#adminUnusedPhotoPanel");
    const delBtn = modal.querySelector("#adminPhotoDeleteUnusedBtn");
    const items = adminPhotoCompressState.unusedPhotos || [];
    if (!panel) return;
    panel.classList.toggle("hide", !items.length);
    const selectedCount = items.filter(p => adminPhotoCompressState.unusedSelected?.[String(p.path || "")]).length;
    if (delBtn) {
      delBtn.classList.toggle("hide", !items.length);
      delBtn.innerHTML = `<i class="fa-solid fa-trash" aria-hidden="true"></i> Delete selected${selectedCount ? ` (${selectedCount})` : ""}`;
      delBtn.disabled = !selectedCount;
    }
    panel.innerHTML = items.length ? `
      <div class="admin-unused-photo-head">
        <div><strong>Unreferenced storage photos</strong><span>Only unreferenced candidates are listed. Server-side reference checks run again before deletion.</span></div>
        <label class="admin-unused-select-all"><input type="checkbox" id="adminUnusedPhotoSelectAll" ${selectedCount === items.length ? "checked" : ""}><span>Select all</span></label>
      </div>
      <div class="admin-unused-photo-list">
        ${items.map(p => {
          const key = String(p.path || "");
          const selected = !!adminPhotoCompressState.unusedSelected?.[key];
          const shortName = key.split("/").filter(Boolean).pop() || p.label || "Unused photo";
          return `<label class="admin-unused-photo-row${selected ? " is-selected" : ""}">
            <input type="checkbox" data-unused-photo-path="${escapeHtml(key)}" ${selected ? "checked" : ""}>
            <span class="admin-unused-photo-copy"><strong>${escapeHtml(shortName)}</strong><small>${escapeHtml(p.source || "Unreferenced storage object")} · ${escapeHtml(formatStorageBytes(Number(p.bytes)||0))}</small><em title="${escapeHtml(key)}">${escapeHtml(key)}</em></span>
          </label>`;
        }).join("")}
      </div>
    ` : "";
    panel.querySelector("#adminUnusedPhotoSelectAll")?.addEventListener("change", e => {
      const on = !!e.currentTarget.checked;
      items.forEach(p => {
        const key = String(p.path || "");
        if (!key) return;
        if (on) adminPhotoCompressState.unusedSelected[key] = true;
        else delete adminPhotoCompressState.unusedSelected[key];
      });
      renderUnusedPhotos();
    });
    panel.querySelectorAll("[data-unused-photo-path]").forEach(input => {
      input.addEventListener("change", e => {
        const key = String(e.currentTarget.dataset.unusedPhotoPath || "");
        if (!key) return;
        if (e.currentTarget.checked) adminPhotoCompressState.unusedSelected[key] = true;
        else delete adminPhotoCompressState.unusedSelected[key];
        renderUnusedPhotos();
      });
    });
  };
  modal.querySelector("#adminPhotoFindUnusedBtn").onclick = async () => {
    try {
      updateAdminPhotoCompressHint(modal, "Checking storage references…");
      const data = await supabaseRpc("app_admin_list_unused_storage_photos", { p_user_id: userId });
      adminPhotoCompressState.unusedPhotos = Array.isArray(data?.photos) ? data.photos : [];
      adminPhotoCompressState.unusedSelected = {};
      renderUnusedPhotos();
      updateAdminPhotoCompressHint(modal, adminPhotoCompressState.unusedPhotos.length
        ? `${adminPhotoCompressState.unusedPhotos.length} unreferenced photo${adminPhotoCompressState.unusedPhotos.length === 1 ? "" : "s"} found.`
        : "No unused photos found. All storage photos are referenced.");
    } catch (err) { updateAdminPhotoCompressHint(modal, err.message || "Could not inspect unused photos."); }
  };
  modal.querySelector("#adminPhotoDeleteUnusedBtn").onclick = async () => {
    const items = adminPhotoCompressState.unusedPhotos || [];
    const selected = items.filter(p => adminPhotoCompressState.unusedSelected?.[String(p.path || "")]);
    if (!selected.length) { updateAdminPhotoCompressHint(modal, "Select one or more unused photos to delete."); return; }
    const ok = confirm(`Permanently delete ${selected.length} selected unreferenced photo${selected.length === 1 ? "" : "s"} from storage? Each path will be checked again for references before deletion.`);
    if (!ok) return;
    try {
      const result = await callPhotoMaintenance("delete-unused", { user_id: userId, paths: selected.map(p => p.path).filter(Boolean) });
      const selectedPaths = new Set(selected.map(p => String(p.path || "")));
      adminPhotoCompressState.unusedPhotos = items.filter(p => !selectedPaths.has(String(p.path || "")));
      adminPhotoCompressState.unusedSelected = {};
      renderUnusedPhotos();
      updateAdminPhotoCompressHint(modal, `Deleted ${Number(result?.deleted || 0)} selected unused storage photo${Number(result?.deleted || 0) === 1 ? "" : "s"}.`);
      ...
      renderAdminPhotoCompressList(modal);
    } catch (err) { alert(err.message || "Could not delete selected unused photos."); }
  };
