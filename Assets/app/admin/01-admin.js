...
function openAccountPasswordChangeModal(){
  ...
  modal.querySelector("#acctPasswordChangeSave").onclick = async e => {
    const btn = e.currentTarget;
...
function renderAccountSettingsSummary(modal){
  ...
  body.innerHTML = `
    ...
    <section class="settings-card account-security-card">
      <div class="settings-card-head"><span><i class="fa-solid fa-shield-halved"></i> Sign-in security</span></div>
      <button type="button" class="account-settings-action-row" id="accountPasswordChangeBtn"><span class="account-settings-action-icon"><i class="fa-solid fa-key"></i></span><span><strong>Change password</strong><small>Replace your sign-in password securely.</small></span><i class="fa-solid fa-chevron-right"></i></button>
      <button type="button" class="account-settings-action-row" id="accountTwoFactorBtn"><span class="account-settings-action-icon"><i class="fa-solid fa-mobile-screen-button"></i></span><span><strong>Authenticator App 2FA</strong><small id="accountTwoFactorSummaryText">Checking protection status…</small></span><span class="settings-pill two-factor-status-pill is-loading" id="accountTwoFactorSummaryPill">Checking…</span><i class="fa-solid fa-chevron-right"></i></button>
    </section>`;
  ...
  body.querySelector("#accountPasswordChangeBtn")?.addEventListener("click", () => openAccountPasswordChangeModal());
  body.querySelector("#accountTwoFactorBtn")?.addEventListener("click", () => {
    if (typeof window.openTwoFactorManagement === "function") window.openTwoFactorManagement();
  });
  try { window.refreshAccountTwoFactorSummary?.(modal); } catch (_) {}
}
...
function openAccountSettingsModal(){
  ...
  modal.innerHTML = `
    ...
    <div class="modal-dialog settings-sheet account-settings-overview" role="dialog" aria-modal="true" aria-labelledby="accountSettingsTitle">
      <div class="settings-sheet-head"><div><h3 id="accountSettingsTitle">Account settings</h3><p>Your profile, company identity and sign-in security in one place.</p></div><button type="button" class="btn ghost tiny" data-admin-modal-close="accountSettingsModal" aria-label="Close">✕</button></div>
...
(() => {
  ...
  async function openAdminLiveChatAgentsModal(){
    ...
    try {
      const data = await supabaseRpc("app_admin_live_chat_agent_settings", {});
    ...
    try {
      ...
      body.innerHTML = `
        <div class="admin-live-record-summary">
          ...
          <div><span>Status</span><strong>${escapeHtml(chat.routing_status || chat.status || "open")}</strong><small>${chat.closed_at ? `Closed ${escapeHtml(fmt(chat.closed_at))}` : "Conversation record retained"}</small></div>
        </div>
  ...
  async function openAdminLiveChatRecordsModal(){
    ...
    const load = async (reset = true) => {
      ...
      try {
        ...
        const rows = Array.isArray(data?.items) ? data.items : [];
        stateRecords.items = reset ? rows : stateRecords.items.concat(rows);
        stateRecords.total = Number(data?.total || stateRecords.items.length);
        stateRecords.hasMore = !!data?.has_more;
        stateRecords.offset = stateRecords.items.length;
        render();
