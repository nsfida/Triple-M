  ...
  return `
    <article class="admin-user-card${nestClass}" data-user-id="${escapeHtml(user.id)}"${nestAttr}>
      ...
      <div class="admin-user-details" hidden>
        ...
        ${companyLine}
        ${contactLine}
        ${isProtectedAdminSession() ? `<div class="admin-cred-lock-row">
          <button type="button" class="admin-cred-lock-btn" data-admin-action="view_credentials" title="View secured credentials" aria-label="View secured credentials for ${escapeHtml(user.username)}">
...
function renderAdminUserTileHtml(user, memberCount = 0){
  ...
  return `<button type="button" class="admin-user-tile${inactive ? " is-inactive" : ""}" data-admin-user-tile="${escapeHtml(user.id)}" title="Open ${escapeHtml(name)}">
    <span class="admin-user-tile-avatar"><i class="fa-solid ${user.is_protected ? "fa-user-shield" : "fa-user"}" aria-hidden="true"></i>${memberCount > 0 ? `<span class="admin-user-tile-count">${escapeHtml(String(memberCount))}</span>` : ""}</span>
    ...
    <div class="modal-dialog settings-sheet">
      <div class="settings-sheet-head">
        <div>
          ...
          <p>Profile, company details, and password</p>
        </div>
      ...
      if (!rows.length) {
        ...
        list.innerHTML = rows.map(u => `
          <label class="admin-live-agent-row">
            ...
            <span class="admin-live-agent-avatar"><i class="fa-solid fa-headset"></i></span>
