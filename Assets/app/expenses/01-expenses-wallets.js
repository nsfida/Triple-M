...
function persistExpenseWalletOrder(orderedGroupIds){
  ...
  ids.forEach((gid, index) => {
    ...
    changed = true;
    queueDatabasePatch(entry.id, { notes: nextNotes }, "Wallet order", entry);
  });
  if (!changed) return;
  invalidateExpenseAccountsSyncCache();
...
function ensureExpenseWalletReorder(host, options = {}){
  ...
  host.addEventListener("pointerdown", (e) => {
    ...
    if (e.target.closest("button, a, input, select, textarea, .expenseWalletQuick, .overview-card-actions")) return;
  ...
  host.addEventListener("contextmenu", (e) => {
    ...
    if (dragging || pressTimer) e.preventDefault();
  });
}
