...
function openExpenseTransactionDetail(txId){
  ...
  const aiOrigin = (typeof isTriplemAiOrigin === "function" && isTriplemAiOrigin("expenses", tx.id, tx.groupId || tx.group_id)) || state.entries?.some?.(entry => String(entry?.id || "") === String(tx.id) && /\[AI_CREATED\]/i.test(String(entry?.notes || "")));
...
function renderExpenseHistoryRangeControls(){
  return `<span class="expense-history-controls">
    <span class="expense-history-download-wrap">
      <button type="button" class="icon-btn ghost expenseActionBtn expense-history-download" data-action="pdf-menu" data-type="transactions-history" title="Download Transactions History PDF" aria-haspopup="true" aria-expanded="false"><i class="fa-solid fa-file-pdf"></i></button>
...
function renderExpensesList(){
  ...
  // Search is global; apply the local blob too so opening balances and loaded legacy rows obey it.
  ...
  if (topupTransactions.length > 0){
    ...
    html += `<details class="expense-collapsible-section" id="topupRecordsSection" data-expense-details-id="topupRecordsSection" open>
      ...
      <div class="expense-collapsible-content"><div class="empty">No top-up records found for the current filters.</div></div>
    </details>`;
  }
  }
  ...
  if (transferEvents.length > 0 && transferCurrencies.length > 0){
    ...
    html += `<details class="expense-collapsible-section" id="transferRecordsSection" data-expense-details-id="transferRecordsSection" open>
      ...
      <div class="expense-collapsible-content"><div class="empty">No transfer records found for the current filters.</div></div>
    </details>`;
  }
  }
