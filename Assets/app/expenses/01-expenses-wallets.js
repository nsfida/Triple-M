...
function closeExpenseTransactionMenus(){
  ...
  document.querySelectorAll(".expense-tx-menu-btn[aria-expanded=true]").forEach(btn => btn.setAttribute("aria-expanded","false"));
...
function openExpenseTransactionDetail(txId){
  ...
  modal.querySelector("#expenseTxDetailBody").innerHTML = `
    ...
    <div class="expense-tx-actions">
      ${teamCanShowEdit("entries") ? `<button class="btn ghost" type="button" data-expense-tx-edit="${escapeHtml(tx.id)}"><i class="fa-solid fa-pen"></i> Edit</button>` : ""}
      <button class="btn ghost" type="button" data-expense-tx-pdf="${escapeHtml(tx.id)}"><i class="fa-solid fa-file-pdf"></i> PDF</button>
      ${teamCanShowDelete("entries") ? `<button class="btn danger" type="button" data-expense-tx-delete="${escapeHtml(tx.id)}"><i class="fa-solid fa-trash"></i> Delete</button>` : ""}
    </div>`;
...
function renderExpensesList(){
  ...
  let html = "";
  const expenseRecordView = ["history", "topups", "transfers", "btc"].includes(state.expenseRecordView)
...
function ensureExpensesListDelegation(){
  ...
  els.expensesList.addEventListener("keydown", e => {
    ...
    if (!txRow || !els.expensesList.contains(txRow)) return;
  ...
  els.expensesList.addEventListener("click", async e => {
    ...
    const txMenuBtn = e.target.closest(".expense-tx-menu-btn");
    if (txMenuBtn && els.expensesList.contains(txMenuBtn)) {
      e.preventDefault(); e.stopPropagation();
      const panel = txMenuBtn.parentElement?.querySelector(".expense-tx-menu");
