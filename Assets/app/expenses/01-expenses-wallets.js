...
/** Bounds for lazy activity fetch: search uses filter-area dates (empty = full history). */
function expenseActivityQueryBounds(){
  const search = String(state.search.expenses || "").trim();
  if (search) {
    return {
      from: String(state.expenseDateFrom || "").trim(),
      to: String(state.expenseDateTo || "").trim(),
      label: (state.expenseDateFrom || state.expenseDateTo) ? "Filter dates" : "Full history",
      mode: "search"
    };
  }
...
function expenseHistoryRangeText(){
  // When searching, PDF/labels should reflect filter dates / full history — not the banner.
