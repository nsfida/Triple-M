...
(function () {
            ...
            <div class="asset-card-aside">
              ...
              <div class="menu-wrap asset-card-menu">
                ...
                <div class="menu-dropdown asset-card-dropdown" data-dep-menu-panel="${esc(asset.id)}" role="menu" hidden>
                  ...
                  <button type="button" class="menu-item dep-pdf-menu-item" role="menuitem" data-dep-card-action="pdf-overview" data-dep-id="${esc(asset.id)}"><i class="fa-solid fa-file-pdf"></i> PDF overview</button>
                  <button type="button" class="menu-item dep-pdf-menu-item" role="menuitem" data-dep-card-action="pdf-schedule" data-dep-id="${esc(asset.id)}"><i class="fa-solid fa-table-list"></i> PDF full schedule</button>
                  <button type="button" class="menu-item dep-pdf-menu-item" role="menuitem" data-dep-card-action="pdf-yearly" data-dep-id="${esc(asset.id)}"><i class="fa-solid fa-calendar-days"></i> PDF yearly summary</button>
                  <button type="button" class="menu-item dep-pdf-menu-item" role="menuitem" data-dep-card-action="pdf-valuation" data-dep-id="${esc(asset.id)}"><i class="fa-solid fa-scale-balanced"></i> PDF valuation</button>
                  ${asset.status === "sold" || asset.status === "disposed" ? `<button type="button" class="menu-item dep-pdf-menu-item" role="menuitem" data-dep-card-action="pdf-disposal" data-dep-id="${esc(asset.id)}"><i class="fa-solid fa-file-circle-check"></i> PDF disposal report</button>` : ""}
  ...
  function populateDepCurrencySelect(selected) {
    const select = document.getElementById("depAssetFormCurrency");
    if (!select) return;
    const wanted = String(selected || "AED").toUpperCase();
    if (typeof assetCurrencyOptionsHtml === "function") {
      select.innerHTML = assetCurrencyOptionsHtml(wanted);
    } else if (window.TriplemCurrencyRegistry?.all) {
      const rows = window.TriplemCurrencyRegistry.all().map(item => {
        const code = String(item?.code || "").toUpperCase();
        const symbol = String(item?.symbol || "");
        const cls = window.TriplemCurrencyRegistry?.fontClass?.(code) || "currency-font-normal";
        return `<option value="${esc(code)}" data-currency="${esc(code)}" class="${esc(cls)}">${esc(`${symbol ? symbol + " " : ""}${code}`)}</option>`;
      });
  ...
  function depPurchaseWalletAccounts(currency) {
    if (typeof assetPurchaseWalletAccounts === "function") return assetPurchaseWalletAccounts(currency);
    ...
    try {
      return getExpenseAccounts({ applyUiFilters: false })
        .filter(account => String(account?.currency || "").toUpperCase() === cur && cur !== "BTC")
  ...
  function populateDepPurchaseWalletSelect(options = {}) {
    const select = document.getElementById("depAssetFormWallet");
    const help = document.getElementById("depAssetFormWalletHelp");
    if (!select) return;
    const currency = String(document.getElementById("depAssetFormCurrency")?.value || "AED").toUpperCase();
    const previous = options.preserveSelection === false ? "" : String(select.value || "");
    const accounts = depPurchaseWalletAccounts(currency);
    ...
    accounts.forEach(account => {
      const id = esc(String(account.group_id || ""));
      const name = esc(String(account.person_name || "Wallet"));
      const balance = typeof formatReportAmount === "function"
        ? esc(String(formatReportAmount(Number(account.balance || 0), currency)))
        : esc(`${typeof currencySymbol === "function" ? currencySymbol(currency) : currency} ${Number(account.balance || 0).toLocaleString()}`);
      rows.push(`<option value="${id}" data-currency="${esc(currency)}" class="${esc(currencyClass)}">${name} · ${balance}</option>`);
    });
    if (!accounts.length) rows.push(`<option value="" data-currency="${esc(currency)}" class="${esc(currencyClass)}" disabled>No ${esc(currencyLabel)} wallets available</option>`);
    select.innerHTML = rows.join("");
    ...
    if (help) {
      help.textContent = accounts.length
        ...
        : `No ${currency} wallet is currently available. The depreciation asset can still be saved without a wallet deduction.`;
    }
    if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(select);
  }

  async function refreshDepPurchaseWallets(options = {}) {
    const select = document.getElementById("depAssetFormWallet");
    if (!select || depUi.formMode !== "create") return;
    const currency = String(document.getElementById("depAssetFormCurrency")?.value || "AED").toUpperCase();
    const previous = options.preserveSelection === false ? "" : String(select.value || "");
    if (options.load !== false && navigator.onLine !== false && typeof loadExpenseWalletSummaries === "function") {
      select.innerHTML = `<option value="">Loading ${esc(currency)} wallets…</option>`;
      try { await loadExpenseWalletSummaries({ force: false }); } catch (err) { console.warn("Depreciation asset wallet list refresh failed:", err); }
    }
    populateDepPurchaseWalletSelect({ preserveSelection: false });
    if (previous && depPurchaseWalletAccounts(currency).some(account => String(account.group_id || "") === previous)) select.value = previous;
    if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(select);
  }

  function validateDepPurchaseWallet(walletGroupId, amount, currency) {
    if (typeof validateAssetPurchaseWallet === "function") return validateAssetPurchaseWallet(walletGroupId, amount, currency);
    const walletId = String(walletGroupId || "").trim();
    if (!walletId) return null;
    const account = depPurchaseWalletAccounts(currency).find(row => String(row.group_id || "") === walletId);
    if (!account) throw new Error("Selected wallet was not found. Reopen the depreciation asset form to refresh wallets.");
    if (String(account.currency || "").toUpperCase() !== String(currency || "").toUpperCase()) throw new Error("Selected wallet currency does not match the depreciation asset currency.");
    if (Number(amount || 0) > Number(account.balance || 0) + 0.00000001) {
      const available = typeof formatReportAmount === "function" ? formatReportAmount(Number(account.balance || 0), account.currency) : `${account.currency} ${Number(account.balance || 0).toLocaleString()}`;
      throw new Error(`Insufficient wallet balance. Available: ${available}.`);
    }
    return account;
  }

  async function createDepPurchaseWalletEntry(walletGroupId, asset) {
    if (!walletGroupId || !(Number(asset?.purchase_price || 0) > 0)) return;
    validateDepPurchaseWallet(walletGroupId, asset?.purchase_price, asset?.currency);
    if (typeof createAssetPurchaseWalletEntry !== "function") throw new Error("Wallet ledger service is unavailable.");
    await createAssetPurchaseWalletEntry(walletGroupId, asset, {
      expenseType: "Depreciation Asset Purchase",
      notePrefix: "Depreciation asset purchase",
      saveLabel: "Depreciation asset purchase wallet deduction"
    });
  }
  ...
  function fillForm(asset) {
    ...
    populateDepCurrencySelect(asset?.currency || "AED");
    ...
    const walletWrap = document.getElementById("depAssetFormWalletWrap");
    const walletSelect = document.getElementById("depAssetFormWallet");
    ...
    if (!isEdit) populateDepPurchaseWalletSelect({ preserveSelection: false });
  ...
  function openForm(assetId) {
    ...
    const asset = assetId ? getById(assetId) : null;
    fillForm(asset);
    openModal("depAssetFormModal");
    document.getElementById("depAssetFormName")?.focus();
    if (!asset) refreshDepPurchaseWallets({ load: true, preserveSelection: true }).catch(err => console.warn("Depreciation wallet refresh failed:", err));
  }
  ...
  async function saveForm() {
    ...
    const purchaseWalletId = creating ? String(document.getElementById("depAssetFormWallet")?.value || "").trim() : "";
    if (purchaseWalletId) {
      try { validateDepPurchaseWallet(purchaseWalletId, input.purchase_price, input.currency); }
    ...
    try {
      if (creating && navigator.onLine === false) {
        ...
        let walletWarning = "";
        if (purchaseWalletId) {
          try { await createDepPurchaseWalletEntry(purchaseWalletId, payload); }
          catch (walletErr) { walletWarning = ` Wallet deduction could not be queued: ${walletErr?.message || walletErr}.`; console.warn("Offline depreciation wallet deduction failed:", walletErr); }
        }
        closeDepModal("depAssetFormModal");
        if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Depreciation asset “${input.name}” saved locally. Use Save to sync when internet returns.${walletWarning}`, walletWarning ? "warning" : "success");
        return;
      }
      if (!creating) {
      ...
      let walletWarning = "";
      if (creating && purchaseWalletId) {
        try { await createDepPurchaseWalletEntry(purchaseWalletId, payload); }
        catch (walletErr) { walletWarning = ` Wallet deduction failed: ${walletErr?.message || walletErr}.`; console.error("Depreciation asset saved but purchase wallet deduction failed.", walletErr); }
      }
      closeDepModal("depAssetFormModal");
      depUi.loaded = false;
      ...
      depUi.selectedId = assetId;
      openDetail(assetId);
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Depreciation asset ${creating ? "created" : "updated"}: ${input.name}.${walletWarning}`, walletWarning ? "warning" : "success");
      if (walletWarning) alert(`The depreciation asset was saved, but the wallet deduction failed: ${walletWarning.replace(/^ Wallet deduction failed: |\.$/g, "")}. Please add the matching wallet expense manually if required.`);
    } catch (err) {
  ...
  async function runReportPdf(options = {}) {
    if (!window.jspdf) {
    ...
    const singleAsset = options.assetId ? getById(options.assetId) : null;
    const singleKind = options.kind || "overview";
    const scheduleStart = singleAsset ? parseDateOnly(singleAsset.in_service_date || singleAsset.purchase_date) : null;
    const scheduleMonths = singleAsset ? Math.max(1, Math.round(Number(singleAsset.useful_life_years || 1) * 12)) : 0;
    const plannedScheduleEnd = scheduleStart ? toDateKey(addMonths(monthStart(scheduleStart), scheduleMonths - 1)) : "";
    const reportSelection = singleAsset
      ? {
          rows: [singleAsset],
          from: options.from ?? (singleAsset.in_service_date || singleAsset.purchase_date || ""),
          to: options.to ?? (singleKind === "schedule"
            ? (singleAsset.disposal_date || plannedScheduleEnd || toDateKey(new Date()))
            : (singleAsset.disposal_date || toDateKey(new Date()))),
          kind: singleKind
        }
      : reportFilterAssets();
    const { rows, from, to, kind } = reportSelection;
    ...
    const titles = {
      overview: "Depreciation Asset Overview",
      schedule: "Full Depreciation Schedule",
      monthly: "Monthly Depreciation Report",
      yearly: "Yearly Depreciation Report",
      valuation: "Asset Valuation Report",
      disposal: "Asset Disposal Report"
    ...
    if (useAssetPdfStyle) {
      ...
      y = drawAssetPdfHeadingBlock(doc, singleAsset ? singleAsset.name : "Depreciation assets", title, assetLayout.leftX, y, assetLayout.width);
      ...
      const kpis = singleAsset
        ? (() => {
            const c = computeDepreciation(singleAsset, parseDateOnly(to) || new Date());
            return [
              { label: "Cost", value: moneyPlain(singleAsset.purchase_price, singleAsset.currency), accent: ASSET_PDF_THEME.teal },
              { label: "Accumulated", value: moneyPlain(c.accumulated_depreciation, singleAsset.currency), accent: ASSET_PDF_THEME.amber },
              { label: "Book value", value: moneyPlain(c.current_book_value, singleAsset.currency), accent: ASSET_PDF_THEME.emerald },
              { label: "Remaining", value: String(c.remaining_label || "—"), accent: ASSET_PDF_THEME.ink }
            ];
          })()
        : [
      ...
      y = drawAssetPdfKpiCards(doc, kpis, assetLayout.leftX, y, assetLayout.width);
    ...
    if (kind === "overview" && singleAsset) {
      const c = computeDepreciation(singleAsset, parseDateOnly(to) || new Date());
      const proceeds = singleAsset.disposal_proceeds == null ? null : Number(singleAsset.disposal_proceeds || 0);
      head = [["Field", "Value"]];
      body = [
        ["Asset", singleAsset.name],
        ["Type", typeLabel(singleAsset.asset_type)],
        ["Status", statusLabel(singleAsset.status)],
        ["Method", methodLabel(singleAsset.depreciation_method)],
        ["Currency", singleAsset.currency],
        ["Purchase date", singleAsset.purchase_date || "—"],
        ["In-service date", singleAsset.in_service_date || singleAsset.purchase_date || "—"],
        ["Purchase cost", moneyPlain(singleAsset.purchase_price, singleAsset.currency)],
        ["Residual value", moneyPlain(singleAsset.salvage_value, singleAsset.currency)],
        ["Useful life", `${singleAsset.useful_life_years} years`],
        ["Monthly depreciation", moneyPlain(c.monthly_depreciation, singleAsset.currency)],
        ["Annual depreciation", moneyPlain(c.annual_depreciation, singleAsset.currency)],
        ["Accumulated depreciation", moneyPlain(c.accumulated_depreciation, singleAsset.currency)],
        ["Current book value", moneyPlain(c.current_book_value, singleAsset.currency)],
        ["Remaining useful life", c.remaining_label || "—"],
        ["Last estimate review", singleAsset.last_review_date || "—"],
        ["Disposal date", singleAsset.disposal_date || "—"],
        ["Disposal proceeds", proceeds == null ? "—" : moneyPlain(proceeds, singleAsset.currency)],
        ["Gain / (Loss)", proceeds == null ? "—" : moneyPlain(proceeds - c.current_book_value, singleAsset.currency)],
    ...
    const safeName = singleAsset
      ? (typeof assetPdfSafeFilename === "function" ? assetPdfSafeFilename(singleAsset.name) : String(singleAsset.name || "Asset").replace(/[^a-z0-9_-]+/gi, "_"))
      : "Portfolio";
    doc.save(`Depreciation_${safeName}_${kind}_${new Date().toISOString().slice(0, 10)}.pdf`);
    if (!singleAsset && options.closeModal !== false) closeDepModal("depAssetReportModal");
  }

  async function downloadDepAssetPdf(assetId, kind) {
    const asset = getById(assetId);
    if (!asset) throw new Error("Depreciation asset not found.");
    if (kind === "disposal" && asset.status !== "sold" && asset.status !== "disposed") {
      throw new Error("A disposal report is available after the asset is sold or disposed.");
    }
    await runReportPdf({ assetId, kind: kind || "overview", closeModal: false });
  }
  ...
  function openDepCardMenu(trigger) {
    ...
    if (typeof positionFixedMenuDropdown === "function") {
      positionFixedMenuDropdown(panel, trigger, { minWidth: 250 });
    } else {
      const rect = trigger.getBoundingClientRect();
      const panelWidth = Math.max(panel.offsetWidth || 250, 250);
  ...
  function bindUI() {
    ...
    document.getElementById("depAssetFormCurrency")?.addEventListener("change", () => {
      const select = document.getElementById("depAssetFormCurrency");
      if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(select);
      if (depUi.formMode === "create") refreshDepPurchaseWallets({ load: false, preserveSelection: false });
    });
    ...
    document.getElementById("depAssetsList")?.addEventListener("click", e => {
      ...
      if (actionBtn) {
        ...
        else if (action === "pdf-overview") downloadDepAssetPdf(id, "overview").catch(err => alert(err?.message || err));
        else if (action === "pdf-schedule") downloadDepAssetPdf(id, "schedule").catch(err => alert(err?.message || err));
        else if (action === "pdf-yearly") downloadDepAssetPdf(id, "yearly").catch(err => alert(err?.message || err));
        else if (action === "pdf-valuation") downloadDepAssetPdf(id, "valuation").catch(err => alert(err?.message || err));
        else if (action === "pdf-disposal") downloadDepAssetPdf(id, "disposal").catch(err => alert(err?.message || err));
