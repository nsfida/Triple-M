...
(function () {
  ...
  async function saveForm() {
    ...
    const calcInput = { ...input, status: existing?.status || "active", disposal_date: existing?.disposal_date || null };
    ...
    try {
      ...
      if (!creating) {
        ...
        await supabase(`depreciation_history?depreciation_asset_id=eq.${encodeURIComponent(assetId)}${ownerQ}`, { method: "DELETE" });
      } else {
        await supabase("depreciation_assets", { method: "POST", body: JSON.stringify(payload) });
      }
      if (histPayload.length) await supabase("depreciation_history", { method: "POST", body: JSON.stringify(histPayload) });
      closeDepModal("depAssetFormModal");
      depUi.loaded = false;
