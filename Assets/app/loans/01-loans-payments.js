...
async function deleteTransfer(entry) {
  ...
  if (!transferPartner) {
    ...
    if (!(await appConfirmDelete("Move this transfer record to recycle bin? No matching transfer partner found.", { title: "Delete transfer record?", confirmLabel: "Move to recycle bin" }))) return;
    addToRecycleBin(entry);
    unmarkDbSnapshotRows([entry]);
  ...
  if (!(await appConfirmDelete(confirmMessage, { title: "Delete transfer records?", confirmLabel: "Move both to recycle bin" }))) return;
