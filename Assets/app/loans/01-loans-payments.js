...
async function deleteInstallmentPayment(entryId){
  ...
  if (!isBackupMode()) {
    persistDeleteEntry(entry, { label: "Installment payment delete" }).catch(err => {
      ...
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Installment payment delete failed: ${err?.message || err}`, "error");
    });
  }
  ...
  if (typeof showEntryConfirmation === "function") {
    showEntryConfirmation(`Installment payment deleted: ${moneyText(entry.action_amount || 0, entry.currency)}.`, "success");
  }
  return true;
}
...
async function submitEdit(){
  ...
  if (editingInstallmentPayment) {
    ...
    if (typeof showEntryConfirmation === "function") {
      showEntryConfirmation(`Installment payment updated: ${moneyText(reconciledCurrent.action_amount || 0, reconciledCurrent.currency)}.`, "success");
    }
    return;
  }
  ...
  if(state.editKind === "principal"){
    ...
    if (hasGoodsTag(currentEntry.notes)) {
      ...
      const accountDetails = typeof collectExpenseAccountEditDetails === "function"
        ? collectExpenseAccountEditDetails()
        : normalizeExpenseDetails(expenseMetaFromNotes(currentEntry.notes).accountDetails || {});
    ...
    if (hasGoodsTag(currentEntry.notes)) {
      ...
      const editedDetails = typeof collectExpenseEditDetails === "function" ? collectExpenseEditDetails() : null;
      editedNotes = upsertExpenseMetaInNote(nt, {
        ...expenseMeta,
        ...(editedDetails != null ? { details: editedDetails } : {}),
        ...getEditTaxMeta(currentEntry, amt)
      });
    }
  ...
  if (isBackupMode() && typeof showEntryConfirmation === "function") {
    showEntryConfirmation("Entry updated locally.", "success");
  }
}
...
async function deleteEntry(id){
  ...
  if(entry.entry_kind === "principal"){
    ...
    if (isExpenseWallet) {
      ...
      if (!(await appConfirmDelete(`Delete the entire loan for ${entry.person_name}? This will move ALL linked repayments to recycle bin.`, { title: "Delete entire loan?", confirmLabel: "Move to recycle bin" }))) return;
    }
    ...
    if (isBackupMode()){
      ...
      persistDeleteGroup(entry.group_id, { entries: groupEntries, label: "Delete" }).catch(err => {
        ...
        if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Delete sync failed: ${err?.message || err}`, "error");
    ...
    const transferDeleted = await deleteTransfer(entry);
    if (!transferDeleted) return;
  } else {
    if (!(await appConfirmDelete("Move this entry to recycle bin?", { title: "Delete entry?", confirmLabel: "Move to recycle bin" }))) return;
    addToRecycleBin(entry);
    unmarkDbSnapshotRows([entry]);
  ...
  renderRecycleBinDropdown();
  if (typeof showEntryConfirmation === "function") {
    const deletedLabel = isExpenseWallet
      ? `Wallet “${entry.person_name || "Wallet"}” moved to recycle bin.`
      : hasGoodsTag(entry.notes)
        ? "Inventory entry moved to recycle bin."
        : hasExpenseAccountTag(entry.notes)
          ? "Expense entry moved to recycle bin."
          : hasInstallmentTag(entry.notes)
            ? "Installment entry moved to recycle bin."
            : "Loan entry moved to recycle bin.";
    showEntryConfirmation(deletedLabel, "success");
  }
}
...
async function deleteTransfer(entry) {
  ...
  if (!transferPartner) {
    ...
    if (!(await appConfirmDelete("Move this transfer record to recycle bin? No matching transfer partner found.", { title: "Delete transfer record?", confirmLabel: "Move to recycle bin" }))) return false;
    addToRecycleBin(entry);
    unmarkDbSnapshotRows([entry]);
...
async function deletePersonRecords(personNameEncoded, direction){
  ...
  if (!(await appConfirmDelete(`Move full record for ${personName} to recycle bin? This will move ${matchingEntries.length} entr${matchingEntries.length === 1 ? "y" : "ies"} from ${directionLabel} to recycle bin.`, { title: "Delete full record?", confirmLabel: "Move to recycle bin" }))) return;
  ...
  if (isBackupMode()){
    ...
    Promise.all([
      ...
      console.error("Person delete sync failed.", err);
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Loan record delete failed to sync: ${err?.message || err}`, "error");
    });
  }
  renderAll();
  renderRecycleBinDropdown();
  if (typeof showEntryConfirmation === "function") {
    showEntryConfirmation(`Loan record for ${personName} moved to recycle bin.`, "success");
  }
}
