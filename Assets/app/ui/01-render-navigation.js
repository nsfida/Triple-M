...
function renderInstallmentPlans(){
  ...
  container.innerHTML = plans.map(plan => {
    ...
    const pendingPlanEntry = [plan.principal, ...(plan.payments || [])].find(row =>
      row?.id && typeof hasPendingOfflineEntity === "function" && hasPendingOfflineEntity(row.id)
    );
    const pendingPlanId = pendingPlanEntry?.id || "";
          ...
          <div class="ip-card-head">
            <div class="ip-card-title">
              <div class="loan-name">
                ...
                ${pendingPlanId && typeof offlinePendingBadgeHtml === "function" ? offlinePendingBadgeHtml(pendingPlanId) : ""}
              </div>
            ...
            <div class="ip-card-actions">
              <div class="card-action-grid card-action-grid--single" role="group" aria-label="Plan actions">
                ${pendingPlanId && typeof offlineSyncButtonHtml === "function" ? offlineSyncButtonHtml(pendingPlanId) : ""}
                <div class="menu-wrap">
                  ...
                  <div class="menu-dropdown" data-person-menu-panel="${escapeHtml(plan.group_id)}">
                    ...
                    <button class="menu-item installmentActionBtn" type="button" data-action="pdf" data-group-id="${escapeHtml(plan.group_id)}"><i class="fa-solid fa-file-pdf"></i> Download statement</button>
  ...
  container.querySelectorAll(".person-menu-btn").forEach(btn => {
    btn.addEventListener("click", e => {
      e.preventDefault();
      e.stopPropagation();
      // The installment menu is temporarily portaled to <body> while open so it
      // cannot paint behind glass cards. Resolve it globally as well as locally,
      // otherwise a second click cannot find the already-portaled panel to close it.
      const menuKey = String(btn.dataset.personMenu || "");
      const localPanel = btn.closest(".menu-wrap")?.querySelector(".menu-dropdown");
      const panel = localPanel || Array.from(document.querySelectorAll("[data-person-menu-panel]")).find(
        candidate => String(candidate.dataset.personMenuPanel || "") === menuKey
      );
      if (!panel) return;
      ...
      if (nowOpen) {
        ...
        if (typeof positionFixedMenuDropdown === "function") {
          positionFixedMenuDropdown(panel, btn, { minWidth: 200, portal: true });
        } else {
...
function renderInstallmentPlanOverlayBody(plan){
  ...
  const paymentRows = (plan.payments || []).slice().sort((a, b) => dateStamp(b.action_date) - dateStamp(a.action_date)).map(row => {
    ...
    const paymentActions = !isDownPayment && row.id
      ? `<div class="card-action-grid" role="group" aria-label="Payment actions">
          ${typeof offlineSyncButtonHtml === "function" ? offlineSyncButtonHtml(row.id) : ""}
  ...
  return `
    ...
    <div class="ipo-actions">
      ...
      <button class="btn ghost tiny installmentOverlayBtn pdf-icon-btn" type="button" data-action="pdf" data-group-id="${escapeHtml(plan.group_id)}"><i class="fa-solid fa-file-pdf" aria-hidden="true"></i> PDF</button>
    </div>
...
const WORKSPACE_HOME_TAB = "home";

function workspaceTabKey(tab){
  const key = String(tab || "").trim();
  return ["given", "received", "taken", "returned", "loans"].includes(key) ? "loans" : key;
}

function getWorkspaceVisibleTabButtons(){
  return Array.from(document.querySelectorAll("#app > .tabs .tab[data-tab]"))
    .filter(btn => {
      const key = String(btn.dataset.tab || "").trim();
      if (!key || key === "messages" || key === "about") return false;
      return !btn.disabled && !btn.classList.contains("hide") && btn.getAttribute("aria-hidden") !== "true";
    });
}

function getWorkspaceSectionLabel(tab){
  const key = workspaceTabKey(tab);
  const button = getWorkspaceVisibleTabButtons().find(btn => workspaceTabKey(btn.dataset.tab) === key)
    || document.querySelector(`#app > .tabs .tab[data-tab="${key}"]`);
  const label = button?.querySelector(".tab-label")?.textContent || button?.textContent || button?.title || key;
  return String(label || "Section").replace(/\s+/g, " ").trim();
}

function workspaceSectionCopy(key){
  const copy = {
    dashboard: "Live financial intelligence and performance.",
    expenses: "Wallets, receiving, spending and transfers.",
    goods: "Stock, customers, sales and operations.",
    insurance: "Companies, policies, sales and commissions.",
    audit: "Trace activity with clear financial controls.",
    "triplem-ai": "Private AI assistance across your workspace.",
    assets: "Assets, value, depreciation and revenue.",
    loans: "Given, received, taken and returned balances.",
    installments: "Bought and sold payment schedules.",
    notes: "Organize notes, reminders and records.",
    bitcoin: "Bitcoin wallets, activity and valuation.",
    admin: "Users, access, plans and administration."
  };
...
function ensureWorkspaceNavigationEvents(){
  const back = document.getElementById("workspaceHomeBackBtn");
  if (back && back.dataset.boundWorkspaceHome !== "1") {
    back.dataset.boundWorkspaceHome = "1";
    back.addEventListener("click", () => activate(WORKSPACE_HOME_TAB));
  }
  const select = document.getElementById("workspaceSectionSelect");
  if (select && select.dataset.boundWorkspaceHome !== "1") {
    select.dataset.boundWorkspaceHome = "1";
    select.addEventListener("change", () => {
      const tab = String(select.value || "").trim();
      if (tab) activate(tab);
    });
  }
}
...
function renderWorkspaceHome(){
  const grid = document.getElementById("workspaceHomeGrid");
  if (!grid) return;
  const tabs = getWorkspaceVisibleTabButtons();
  grid.innerHTML = tabs.map((btn, index) => {
    const key = workspaceTabKey(btn.dataset.tab);
    const rawKey = String(btn.dataset.tab || key);
    ...
    return `
      <button class="workspace-home-card workspace-home-card--${escapeHtml(key)}" type="button" data-workspace-tab="${escapeHtml(rawKey)}" style="--workspace-card-index:${index}">
        ...
        <span class="workspace-card-open" aria-hidden="true"><i class="fa-solid fa-arrow-up-right-from-square"></i></span>
      </button>`;
  }).join("");
  grid.querySelectorAll("[data-workspace-tab]").forEach(card => {
    card.addEventListener("click", () => activate(card.dataset.workspaceTab));
  });
}

function showWorkspaceHome(){
  if (!state.unlocked) return;
  const home = document.getElementById("workspaceHome");
  ...
  document.getElementById("mainOverview")?.style.setProperty("display", "none");
  document.getElementById("walletsOverviewSection")?.style.setProperty("display", "none");
  if (nav) nav.classList.add("hide");
  if (home) home.classList.remove("hide");
  document.body.classList.add("workspace-home-active");
  document.body.dataset.workspaceSection = WORKSPACE_HOME_TAB;
  renderWorkspaceHome();
  ensureWorkspaceNavigationEvents();
  try { window.scrollTo({ top: 0, behavior: "auto" }); } catch (_) {}
}

function syncWorkspaceSectionNavigation(tab){
  const home = document.getElementById("workspaceHome");
  const nav = document.getElementById("workspaceSectionNav");
  const select = document.getElementById("workspaceSectionSelect");
  const title = document.getElementById("workspaceSectionTitle");
  const sectionKey = workspaceTabKey(tab);
  if (home) home.classList.add("hide");
  document.body.classList.remove("workspace-home-active");
  document.body.dataset.workspaceSection = sectionKey;
  if (nav) nav.classList.remove("hide");
  if (title) title.textContent = getWorkspaceSectionLabel(sectionKey);
  if (select) {
    const tabs = getWorkspaceVisibleTabButtons();
    const candidateButton = tabs.find(btn => workspaceTabKey(btn.dataset.tab) === sectionKey);
    const candidate = candidateButton?.dataset.tab || sectionKey;
    const options = [
      `<option value="${WORKSPACE_HOME_TAB}">Triplem VIP Home</option>`,
      ...tabs.map(btn => {
        const raw = String(btn.dataset.tab || "");
        return `<option value="${escapeHtml(raw)}">${escapeHtml(getWorkspaceSectionLabel(raw))}</option>`;
      })
    ];
    if (!candidateButton && tabIsAllowed(sectionKey)) {
      options.splice(1, 0, `<option value="${escapeHtml(candidate)}">${escapeHtml(getWorkspaceSectionLabel(sectionKey))}</option>`);
    }
...
function tabIsAllowed(tab){
  ...
  if (key === WORKSPACE_HOME_TAB) return true;
  if (key === "messages") {
    return !isGuestMode();
  }
  if (key === "triplem-ai") {
    return !isGuestMode() && state.triplemAi?.access === true && !state.trialLocked && !!document.getElementById("triplem-aiPanel");
  }
  if (key === "admin") {
    return !isGuestMode()
...
function resolveStartupTab(){
  const preferred = getPreferredDefaultTab() || WORKSPACE_HOME_TAB;
  if (tabIsAllowed(preferred)) return preferred === "loans" ? "given" : preferred;
  const fallbacks = [WORKSPACE_HOME_TAB, "dashboard", "expenses", "insurance", "audit", "triplem-ai", "given", "goods", "accounting", "notes", "bitcoin", "messages"];
...
function getPreferredDefaultTab(){
  ...
  return WORKSPACE_HOME_TAB;
}

function listDefaultStartPageOptions(){
  const options = [
    { id: WORKSPACE_HOME_TAB, label: "Triplem VIP Home", icon: "fa-solid fa-grip" },
    ...
    { id: "insurance", label: "Insurance", icon: "fa-solid fa-shield-heart" },
    { id: "audit", label: "Audit Report", icon: "fa-solid fa-file-shield" },
    { id: "triplem-ai", label: "Triplem AI", icon: "fa-solid fa-wand-magic-sparkles" },
...
function ensureDefaultStartPageModal(){
    ...
    <div class="modal-dialog default-start-page-dialog" role="dialog" aria-modal="true" aria-labelledby="defaultStartPageTitle">
      <div class="modal-head">
        <div>
          ...
          <p class="help">Choose Triplem VIP Home or a section to open when you sign in.</p>
        </div>
  ...
  document.getElementById("defaultStartPageSaveBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (status) status.textContent = `Saved. Next sign-in opens ${saved === WORKSPACE_HOME_TAB ? "Triplem VIP Home" : (saved === "loans" ? "Loans" : getWorkspaceSectionLabel(saved))}.`;
...
function openDefaultStartPageModal(){
  ...
  if (list) {
    if (!options.length) {
      list.innerHTML = `<div class="empty tiny">No pages available.</div>`;
    } else {
...
function resetActivePanelToDefault(){
  ...
  if (nav) nav.classList.add("hide");
  const home = document.getElementById("workspaceHome");
  if (state.unlocked && home) showWorkspaceHome();
  else {
    if (home) home.classList.add("hide");
    document.body.classList.remove("workspace-home-active");
    delete document.body.dataset.workspaceSection;
  }
}
...
function activate(tab){
  ...
  if (tab === WORKSPACE_HOME_TAB) {
    try { if (typeof window.setTriplemAiPageLock === "function") window.setTriplemAiPageLock(false); } catch (_) {}
    showWorkspaceHome();
  ...
  if (tab === "triplem-ai") {
    if (isGuestMode() || state.trialLocked || state.triplemAi?.access !== true) return;
  ...
  try { if (typeof window.setTriplemAiPageLock === "function") window.setTriplemAiPageLock(tab === "triplem-ai"); } catch (_) {}
  ...
  syncWorkspaceSectionNavigation(tab);
  ...
  if (walletsOverview) {
    if (tab === "bitcoin" || tab === "notes" || tab === "admin" || tab === "about" || tab === "messages" || tab === "dashboard" || tab === "assets" || tab === "audit" || tab === "triplem-ai") {
...
async function activateTabDataLoad(tab){
  try {
    if (tab === "insurance") {
      if (typeof window.prepareInsuranceWorkspace === "function") await window.prepareInsuranceWorkspace();
      return;
    }
    if (tab === "admin") {
      let allowed = true;
    ...
    if (tab === "dashboard") {
      ...
      try {
        if (typeof window.prepareFinancialTimelineDashboard === "function") window.prepareFinancialTimelineDashboard();
      } catch (timelineError) {
        console.warn("Financial Timeline prepare failed.", timelineError);
      }
      return;
    }
    ...
    if (tab === "triplem-ai") {
      try {
        if (typeof window.prepareTriplemAiWorkspace === "function") await window.prepareTriplemAiWorkspace();
      } catch (err) {
        console.error("Triplem AI prepare failed:", err);
      }
      return;
    }
    ...
    if (tab === "accounting") {
      try {
        if (typeof window.loadAccountingWorkspace === "function") {
          await window.loadAccountingWorkspace();
        }
      } catch (err) {
  ...
  if (nextDownPayment) {
    if (existingDownPayment) {
      ...
      if (!isBackupMode()) {
        ...
        }, "Installment down payment", nextDownPayment, { silent: true });
      }
    } else {
      saveEntriesImmediately(nextDownPayment, { label: "Installment down payment", notify: false });
    }
  }
  ...
  for (const row of remapped.remaps){
    ...
    if (!isBackupMode()) {
      queueDatabasePatch(row.id, {
        ...
        entry_kind: row.entry_kind,
        notes: row.notes
      }, "Installment payment remap", updatedPayment, { silent: true });
    }
  }
...
function openEditModal(id) {
  ...
  const expenseEditMeta = hasExpenseAccountTag(entry.notes) ? expenseMetaFromNotes(entry.notes) : null;
  const expenseEditRowType = String(expenseEditMeta?.rowType || "").toUpperCase();
  if (isExpenseAccountPrincipal && editHeadTitle) {
    editHeadTitle.textContent = "Edit wallet";
    if (editHeadDesc) editHeadDesc.textContent = "Update the wallet profile and opening balance without altering its transaction history.";
  } else if (expenseEditRowType === "TOPUP" && editHeadTitle) {
    editHeadTitle.textContent = "Edit money added";
    if (editHeadDesc) editHeadDesc.textContent = "Revise the amount, date, notes, and any optional source details.";
  } else if (expenseEditRowType === "EXPENSE" && editHeadTitle) {
    editHeadTitle.textContent = "Edit expense";
    if (editHeadDesc) editHeadDesc.textContent = "Revise the amount, date, notes, tax, and optional business details.";
  }
  ...
  if (entry.entry_kind === "principal") {
    ...
    document.getElementById('editAmountLabel').textContent = expenseEditRowType === "TOPUP" ? "Amount added" : expenseEditRowType === "EXPENSE" ? "Expense amount" : "Payment Amount";
    document.getElementById('editAmount').value = entry.action_amount || "";
    document.getElementById('editDateLabel').textContent = expenseEditRowType === "TOPUP" ? "Transaction date" : expenseEditRowType === "EXPENSE" ? "Expense date" : "Payment Date";
  ...
  if (typeof renderExpenseEditDetails === "function") renderExpenseEditDetails(entry);
  if (typeof renderExpenseAccountEditDetails === "function") renderExpenseAccountEditDetails(entry);
  if (accountTypeSelect && accountTypeSelect.dataset.expenseAccountDetailsBound !== "1") {
    accountTypeSelect.dataset.expenseAccountDetailsBound = "1";
    accountTypeSelect.addEventListener("change", () => {
      const activeEntry = state.entries.find(row => row.id === state.editId);
      if (typeof renderExpenseAccountEditDetails === "function") {
        renderExpenseAccountEditDetails(activeEntry, accountTypeSelect.value, true);
      }
    });
  }
...
async function submitInventoryEditItem(){
  ...
  for (const row of purchaseRows){
    ...
    if (!isBackupMode()) queueDatabasePatch(row.id, { person_name: itemName, currency, notes: nextNotes }, "Inventory stock", nextRow, { silent: true });
  }
  ...
  for (const row of saleRows){
    ...
    state.entries = state.entries.map(entry => entry.id === row.id ? nextRow : entry);
    if (!isBackupMode()) queueDatabasePatch(row.id, { person_name: itemName }, "Inventory sale", nextRow, { silent: true });
  }
...
function sectionDetailsThemeColors(){
  ...
  return {
    ...
    tooltipText: read("--chart-tooltip-text", read("--text", "")),
    tooltipBorder: read("--chart-tooltip-border", read("--line-strong", "")),
    crosshair: read("--chart-crosshair", read("--line-strong", "")),
...
function sectionDetailsLineDataset(label, data, color, opts = {}){
  ...
  return {
    ...
    pointBorderColor: opts.pointBorderColor ?? sectionDetailsThemeColors().doughnutBorder,
...
async function warmDashboardLegacyData(){
  const loads = [];
...
async function warmDashboardData(options = {}){
  if (typeof shouldUseDashboardSummary === "function" && shouldUseDashboardSummary()) {
    try {
      const summary = await loadDashboardSummary({ force: options.force === true });
      if (summary) return summary;
    } catch (error) {
      // Keep the Dashboard accurate if a temporary summary-RPC failure occurs.
      // The legacy path is only used as a safe fallback; detailed tabs stay lazy.
      console.warn("Dashboard summary unavailable; using the compatible fallback.", error);
    }
  }
  return warmDashboardLegacyData();
}
...
function loadDashboardSectionPayload(section, currencyHint = ""){
  if (typeof shouldUseDashboardSummary === "function" && shouldUseDashboardSummary()) {
    const data = typeof buildDashboardPayloadFromSummary === "function"
      ? buildDashboardPayloadFromSummary(section, currencyHint || dashboardSectionCurrency[section])
      : null;
    const currency = data?.selectedCurrency || "";
    if (currency) {
      dashboardSectionCurrency[section] = currency;
      dashboardSelectedCurrency = currency;
    }
    const loading = !state.dashboardSummary?.loaded;
    return { section, currency, data, loading, empty: !loading && !data };
  }
...
function dashboardHeroCardHtml(section, currency, data, opts = {}){
  if (!data) {
    const loading = opts.loading === true;
    return `
      <article class="dashboard-hero-card is-${escapeHtml(section)} ${loading ? "dashboard-loading-card" : "dashboard-empty-card"}" data-dashboard-hero-card="${escapeHtml(section)}" ${loading ? 'aria-busy="true"' : 'aria-busy="false"'}>
        <small>${escapeHtml(getDashboardSectionMeta(section)?.label || "Dashboard")} summary</small>
        <strong>${loading ? "Loading…" : "No data yet"}</strong>
        <div class="dashboard-hero-meta">${loading ? "Fetching authoritative figures" : "Add your first record to populate this section"}</div>
      </article>`;
  }
  if (section === "expenses") {
    return `
  ...
  return `
    <article class="dashboard-hero-card is-installments" data-dashboard-hero-card="installments">
      ...
      <strong>${escapeHtml(String(data.metrics.progressPct))}%</strong>
      <div class="dashboard-hero-meta">${escapeHtml(String(data.metrics.active))} active · ${escapeHtml(String(data.metrics.overdue))} overdue</div>
    </article>
  `;
}

function dashboardHeroHtml(section, currency, data, opts = {}){
  return `
    <div class="dashboard-hero dashboard-hero-single" data-dashboard-hero="1">
      ${dashboardHeroCardHtml(section, currency, data, opts)}
    </div>
  `;
}
...
function dashboardDesktopHeroHtml(payloads, sections){
  ...
  return `
    <div class="dashboard-hero" data-dashboard-hero="1" style="--dashboard-hero-cols:${list.length}">
      ${list.map(opt => dashboardHeroCardHtml(opt.id, payloads[opt.id]?.currency, payloads[opt.id]?.data, { loading: payloads[opt.id]?.loading === true })).join("")}
    </div>
  `;
}

function updateDashboardHeroCard(root, section, currency, data, opts = {}){
  if (!root) return;
  const card = root.querySelector(`[data-dashboard-hero-card="${section}"]`);
  if (!card) return;
  card.outerHTML = dashboardHeroCardHtml(section, currency, data, opts);
}


function dashboardSectionMetricsHtml(section, data, opts = {}){
  if (!data) {
    if (opts.loading === true) return `<div class="dashboard-loading-card" aria-busy="true" aria-label="Loading authoritative financial summary">
      <span class="dashboard-loading-line"></span><span class="dashboard-loading-line"></span><span class="dashboard-loading-line"></span>
    </div>`;
    return `<div class="dashboard-empty dashboard-empty-summary"><i class="fa-regular fa-chart-bar" aria-hidden="true"></i><strong>No data to summarize yet.</strong><span>Your charts will appear automatically after you add records.</span></div>`;
  }
  if (section === "expenses") {
    return [
        ...
        <table class="dashboard-assets-table">
          <thead>
            <tr>
              <th>Asset</th>
              <th>Status</th>
              <th class="num">Invested / spent</th>
...
function dashboardSectionBlockHtml(section, currency, data, opts = {}){
  ...
  return `
    <section class="dashboard-block${wide ? " is-wide" : ""}" data-dashboard-block="${escapeHtml(section)}">
      ...
      ${loansSplit}
      <div class="section-details-metrics" data-dashboard-metrics>${dashboardSectionMetricsHtml(section, data, { loading: opts.loading === true })}</div>
      <div data-dashboard-section-body>${data ? dashboardSectionChartsHtml(section) : (opts.loading === true ? "" : `<div class="dashboard-empty dashboard-empty-charts"><i class="fa-solid fa-chart-simple" aria-hidden="true"></i><span>No chart data to load yet.</span></div>`)}</div>
...
function paintDashboardSectionCharts(section, data){
  ...
  if (section === "assets") {
    ...
    makeChart(document.getElementById("dashboardChartAssetsMix"), {
      type: "doughnut",
      data: {
        labels: ["Purchase", "Invested / spent", "Revenue", "Sale proceeds"],
        datasets: [sectionDetailsDoughnutDataset(
          [
            Math.max(0, Number(data.metrics?.purchaseValue || 0)),
          ...
          [colors.black, colors.pink, colors.sky, colors.primary]
        )]
      },
...
function refreshDashboardSection(sectionKey, { soft = false, currencyHint = "" } = {}){
  ...
  const { currency, data, loading } = payload;

  if (soft) block.classList.add("is-updating");

  updateDashboardHeroCard(root, section, currency, data, { loading: loading === true });
  ...
  if (metrics) metrics.innerHTML = dashboardSectionMetricsHtml(section, data, { loading: loading === true });
  ...
  if (body) body.innerHTML = data
    ? dashboardSectionChartsHtml(section)
    : (loading === true ? "" : `<div class="dashboard-empty dashboard-empty-charts"><i class="fa-solid fa-chart-simple" aria-hidden="true"></i><span>No chart data to load yet.</span></div>`);
  if (data) paintDashboardSectionCharts(section, data);
...
function switchDashboardActiveSection(nextSection){
  ...
  const { currency, data, loading } = payload;
  content.innerHTML = `
    ${dashboardHeroHtml(section, currency, data, { loading: loading === true })}
    <div class="dashboard-grid" data-dashboard-section-host>
      ${dashboardSectionBlockHtml(section, currency, data, { mobile: true, forceWide: true, loading: loading === true })}
    </div>
  `;
  if (data) paintDashboardSectionCharts(section, data);
  ...
  if (mobile) {
    ...
    root.innerHTML = `
      <div class="dashboard-dynamic" data-dashboard-dynamic>
        ...
        <div class="dashboard-grid" data-dashboard-section-host>
          ${allowedSections.map(opt => dashboardSectionBlockHtml(opt.id, payloads[opt.id].currency, payloads[opt.id].data, { loading: payloads[opt.id].loading === true })).join("")}
        </div>
      </div>
    `;
    if (wasHydrated) root.classList.add("is-hydrated");
    ...
    bindDashboardSectionInteractions(root);
    allowedSections.forEach(opt => {
      if (payloads[opt.id].data) paintDashboardSectionCharts(opt.id, payloads[opt.id].data);
    });
  }
...
async function openInventoryItemDetailsOverlay(groupId){
  ...
  const id = String(groupId || "").trim();
  if (!id) return;
  els.sectionDetailsModal.dataset.detailsKind = "inventory-item";
  els.sectionDetailsModal.dataset.detailsId = id;
  ...
  if (!group) {
    ...
    if (els.sectionDetailsTitle) {
      const aiOrigin = (typeof isTriplemAiOrigin === "function" && isTriplemAiOrigin("inventory", group?.principal?.id)) || /\[AI_CREATED\]/i.test(String(group?.principal?.notes || ""));
      els.sectionDetailsTitle.innerHTML = `<i class="fa-solid fa-box"></i><span class="section-details-title-text">${escapeHtml(group.person_name || "Item")}</span>${aiOrigin && typeof aiOriginBadgeHtml === "function" ? aiOriginBadgeHtml("AI") : ""}`;
    }
    if (els.sectionDetailsDesc) {
...
function openLoanPdfSelectionModal(personName, direction){
  const data = typeof getLoanPersonPdfData === "function" ? getLoanPersonPdfData(personName, direction) : null;
  if (!data) return;
  let modal = document.getElementById("loanPdfSelectionModal");
  if (!modal) { modal=document.createElement("div"); modal.id="loanPdfSelectionModal"; modal.className="modal hide"; document.body.appendChild(modal); }
  modal.innerHTML = `<div class="modal-backdrop" data-loan-pdf-close></div><div class="modal-dialog loan-pdf-selection-dialog" role="dialog" aria-modal="true" aria-labelledby="loanPdfSelectionTitle"><div class="modal-head"><div><p class="loan-detail-kicker"><i class="fa-solid fa-file-pdf"></i> PDF export</p><h3 id="loanPdfSelectionTitle">Select loan transactions</h3><p>${escapeHtml(data.personName)} · ${direction === "given" ? "Given" : "Taken"}</p></div><button class="icon-btn ghost" data-loan-pdf-close type="button">×</button></div><div class="modal-body"><label class="loan-pdf-select-all"><input type="checkbox" id="loanPdfSelectAll" checked> <span>Select all ${data.rows.length} transactions</span></label><div class="loan-pdf-selection-list">${data.rows.map((r,i)=>`<label class="loan-pdf-selection-row"><input type="checkbox" data-loan-pdf-id="${escapeHtml(r.id)}" checked><span><strong>${escapeHtml(r.type)}</strong><small>${escapeHtml(displayDate(r.date))}${r.createdAt ? ` · ${escapeHtml(new Date(r.createdAt).toLocaleString())}` : ""}</small></span><b>${money(r.amount,data.currency)}</b></label>`).join("")}</div><div class="modal-footer"><button class="btn ghost" type="button" data-loan-pdf-close>Cancel</button><button class="btn primary" id="loanPdfDownloadSelected" type="button"><i class="fa-solid fa-file-pdf"></i> Download selected</button></div></div></div>`;
  const close=()=>closeModal("loanPdfSelectionModal"); modal.querySelectorAll("[data-loan-pdf-close]").forEach(el=>el.onclick=close);
  const all=modal.querySelector("#loanPdfSelectAll"); all.onchange=()=>modal.querySelectorAll("[data-loan-pdf-id]").forEach(cb=>cb.checked=all.checked);
  modal.querySelector("#loanPdfDownloadSelected").onclick=async()=>{const ids=[...modal.querySelectorAll("[data-loan-pdf-id]:checked")].map(cb=>cb.dataset.loanPdfId); if(!ids.length){alert("Select one or more transactions.");return;} await downloadLoanTransactionsSelectionPDF(encodeURIComponent(personName),direction,ids);};
  modal.classList.remove("hide"); modal.setAttribute("aria-hidden","false"); document.body.style.overflow="hidden";
}

function renderLoanDetailsActions(personName, direction){
  const host=els.sectionDetailsActions; if(!host)return;
  host.classList.remove("hide");
  host.innerHTML=`<div class="loan-detail-pdf-menu-wrap"><button type="button" class="btn ghost tiny" id="loanDetailPdfMenuBtn" aria-haspopup="menu" aria-expanded="false"><i class="fa-solid fa-file-pdf"></i> PDF <i class="fa-solid fa-chevron-down"></i></button><div class="loan-detail-pdf-menu" id="loanDetailPdfMenu" role="menu"><button type="button" data-loan-pdf-full><i class="fa-solid fa-file-lines"></i><span>Full statement</span></button><button type="button" data-loan-pdf-select><i class="fa-solid fa-list-check"></i><span>Select transactions</span></button></div></div>`;
  const btn=host.querySelector("#loanDetailPdfMenuBtn"), menu=host.querySelector("#loanDetailPdfMenu");
  btn.onclick=e=>{e.stopPropagation();const on=!menu.classList.contains("is-open");menu.classList.toggle("is-open",on);btn.setAttribute("aria-expanded",on?"true":"false");};
  host.querySelector("[data-loan-pdf-full]").onclick=async()=>{menu.classList.remove("is-open");await downloadPersonPDF(encodeURIComponent(personName),direction);};
  host.querySelector("[data-loan-pdf-select]").onclick=()=>{menu.classList.remove("is-open");openLoanPdfSelectionModal(personName,direction);};
}
...
function openLoanDetailsOverlay(personName, direction){
  ...
  const dir = direction === "given" ? "given" : "taken";
  if (!name) return;
  els.sectionDetailsModal.dataset.detailsKind = "loan";
  els.sectionDetailsModal.dataset.detailsPerson = name;
  els.sectionDetailsModal.dataset.detailsDirection = dir;
  ...
  if (!data) {
    ...
    if (els.sectionDetailsTitle) {
      const aiOrigin = state.entries?.some?.(entry => String(entry?.person_name || "").trim().toLowerCase() === name.toLowerCase() && String(entry?.direction || "") === dir && (((typeof isTriplemAiOrigin === "function") && isTriplemAiOrigin("loans", entry?.id, entry?.group_id)) || /\[AI_CREATED\]/i.test(String(entry?.notes || ""))));
      els.sectionDetailsTitle.innerHTML = `<i class="fa-solid fa-user"></i><span class="section-details-title-text">${escapeHtml(data.person_name)}</span>${aiOrigin && typeof aiOriginBadgeHtml === "function" ? aiOriginBadgeHtml("AI") : ""}`;
    }
    if (els.sectionDetailsDesc) {
    ...
    renderLoanDetailsActions(name, dir);
    if (!sectionDetailsEnsureChartLib()) {
      els.sectionDetailsBody.innerHTML = `<div class="section-details-empty">Chart library is still loading. Close and open Details again.</div>`;
    } else {
