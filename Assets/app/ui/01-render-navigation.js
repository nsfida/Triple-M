...
function loadDashboardSectionPayload(section, currencyHint = ""){
  if (typeof shouldUseDashboardSummary === "function" && shouldUseDashboardSummary()) {
    ...
    return { section, currency, data, loading: !state.dashboardSummary?.loaded };
  }
...
function dashboardHeroCardHtml(section, currency, data){
  if (!data) {
    return `
      <article class="dashboard-hero-card is-${escapeHtml(section)} dashboard-loading-card" data-dashboard-hero-card="${escapeHtml(section)}" aria-busy="true">
        <small>${escapeHtml(getDashboardSectionMeta(section)?.label || "Dashboard")} summary</small>
        <strong>Loading…</strong>
        <div class="dashboard-hero-meta">Fetching authoritative figures</div>
      </article>`;
  }
  if (section === "expenses") {
    return `
...
function dashboardSectionMetricsHtml(section, data){
  if (!data) return `<div class="dashboard-loading-card" aria-busy="true" aria-label="Loading authoritative financial summary">
...
function dashboardSectionBlockHtml(section, currency, data, opts = {}){
  ...
  return `
    <section class="dashboard-block${wide ? " is-wide" : ""}" data-dashboard-block="${escapeHtml(section)}">
      ...
      <div data-dashboard-section-body>${data ? dashboardSectionChartsHtml(section) : ""}</div>
