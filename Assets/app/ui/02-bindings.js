...
function collapseMainOverview() {
  ...
  if (els.toggleMainOverviewBtn) els.toggleMainOverviewBtn.setAttribute("aria-expanded", "false");
  setMainOverviewHeading(getActiveTabKey() === "goods" ? "inventory" : "loans");
  if (els.toggleMainOverviewBtn) els.toggleMainOverviewBtn.setAttribute("aria-label", els.toggleMainOverviewBtn.title || "Expand overview");
}
...
function openUserHelpSupportModal(){
  let modal = document.getElementById("userHelpSupportModal");
  if (!modal) {
    modal = document.createElement("div");
    modal.id = "userHelpSupportModal";
    modal.className = "modal hide";
    document.body.appendChild(modal);
  }
  modal.innerHTML = `<div class="modal-backdrop" data-user-help-close></div>
    <div class="modal-dialog settings-sheet user-help-support-sheet" role="dialog" aria-modal="true" aria-labelledby="userHelpSupportTitle">
      <div class="settings-sheet-head">
        <div><h3 id="userHelpSupportTitle"><i class="fa-solid fa-circle-question"></i> Help &amp; Support</h3><p>Official Triplem VIP assistance channels</p></div>
        <button type="button" class="btn ghost tiny" data-user-help-close aria-label="Close">✕</button>
      </div>
      <div class="modal-body settings-sheet-body user-help-support-body">
        <p class="user-help-support-intro">For account help, product guidance or general assistance, contact Triplem VIP through either official channel below.</p>
        <a class="user-help-support-card" href="mailto:contact@triplem.vip"><i class="fa-solid fa-envelope"></i><span><small>Email support</small><strong>contact@triplem.vip</strong></span><i class="fa-solid fa-arrow-up-right-from-square"></i></a>
        <a class="user-help-support-card user-help-support-card--whatsapp" href="https://wa.me/923339004564" target="_blank" rel="noopener noreferrer"><i class="fa-brands fa-whatsapp"></i><span><small>WhatsApp support</small><strong>+923339004564</strong></span><i class="fa-solid fa-arrow-up-right-from-square"></i></a>
        <div class="user-help-support-note"><i class="fa-solid fa-shield-halved"></i><span>Use only these official support channels for account-related assistance. Never share passwords, Smart PINs, verification codes or recovery secrets.</span></div>
      </div>
    </div>`;
  ...
  modal.querySelectorAll("[data-user-help-close]").forEach(el => el.onclick = close);
...
window.addEventListener("resize", () => {
  ...
  [els.entryModal, els.editModal, els.goodsModal, els.goodsSettlementModal, els.inventoryCustomerModal, els.inventoryEditItemModal, els.installmentPlanModal, els.installmentEditModal, document.getElementById("noteCreateModal"), document.getElementById("noteReminderModal"), document.getElementById("noteDetailModal"), els.expenseModal, els.sectionDetailsModal].forEach(m => {
    if (!m) return;
  ...
  document.addEventListener("keydown", e => {
    if (e.key === "Escape") {
      ...
      const noteCreateModal = document.getElementById("noteCreateModal");
      if (noteCreateModal && !noteCreateModal.classList.contains("hide")) closeModal("noteCreateModal");
      const noteDetailModal = document.getElementById("noteDetailModal");
      if (noteDetailModal && !noteDetailModal.classList.contains("hide")) closeModal("noteDetailModal");
  ...
  document.getElementById("userHelpBtn")?.addEventListener("click", e => {
    e.preventDefault();
    e.stopPropagation();
    closeAllMenus();
    openUserHelpSupportModal();
  });
  ...
  const inventoryLayoutSelect = document.getElementById("inventoryLayoutSelect");
  if (inventoryLayoutSelect) {
    inventoryLayoutSelect.value = state.inventoryStockLayout === "list" ? "list" : "category";
    inventoryLayoutSelect.addEventListener("change", () => {
      const next = inventoryLayoutSelect.value === "list" ? "list" : "category";
      if (state.inventoryStockLayout !== next) {
  ...
  const applyExpenseCustomDateBtn = document.getElementById("applyExpenseCustomDateBtn");
  if (typeof syncExpenseDateFilterControls === "function") syncExpenseDateFilterControls();
  if (expenseDateRangeFilter) {
    expenseDateRangeFilter.addEventListener("change", async e => {
      const allowed = new Set(["today", "yesterday", "last7", "month", "all", "custom"]);
      const nextRange = allowed.has(e.target.value) ? e.target.value : "today";
      if (nextRange === "custom" && (!state.expenseHistoryCustomFrom || !state.expenseHistoryCustomTo)) {
        const today = typeof localDateInputValue === "function" ? localDateInputValue(new Date()) : new Date().toISOString().slice(0, 10);
        state.expenseHistoryCustomFrom = state.expenseHistoryCustomFrom || today;
        state.expenseHistoryCustomTo = state.expenseHistoryCustomTo || today;
      }
      await setExpenseHistoryRange(nextRange, state.expenseRecordView === "history");
    });
  }
  if (applyExpenseCustomDateBtn) {
    applyExpenseCustomDateBtn.addEventListener("click", async () => {
      const fromValue = String(expenseCustomDateFrom?.value || "");
      const toValue = String(expenseCustomDateTo?.value || "");
      const completeDate = value => /^\d{4}-\d{2}-\d{2}$/.test(value);
      if (!completeDate(fromValue) || !completeDate(toValue)) {
        alert("Please select both custom dates.");
        return;
      }
      ...
      await setExpenseHistoryRange("custom", state.expenseRecordView === "history");
    });
  }
  ...
  document.getElementById("trialExpiredSettingsBtn")?.addEventListener("click", () => {
    hideTrialExpiredOverlay();
    if (typeof openPlanSubscriptionModal === "function") openPlanSubscriptionModal();
    else openAccountSettingsModal();
  });
