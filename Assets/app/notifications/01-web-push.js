/* Triplem VIP secure Web Push client + Main Admin/visitor notification center — v124 */
(() => {
  "use strict";
  ...
  async function toggleAccountNotifications() {
    const subscription = await currentSubscription().catch(() => null);
    if (getAccountPreference() !== "off" && subscription && Notification.permission === "granted") await disable();
    else await enable();
  }
  ...
  function bindUserToggle() {
    [document.getElementById("pushNotificationToggleBtn"), document.getElementById("pushQuickToggleBtn")].filter(Boolean).forEach(btn => {
      ...
      btn.addEventListener("click", async event => {
        ...
        btn.disabled = true;
        try { await toggleAccountNotifications(); }
        catch (error) { alert(error?.message || "Device notifications could not be updated."); }
        finally { btn.disabled = false; refreshControl().catch(() => {}); }
      });
    });
  }
  ...
  function ensurePushConsentModal() {
    ...
    modal.innerHTML = `
      <div class="push-consent-card" role="dialog" aria-modal="true" aria-labelledby="pushConsentTitle">
        ...
        <div class="push-consent-copy"><p class="push-consent-kicker">Triplem VIP Notifications</p><h3 id="pushConsentTitle">Stay connected to important activity</h3><p id="pushConsentText">Enable secure device notifications for messages, account activity, reminders and important security updates, including when your browser is closed.</p></div>
        <p class="push-consent-error" id="pushConsentError" aria-live="polite"></p>
        <div class="push-consent-actions"><button type="button" class="btn ghost push-consent-later" id="pushConsentLaterBtn">Keep off for now</button><button type="button" class="btn primary push-consent-enable" id="pushConsentEnableBtn"><i class="fa-solid fa-bell"></i> Enable notifications</button></div>
      </div>`;
  ...
  function openPushConsentModal(mode) {
    ...
    if (title) title.textContent = accountMode
      ? (browserBlocked ? "Notifications are currently blocked" : "Stay connected to important activity")
    ...
    if (text) text.textContent = accountMode
      ? (browserBlocked
          ...
          : "Enable secure device notifications for messages, account activity, reminders and important security updates, including when your browser is closed.")
    ...
    if (laterBtn) laterBtn.textContent = accountMode ? "Keep off for now" : "Not now";
