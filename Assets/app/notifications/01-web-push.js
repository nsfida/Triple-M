...
(() => {
  ...
  async function disable() {
    ...
    if (subscription) {
      ...
      if (endpoint && typeof window.supabaseRpc === "function" && typeof state !== "undefined" && state?.sessionToken) {
        await window.supabaseRpc("app_unregister_push_subscription", { p_endpoint: endpoint }).catch(() => {});
      }
      await subscription.unsubscribe().catch(() => false);
    }
  ...
  async function refreshControl() {
    ...
    if (subscription && Notification.permission === "granted") {
      setPushControlStatus({ enabled: true, label: "Device notifications on", detail: "Works when Triplem VIP is closed." });
    } else {
  ...
  function bindUserToggle() {
    ...
    btn.addEventListener("click", async event => {
      ...
      try {
        const subscription = await currentSubscription().catch(() => null);
        if (subscription && Notification.permission === "granted") await disable();
        else await enable();
      } catch (error) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-dialog admin-push-dialog" role="dialog" aria-modal="true" aria-labelledby="adminPushTitle">
        <div class="modal-head admin-push-head">
          <div><p class="admin-push-kicker"><i class="fa-solid fa-bell"></i> Web Push Center</p><h3 id="adminPushTitle">Send Triplem VIP notification</h3><p>Securely notify all registered users or a selected audience.</p></div>
  ...
  function renderAdminPushUsers() {
    ...
    list.innerHTML = rows.length ? rows.map(user => {
      ...
      return `<label class="admin-push-user-row ${checked ? "is-selected" : ""}"><input type="checkbox" data-admin-push-user="${esc(id)}" ${checked ? "checked" : ""}/><span class="admin-push-user-avatar"><i class="fa-solid fa-user"></i></span><span><strong>${esc(name)}</strong><small>${esc(meta)}</small></span></label>`;
  ...
  function updateAdminPushSummary() {
    ...
    if (summary) {
      summary.innerHTML = stateLocal.adminMode === "all"
        ? `<i class="fa-solid fa-users"></i><div><strong>${rows.length} registered user${rows.length === 1 ? "" : "s"}</strong><span>All active accounts except your Main Admin account will receive the in-app notification.</span></div>`
        : `<i class="fa-solid fa-user-check"></i><div><strong>${selected} selected user${selected === 1 ? "" : "s"}</strong><span>Search and choose one or several recipients below.</span></div>`;
    }
  }

  function setAdminPushMode(mode) {
    stateLocal.adminMode = mode === "selected" ? "selected" : "all";
  ...
  async function openAdminPushModal() {
    ...
    await ensureAdminUsersLoaded().catch(() => []);
    renderAdminPushUsers();
    updateAdminPushSummary();
  }
  ...
  async function sendAdminPush() {
    ...
    const audience = stateLocal.adminMode === "all" ? `${adminUsers().length} registered users` : `${selected.length} selected user${selected.length === 1 ? "" : "s"}`;
  ...
  function bind() {
    ...
    window.setTimeout(() => syncExistingSubscription().catch(() => {}), 900);
    ...
    if (!stateLocal.bound) {
      ...
      window.addEventListener("focus", () => { syncExistingSubscription().catch(() => {}); bindAdminPush(); }, { passive: true });
