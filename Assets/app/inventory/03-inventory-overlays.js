    ...
    <tr>
      ...
      <td>
        <div class="inventory-history-actions">
          ${typeof offlineSyncButtonHtml === "function" && row.entryId ? offlineSyncButtonHtml(row.entryId) : ""}
          ${row.isSale || row.kind === "Settlement"
            ? `<button class="tiny sectionHistoryBtn" data-action="receipt" data-id="${escapeHtml(row.entryId)}" title="Receipt PDF"><i class="fa-solid fa-file-pdf"></i></button>
               ${row.isSale ? `<button class="tiny soft inventoryThermalPrintBtn" type="button" data-entry-id="${escapeHtml(row.entryId)}" title="Thermal merchant receipt"><i class="fa-solid fa-receipt"></i></button>` : ""}`
...
async function deleteInventoryReceipt(entryId){
  ...
  const ok = await appConfirmDelete(
    `Delete invoice ${invoiceNumber} (${lineCount} line${lineCount === 1 ? "" : "s"})?\n\nSold quantity will return to stock.`,
    { title: "Delete invoice?", confirmLabel: "Delete invoice", note: "The invoice moves through the existing recycle-bin flow where supported, and sold quantity returns to stock." }
  );
  if (!ok) return false;
  ...
  if (typeof showEntryConfirmation === "function") {
    if (deleteSyncFailures > 0) {
      showEntryConfirmation(`Invoice ${invoiceNumber} was removed locally, but ${deleteSyncFailures} database delete${deleteSyncFailures === 1 ? "" : "s"} failed to sync.`, "error");
    } else {
      showEntryConfirmation(`Invoice ${invoiceNumber} deleted and stock restored.`, "success");
    }
  }
  return true;
}
...
function openInventoryReceiptEditor(entryId){
  ...
  const aiOrigin = (typeof isTriplemAiOrigin === "function" && isTriplemAiOrigin("inventory", saleEntry.id)) || /\[AI_CREATED\]/i.test(String(saleEntry.notes || ""));
  if (titleEl) titleEl.innerHTML = `Edit sales invoice ${aiOrigin && typeof aiOriginBadgeHtml === "function" ? aiOriginBadgeHtml("AI") : ""}`;
      ...
      <article class="inventory-variant-row ${inStock ? "" : "is-empty"}" data-sku-group="${escapeHtml(group.group_id)}">
        <div class="inventory-variant-main">
          <div class="inventory-variant-identity">
            ...
            <div class="inventory-variant-identity-text">
              ...
              <div class="inventory-section-item-sub">
                ...
                ${pendingEntryId && typeof offlinePendingBadgeHtml === "function" ? offlinePendingBadgeHtml(pendingEntryId) : ""}
              </div>
            </div>
          </div>
          ...
          <div class="inventory-variant-actions">
            ${pendingEntryId && typeof offlineSyncButtonHtml === "function" ? offlineSyncButtonHtml(pendingEntryId) : ""}
...
async function renderInventorySectionOverlayBody(sectionType){
  ...
  if (activeBrand && activeLine && (cfg.usesProductLines || cfg.usesVariants)) {
    ...
    bindInventoryCatalogMenus(body, async (action, { dataset }) => {
      ...
      if (action === "delete") {
        if (!(await appConfirmDelete(`Delete ${tax.variant.toLowerCase()} “${oldLabel}”?`, { title: `Delete ${tax.variant.toLowerCase()}?`, confirmLabel: "Delete" }))) return;
  ...
  if (activeBrand && cfg.usesProductLines) {
    ...
    bindInventoryCatalogMenus(body, async (action, { dataset }) => {
      if (dataset.catalogKind === "sub-brand" || dataset.subBrandId) {
        ...
        if (!(await appConfirmDelete(`Delete ${subBrandLabel.toLowerCase()} “${dataset.subBrandName || ""}” and its ${tax.productLinePlural}/${tax.variantPlural}/stock?`, { title: `Delete ${subBrandLabel.toLowerCase()}?`, confirmLabel: "Delete" }))) return;
      ...
      if (action === "delete") {
        ...
        if (!(await appConfirmDelete(stockCount
          ? `Delete ${tax.productLine.toLowerCase()} “${oldName}” and ${stockCount} stock item${stockCount === 1 ? "" : "s"}?`
          : `Delete ${tax.productLine.toLowerCase()} “${oldName}”?`,
          { title: `Delete ${tax.productLine.toLowerCase()}?`, confirmLabel: "Delete" }))) return;
  ...
  bindInventoryCatalogMenus(body, async (action, { dataset }) => {
    ...
    if (action === "delete") {
      ...
      if (!(await appConfirmDelete(msg, { title: "Delete inventory records?", confirmLabel: "Delete" }))) return;
      if (brandId) {
        try {
...
function renderInventorySectionsGrid(groups){
  ...
  bindInventoryCatalogMenus(els.goodsList, async (action, { dataset }) => {
    ...
    if (action === "delete") {
      ...
      if (!(await appConfirmDelete(warn, { title: "Delete category?", confirmLabel: "Continue", note: "A Smart Pin confirmation follows before permanent deletion." }))) return;
...
function renderInventoryList(){
  ...
  const paint = () => {
    ...
    if (state.inventoryLazy.loading && !groups.length){
      ...
      updateSaleDraftDock();
      appendInventoryLoadMoreControl();
      return;
    }
    if (!groups.length){
      const filters = [];
...
function appendInventoryLoadMoreControl(){
  if (!els.goodsList || !isInventoryLazyMode() || !state.inventoryLazy.summaryHasMore) return;
  if (els.goodsList.querySelector(".inventory-load-more")) return;
  const wrap = document.createElement("div");
  wrap.className = "lazy-load-more";
  wrap.innerHTML = `<button type="button" class="btn soft inventory-load-more" ${state.inventoryLazy.loading ? "disabled" : ""}>${state.inventoryLazy.loading ? "Loading…" : "Load more items"}</button>`;
  els.goodsList.appendChild(wrap);
}
...
function ensureInventoryItemDetailsDelegation(){
  ...
  container.addEventListener("click", e => {
    const loadMore = e.target.closest(".inventory-load-more");
    if (loadMore && container.contains(loadMore)) {
      e.preventDefault();
      e.stopPropagation();
      loadMore.disabled = true;
      loadMore.textContent = "Loading…";
      loadInventorySummaries({ append: true })
        .then(() => renderInventoryList())
        .catch(error => console.warn("More inventory items could not be loaded.", error));
      return;
    }
