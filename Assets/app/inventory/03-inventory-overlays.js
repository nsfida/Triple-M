...
function renderInventorySectionsGrid(groups){
  ...
  const startInlineCategoryCreate = () => {
    ...
    wrap.className = "inventory-section-card inventory-section-create-card inventory-section-create-card-wide";
    wrap.id = "inventoryInlineCategoryCreate";
    const defaults = typeof getCategoryConfig === "function"
    ...
    wrap.innerHTML = (typeof categoryBuilderFormHtml === "function"
      ? categoryBuilderFormHtml({
          name: "",
          usesBrands: true,
          usesSubBrands: true,
          ...
          idPrefix: "inventoryNewCategory",
          title: "New category",
          badge: "Setup",
          actionsHtml: `
            <div class="inventory-section-card-actions">
              <button type="button" class="tiny primary" id="inventoryNewCategorySaveBtn">Save category</button>
    ...
    const builder = wrap.querySelector("[data-cat-builder]");
    if (builder && typeof bindCategoryBuilderPreview === "function") bindCategoryBuilderPreview(builder);
    const input = wrap.querySelector('[data-cat-field="name"]') || wrap.querySelector("#inventoryNewCategoryNameInput");
    const save = async () => {
      ...
      try {
        const created = await addCustomCategory(name, data || {});
  ...
  bindInventoryCatalogMenus(els.goodsList, async (action, { dataset }) => {
    ...
    if (action === "edit") {
      const host = els.goodsList.querySelector(".inventory-sections-grid");
      if (!host) return;
      host.querySelector("#inventoryInlineCategoryEdit")?.remove();
      const wrap = document.createElement("article");
      wrap.className = "inventory-section-card inventory-section-create-card inventory-section-create-card-wide";
      wrap.id = "inventoryInlineCategoryEdit";
      wrap.innerHTML = typeof categoryBuilderFormHtml === "function"
        ? categoryBuilderFormHtml(cfg, {
            idPrefix: "inventoryEditCategory",
            title: "Edit category",
            badge: "Edit",
            actionsHtml: `
              <div class="inventory-section-card-actions">
                <button type="button" class="tiny primary" id="inventoryEditCategorySaveBtn">Save changes</button>
                <button type="button" class="tiny ghost" id="inventoryEditCategoryCancelBtn">Cancel</button>
              </div>`
          })
      ...
      const builder = wrap.querySelector("[data-cat-builder]");
      if (builder && typeof bindCategoryBuilderPreview === "function") bindCategoryBuilderPreview(builder);
      wrap.querySelector("#inventoryEditCategoryCancelBtn")?.addEventListener("click", e => {
        e.preventDefault();
        e.stopPropagation();
        wrap.remove();
      });
      wrap.querySelector("#inventoryEditCategorySaveBtn")?.addEventListener("click", async e => {
        e.preventDefault();
        e.stopPropagation();
        const data = typeof readCategoryBuilderForm === "function" ? readCategoryBuilderForm(builder) : null;
        ...
        const saveBtn = wrap.querySelector("#inventoryEditCategorySaveBtn");
        if (saveBtn) saveBtn.disabled = true;
        try {
      ...
      setTimeout(() => wrap.querySelector('[data-cat-field="name"]')?.focus(), 30);
      return;
    }
    if (action === "delete") {
