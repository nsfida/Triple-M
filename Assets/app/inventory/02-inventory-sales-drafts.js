...
/* Build 011 — elevated loan transaction actions/details. */
let loanTxMenuPortalState = { panel: null, placeholder: null, button: null };
function closeLoanTransactionMenus(){
  const { panel, placeholder, button } = loanTxMenuPortalState;
  if (panel) {
  ...
  document.querySelectorAll(".loan-tx-menu.is-open").forEach(menu => menu.classList.remove("is-open", "is-portal"));
  document.querySelectorAll(".loan-tx-menu-btn[aria-expanded=true]").forEach(btn => btn.setAttribute("aria-expanded", "false"));
  loanTxMenuPortalState = { panel: null, placeholder: null, button: null };
}
function openLoanTransactionMenu(button, panel){
  if (!button || !panel) return;
  closeLoanTransactionMenus();
  const placeholder = document.createComment("loan-transaction-menu");
  ...
  const w = Math.max(178, Math.min(230, panel.offsetWidth || 190));
  ...
  const top = below + h <= window.innerHeight - margin ? below : Math.max(margin, r.top - h - 7);
  panel.style.left = `${Math.round(left)}px`;
  panel.style.top = `${Math.round(top)}px`;
  panel.style.right = "auto";
  panel.style.visibility = "visible";
  button.setAttribute("aria-expanded", "true");
  loanTxMenuPortalState = { panel, placeholder, button };
}
function loanTransactionDetailData(entryId, personName, direction){
  const id = String(entryId || "");
  const person = decodeURIComponent(String(personName || ""));
  const dir = direction === "given" ? "given" : "taken";
  if (typeof getLoanPersonPdfData === "function") {
    const data = getLoanPersonPdfData(person, dir);
    const row = data?.rows?.find?.(r => String(r.id) === id);
    if (data && row) return { ...row, personName: data.personName, direction: dir, currency: data.currency };
  }
  ...
  return {
    id: entry.id,
    personName: entry.person_name || person,
    direction: entry.direction === "given" ? "given" : dir,
    currency: entry.currency || "",
    type: isPrincipal ? "Principal" : (entry.entry_kind === "partial" ? "Partial payment" : "Full payment"),
    date: isPrincipal ? entry.loan_date : entry.action_date,
    createdAt: entry.created_at || null,
    amount: Number(isPrincipal ? entry.principal_amount : entry.action_amount || 0),
    remainingAfter: null,
    note: entry.notes || "—"
  };
}
function ensureLoanTransactionDetailModal(){
  let modal = document.getElementById("loanTransactionDetailModal");
  if (modal) return modal;
  modal = document.createElement("div");
  modal.id = "loanTransactionDetailModal";
  modal.className = "modal hide";
  modal.setAttribute("aria-hidden", "true");
  modal.innerHTML = `<div class="modal-backdrop" data-loan-tx-close></div><div class="modal-dialog loan-tx-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="loanTxDetailTitle"><div class="modal-head"><div><p class="loan-detail-kicker"><i class="fa-solid fa-hand-holding-dollar"></i> Loan transaction</p><h3 id="loanTxDetailTitle">Transaction details</h3><p id="loanTxDetailMeta"></p></div><button class="icon-btn ghost" type="button" data-loan-tx-close aria-label="Close">×</button></div><div class="modal-body" id="loanTxDetailBody"></div></div>`;
  document.body.appendChild(modal);
  modal.querySelectorAll("[data-loan-tx-close]").forEach(el => el.onclick = () => closeModal("loanTransactionDetailModal"));
  return modal;
}
function openLoanTransactionDetail(entryId, personName, direction){
  const tx = loanTransactionDetailData(entryId, personName, direction);
  if (!tx) return;
  const modal = ensureLoanTransactionDetailModal();
  ...
  const recordedText = recorded && !Number.isNaN(recorded.getTime())
    ...
    : "Exact recorded time unavailable on legacy record";
  const dirLabel = tx.direction === "given" ? "Loan given" : "Loan taken";
  modal.querySelector("#loanTxDetailTitle").textContent = tx.personName || "Loan transaction";
  modal.querySelector("#loanTxDetailMeta").textContent = `${dirLabel} · Recorded ${recordedText}`;
  modal.querySelector("#loanTxDetailBody").innerHTML = `
    <div class="loan-tx-hero"><div class="loan-tx-icon"><i class="fa-solid fa-coins"></i></div><div><small>${escapeHtml(tx.type || "Transaction")}</small><strong>${money(tx.amount, tx.currency)}</strong><span>${escapeHtml(dirLabel)}</span></div></div>
    <div class="loan-tx-detail-grid">
      <div><small>Counterpart</small><strong><i class="fa-solid fa-user"></i> ${escapeHtml(tx.personName || "—")}</strong></div>
      ...
      <div><small>Remaining after</small><strong>${tx.remainingAfter == null ? "—" : money(tx.remainingAfter, tx.currency)}</strong></div>
    </div>
    <div class="loan-tx-note"><small>Notes</small><p>${escapeHtml(tx.note || "No additional notes")}</p></div>
    <div class="loan-tx-actions">
      ${teamCanShowEdit("entries") ? `<button class="btn ghost" type="button" data-loan-tx-edit="${escapeHtml(tx.id)}"><i class="fa-solid fa-pen"></i> Edit</button>` : ""}
      <button class="btn ghost" type="button" data-loan-tx-pdf="${escapeHtml(tx.id)}"><i class="fa-solid fa-file-pdf"></i> PDF</button>
      ${teamCanShowDelete("entries") ? `<button class="btn danger" type="button" data-loan-tx-delete="${escapeHtml(tx.id)}"><i class="fa-solid fa-trash"></i> Delete</button>` : ""}
    </div>`;
  modal.querySelector("[data-loan-tx-edit]")?.addEventListener("click", e => { closeModal("loanTransactionDetailModal"); openEditModal(e.currentTarget.dataset.loanTxEdit); });
  modal.querySelector("[data-loan-tx-pdf]")?.addEventListener("click", async e => { if(typeof downloadLoanTransactionsSelectionPDF === "function") await downloadLoanTransactionsSelectionPDF(encodeURIComponent(tx.personName || ""), tx.direction, [e.currentTarget.dataset.loanTxPdf]); });
  modal.querySelector("[data-loan-tx-delete]")?.addEventListener("click", e => { closeModal("loanTransactionDetailModal"); deleteEntry(e.currentTarget.dataset.loanTxDelete); });
  modal.classList.remove("hide"); modal.setAttribute("aria-hidden", "false"); document.body.style.overflow = "hidden";
}
window.openLoanTransactionDetail = openLoanTransactionDetail;

if (!document.documentElement.dataset.loanTxPortalBound) {
  document.documentElement.dataset.loanTxPortalBound = "1";
  document.addEventListener("click", async e => {
    const portal = e.target.closest?.(".loan-tx-menu.is-portal");
    if (portal) {
      const edit = e.target.closest("[data-loan-tx-edit-menu]");
      const pdf = e.target.closest("[data-loan-tx-pdf-menu]");
      const del = e.target.closest("[data-loan-tx-delete-menu]");
      if (edit) { e.preventDefault(); e.stopPropagation(); const id=edit.dataset.loanTxEditMenu; closeLoanTransactionMenus(); openEditModal(id); return; }
      if (pdf) { e.preventDefault(); e.stopPropagation(); const id=pdf.dataset.loanTxPdfMenu, person=pdf.dataset.person, dir=pdf.dataset.direction; closeLoanTransactionMenus(); if(typeof downloadLoanTransactionsSelectionPDF === "function") await downloadLoanTransactionsSelectionPDF(person, dir, [id]); return; }
      if (del) { e.preventDefault(); e.stopPropagation(); const id=del.dataset.loanTxDeleteMenu; closeLoanTransactionMenus(); deleteEntry(id); return; }
      return;
    }
    if (!e.target.closest?.(".loan-tx-action-wrap")) closeLoanTransactionMenus();
  });
  document.addEventListener("keydown", e => { if (e.key === "Escape") closeLoanTransactionMenus(); });
  window.addEventListener("resize", closeLoanTransactionMenus, { passive: true });
  window.addEventListener("scroll", closeLoanTransactionMenus, { passive: true, capture: true });
}
...
function renderLoanCards(container, direction, searchKey = direction, options = {}){
              ...
              <div class="card-action-grid loan-inline-actions${hasUnsynced ? " card-action-grid--triple" : ""}" role="group" aria-label="Loan actions">
                ...
                <div class="menu-wrap">
                  ...
                  <div class="menu-dropdown" data-person-menu-panel="${escapeHtml(group.primaryGroupId || group.person_name || "menu")}">
                    <button class="menu-item personActionBtn" type="button" data-action="pdf" data-person="${encodeURIComponent(group.person_name || "")}" data-direction="${escapeHtml(direction)}"><i class="fa-solid fa-file-pdf"></i> Download PDF</button>
        ...
        <div class="detail">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${group.rows.map(row => `
                  <tr class="loan-transaction-row" data-loan-tx-id="${escapeHtml(row.entryId)}" data-person="${encodeURIComponent(group.person_name || "")}" data-direction="${escapeHtml(direction)}" tabindex="0" role="button" aria-label="Open loan transaction details">
                    ...
                    <td>
                      <div class="loan-tx-action-wrap">
                        ${typeof offlineSyncButtonHtml === "function" ? offlineSyncButtonHtml(row.entryId) : ""}
                        <button type="button" class="icon-btn ghost loan-tx-menu-btn" aria-haspopup="menu" aria-expanded="false" title="Transaction actions"><i class="fa-solid fa-ellipsis-vertical"></i></button>
                        <div class="loan-tx-menu" role="menu">
                          ${teamCanShowEdit("entries") ? `<button type="button" role="menuitem" data-loan-tx-edit-menu="${escapeHtml(row.entryId)}"><i class="fa-solid fa-pen"></i><span>Edit</span></button>` : ""}
                          <button type="button" role="menuitem" data-loan-tx-pdf-menu="${escapeHtml(row.entryId)}" data-person="${encodeURIComponent(group.person_name || "")}" data-direction="${escapeHtml(direction)}"><i class="fa-solid fa-file-pdf"></i><span>Download PDF</span></button>
                          ${teamCanShowDelete("entries") ? `<button type="button" class="is-danger" role="menuitem" data-loan-tx-delete-menu="${escapeHtml(row.entryId)}"><i class="fa-solid fa-trash"></i><span>Delete</span></button>` : ""}
                        </div>
                      </div>
                    </td>
                  </tr>
  ...
  container.querySelectorAll(".loan-tx-menu-btn").forEach(btn => btn.addEventListener("click", e => {
    e.preventDefault(); e.stopPropagation();
    const panel = btn.parentElement?.querySelector(".loan-tx-menu");
    if (panel) openLoanTransactionMenu(btn, panel);
  }));
  container.querySelectorAll(".loan-transaction-row").forEach(row => {
    const open = e => {
      if (e?.target?.closest?.("button,a,input,label,[role=menu]")) return;
      openLoanTransactionDetail(row.dataset.loanTxId, row.dataset.person, row.dataset.direction);
    };
  ...
  container.querySelectorAll("[data-person-menu]").forEach(btn => btn.addEventListener("click", e => {
    ...
    const panel = btn.__triplemLoanPersonMenuPanel || container.querySelector(`[data-person-menu-panel="${key}"]`);
    if (!panel) return;
    btn.__triplemLoanPersonMenuPanel = panel;
    ...
    // Position the dropdown using fixed positioning (flip up near screen bottom)
    if (nowOpen) {
      panel.style.visibility = "hidden";
...
function renderGoodsList(){
            ...
            <div class="lt-action">
              <div class="menu-wrap">
                ...
                <div class="menu-dropdown" data-goods-menu-panel="${escapeHtml(group.group_id)}">
                  <button class="menu-item goodsActionBtn" type="button" data-action="pdf" data-group-id="${escapeHtml(group.group_id)}"><i class="fa-solid fa-file-pdf"></i> Download PDF</button>
  ...
  els.goodsList.querySelectorAll("[data-goods-menu]").forEach(btn => btn.addEventListener("click", e => {
    ...
    if (nowOpen) {
      ...
      panel.classList.add("open");
      if (typeof positionFixedMenuDropdown === "function") {
        positionFixedMenuDropdown(panel, btn, { minWidth: 200 });
      } else {
        const rect = btn.getBoundingClientRect();
        panel.style.top = `${rect.bottom + 6}px`;
        panel.style.left = `${rect.right - panel.offsetWidth}px`;
...
function renderInventoryDraftsSection(){
  ...
  root.querySelectorAll(".inventoryDraftDeleteBtn").forEach(btn => {
    btn.addEventListener("click", async () => {
      if (!(await appConfirmDelete("Delete this draft invoice?", { title: "Delete draft invoice?", confirmLabel: "Delete draft" }))) return;
