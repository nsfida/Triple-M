...
const EXPENSE_META_TAG_KEYS = "ATYPE|ETYPE|ITEM|XTYPE|BADDR|BNET|CLOGO|VATP|VATR|VATM|VATA|NET|GROSS|WSORT|XDET|ADET";

function normalizeExpenseDetails(value){
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const out = {};
  for (const [key, raw] of Object.entries(source)) {
    if (!/^[a-zA-Z][a-zA-Z0-9_]{0,47}$/.test(String(key || ""))) continue;
...
function encodeExpenseDetailsTag(value){
  const details = normalizeExpenseDetails(value);
  if (!Object.keys(details).length) return "";
  try { return encodeURIComponent(JSON.stringify(details)); } catch (_) { return ""; }
}

function decodeExpenseDetailsTag(value){
  const text = String(value || "").trim();
  if (!text) return {};
  try { return normalizeExpenseDetails(JSON.parse(decodeURIComponent(text))); } catch (_) { return {}; }
}
...
function expenseMetaFromNotes(noteValue){
  ...
  return {
    ...
    netAmount: readNum("NET"),
    grossAmount: readNum("GROSS"),
    walletSortOrder: readNum("WSORT"),
    details: decodeExpenseDetailsTag(readText("XDET")),
    accountDetails: decodeExpenseDetailsTag(readText("ADET"))
  };
}
...
function upsertExpenseMetaInNote(noteValue, meta = {}){
  ...
  const merged = {
    ...
    walletSortOrder: pick("walletSortOrder"),
    details: normalizeExpenseDetails(pick("details")),
    accountDetails: normalizeExpenseDetails(pick("accountDetails"))
  };
  ...
  if (merged.walletSortOrder != null && Number.isFinite(Number(merged.walletSortOrder))) {
    tags.push(`[WSORT:${Math.round(Number(merged.walletSortOrder))}]`);
  }
  const encodedDetails = encodeExpenseDetailsTag(merged.details);
  if (encodedDetails) tags.push(`[XDET:${encodedDetails}]`);
  const encodedAccountDetails = encodeExpenseDetailsTag(merged.accountDetails);
  if (encodedAccountDetails) tags.push(`[ADET:${encodedAccountDetails}]`);
...
function defaultInventorySellBy({ categorySlug = "", categoryName = "", qtyPattern = "" } = {}){
  ...
  if (typeof getCategoryConfig === "function") {
    const cfg = getCategoryConfig(categoryName || categorySlug);
    if (cfg?.defaultSellBy === "volume" || cfg?.defaultSellBy === "bottle") return cfg.defaultSellBy;
  }
...
async function setInventorySubView(view = "stock"){
  const allowed = { customers: 1, drafts: 1, barcodes: 1, scanner: 1 };
  const next = allowed[view] ? view : "stock";
  // Leaving scanner view always stops the camera
  if (state.inventoryView === "scanner" && next !== "scanner" && typeof closeInventoryBarcodeScanner === "function") {
    try { closeInventoryBarcodeScanner(); } catch (_) {}
  }
  state.inventoryView = next;
  const showCustomers = next === "customers";
  const showDrafts = next === "drafts";
  const showBarcodes = next === "barcodes";
  const showScanner = next === "scanner";
  els.inventoryStockView?.classList.toggle("hide", showCustomers || showDrafts || showBarcodes || showScanner);
  els.inventoryDraftsView?.classList.toggle("hide", !showDrafts);
  els.inventoryCustomersView?.classList.toggle("hide", !showCustomers);
  els.inventoryBarcodesView?.classList.toggle("hide", !showBarcodes);
  (els.inventoryScannerView || document.getElementById("inventoryScannerView"))?.classList.toggle("hide", !showScanner);
  if (els.inventorySectionDesc) {
    els.inventorySectionDesc.textContent = showCustomers
      ...
      : showDrafts
        ...
        : showBarcodes
          ? "Product barcodes for every stock item — print compact price stickers."
          : showScanner
            ? "Continuous barcode scanning into cart. Offline-safe with automatic sync."
  ...
  if (showCustomers) {
    ...
    if (typeof renderInventoryBarcodesSection === "function") {
      await renderInventoryBarcodesSection();
    }
  } else if (showScanner) {
    if (typeof renderInventoryScannerSection === "function") {
      await renderInventoryScannerSection();
    } else if (typeof openInventoryBarcodeScanner === "function") {
              ...
              <tr>
                ...
                <td>
                  <div class="inventory-history-actions">
                    <button class="tiny inventoryCustomerInvoicePdfBtn" type="button" data-entry-id="${escapeHtml(invoice.entryId)}" title="Download invoice PDF"><i class="fa-solid fa-file-pdf"></i></button>
                    <button class="tiny soft inventoryThermalPrintBtn" type="button" data-entry-id="${escapeHtml(invoice.entryId)}" title="Thermal merchant receipt"><i class="fa-solid fa-receipt"></i></button>
                  ...
                  <div class="inventory-history-actions">
                    ${row.action === "invoice"
                      ? `<button class="tiny inventoryCustomerInvoicePdfBtn" type="button" data-entry-id="${escapeHtml(row.entryId)}" title="Invoice PDF"><i class="fa-solid fa-file-invoice"></i></button>
                         <button class="tiny soft inventoryThermalPrintBtn" type="button" data-entry-id="${escapeHtml(row.entryId)}" title="Thermal merchant receipt"><i class="fa-solid fa-receipt"></i></button>
  ...
  if (!modal) {
    ...
    modal.querySelector("#inventoryBrandsList")?.addEventListener("click", async e => {
      ...
      try {
        ...
        if (delBrand) {
          if (!(await appConfirmDelete("Delete this brand and its variants?", { title: "Delete brand?", confirmLabel: "Delete brand" }))) return;
        ...
        if (delVar) {
          if (!(await appConfirmDelete("Delete this variant?", { title: "Delete variant?", confirmLabel: "Delete variant" }))) return;
...
function inventoryPurchaseLineSeedPrefill(){
  ...
  let itemCategory = seedCategory || INVENTORY_CATEGORY_COUNT;
  if ((!seedCategory || seedCategory === INVENTORY_CATEGORY_COUNT) && seedType && typeof getCategoryConfig === "function") {
    const pattern = normalizeInventoryCategory(getCategoryConfig(seedType)?.qtyPattern || "");
    if (pattern) itemCategory = pattern;
  }
  return {
  ...
  for (const account of accounts){
    ...
    for (const row of account.spends){
      ...
      out.push({
        ...
        toWallet: p.toWallet,
        fromGroupId: account.group_id,
        toGroupId: toAcc?.group_id || partner?.group_id || "",
  ...
  for (const ev of events){
    if (ev.curOut === cur){
      rows.push({
        ...
        walletGroupId: ev.fromGroupId || "",
        counterpartyGroupId: ev.toGroupId || "",
    ...
    if (ev.curIn === cur){
      rows.push({
        ...
        walletGroupId: ev.toGroupId || "",
        counterpartyGroupId: ev.fromGroupId || "",
