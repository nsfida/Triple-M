  ...
  return `
    <article class="company-team-member" data-member-id="${escapeHtml(String(member.id))}">
      ...
      <div class="company-team-member-actions">
        <button type="button" class="btn soft tiny" data-team-action="edit-permissions">Edit member</button>
  ...
  list.querySelectorAll(".company-team-member").forEach(card => {
    ...
    if (removeBtn) {
      removeBtn.onclick = async () => {
        if (!(await appConfirmDelete(`Remove team member "${member.username}"? They will lose access, but shared company data stays intact.`, { title: "Remove team member?", confirmLabel: "Remove member", note: "Shared company records remain intact; only this member’s access is removed." }))) return;
...
function openCompanyTeamMemberPermissionsEditor(card, member){
  ...
  if (eyeBtn && pwInput) {
    eyeBtn.onclick = () => {
      ...
      if (show && storedPw && !pwInput.value) {
        pwInput.value = storedPw;
        pwInput.dataset.revealedStored = "1";
      }
  ...
  editor.querySelector("[data-perm-save]").onclick = async () => {
    ...
    try {
      ...
      let password = editor.querySelector('[data-team-edit="password"]')?.value || "";
      // If they only revealed the stored password without changing it, treat as keep.
      if (password && pwInput?.dataset.revealedStored === "1" && password === storedPw) {
        password = "";
      }
      if (!username) throw new Error("Username is required.");
      if (password) {
      ...
      await supabaseRpc("app_team_update_member", {
        p_user_id: member.id,
        p_username: username,
