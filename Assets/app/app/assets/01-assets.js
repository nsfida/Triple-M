...
function assetCurrencyOptionsHtml(selected) {
  const registryItems = window.TriplemCurrencyRegistry?.all?.() || [];
  const list = registryItems.length
    ? registryItems
    : (Array.isArray(SUPPORTED_CURRENCIES) ? SUPPORTED_CURRENCIES : []).map(code => ({
        code,
        symbol: typeof currencySymbol === "function" ? currencySymbol(code) : code
      }));
  const fallback = String(window.TriplemCurrencyRegistry?.registry?.defaultCurrency || "AED").toUpperCase();
  const requested = String(selected || fallback).toUpperCase();
  const cur = list.some(item => String(item?.code || "").toUpperCase() === requested) ? requested : fallback;
  return list.map(item => {
    const code = String(item?.code || "").toUpperCase();
    const symbol = String(item?.symbol ?? (typeof currencySymbol === "function" ? currencySymbol(code) : ""));
    const cls = window.TriplemCurrencyRegistry?.fontClass?.(code) || (typeof currencyFontClass === "function" ? currencyFontClass(code) : "currency-font-normal");
    const label = `${symbol ? symbol + " " : ""}${code}`;
    return `<option value="${escapeHtml(code)}" data-currency="${escapeHtml(code)}" class="${escapeHtml(cls)}" ${code === cur ? "selected" : ""}>${escapeHtml(label)}</option>`;
  }).join("");
}

function assetPurchaseWalletAccounts(currency) {
  const cur = String(currency || "AED").toUpperCase();
  if (typeof getExpenseAccounts !== "function") return [];
  try {
    return getExpenseAccounts({ applyUiFilters: false })
      .filter(account => String(account?.currency || "").toUpperCase() === cur && String(account?.currency || "").toUpperCase() !== "BTC")
...
function populateAssetPurchaseWalletSelect(options = {}) {
  const select = document.getElementById("assetFormWallet");
  const help = document.getElementById("assetFormWalletHelp");
  if (!select) return;
  const currency = String(document.getElementById("assetFormCurrency")?.value || "AED").toUpperCase();
  const previous = options.preserveSelection === false ? "" : String(select.value || "");
  const accounts = assetPurchaseWalletAccounts(currency);
  const rows = [`<option value="">Skip wallet deduction</option>`];
  const currencyClass = window.TriplemCurrencyRegistry?.fontClass?.(currency) || (typeof currencyFontClass === "function" ? currencyFontClass(currency) : "currency-font-normal");
  const currencyLabel = window.TriplemCurrencyRegistry?.optionLabel?.(currency) || currency;
  accounts.forEach(account => {
    const id = escapeHtml(String(account.group_id || ""));
    const name = escapeHtml(String(account.person_name || "Wallet"));
    const balance = typeof formatReportAmount === "function"
      ? escapeHtml(String(formatReportAmount(Number(account.balance || 0), currency)))
      : escapeHtml(`${typeof currencySymbol === "function" ? currencySymbol(currency) : currency} ${Number(account.balance || 0).toLocaleString()}`);
    rows.push(`<option value="${id}" data-currency="${escapeHtml(currency)}" class="${escapeHtml(currencyClass)}">${name} · ${balance}</option>`);
  });
  if (!accounts.length) rows.push(`<option value="" data-currency="${escapeHtml(currency)}" class="${escapeHtml(currencyClass)}" disabled>No ${escapeHtml(currencyLabel)} wallets available</option>`);
  select.innerHTML = rows.join("");
  if (previous && accounts.some(account => String(account.group_id || "") === previous)) select.value = previous;
  if (help) {
    help.textContent = accounts.length
      ? `Select a ${currency} wallet to deduct the purchase price, or leave Skip wallet deduction selected.`
      : `No ${currency} wallet is currently available. The asset can still be saved without a wallet deduction.`;
  }
  if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(select);
}

async function refreshAssetPurchaseWallets(options = {}) {
  const select = document.getElementById("assetFormWallet");
  if (!select || assetUi.formMode !== "create") return;
  const currency = String(document.getElementById("assetFormCurrency")?.value || "AED").toUpperCase();
  const previous = options.preserveSelection === false ? "" : String(select.value || "");
  if (options.load !== false && navigator.onLine !== false && typeof loadExpenseWalletSummaries === "function") {
    select.innerHTML = `<option value="">Loading ${escapeHtml(currency)} wallets…</option>`;
    try { await loadExpenseWalletSummaries({ force: false }); } catch (err) { console.warn("Asset wallet list refresh failed:", err); }
  }
  populateAssetPurchaseWalletSelect({ preserveSelection: false });
  if (previous && assetPurchaseWalletAccounts(currency).some(account => String(account.group_id || "") === previous)) select.value = previous;
}

function validateAssetPurchaseWallet(walletGroupId, amount, currency) {
  const walletId = String(walletGroupId || "").trim();
  if (!walletId) return null;
  const account = assetPurchaseWalletAccounts(currency).find(row => String(row.group_id || "") === walletId);
  if (!account) throw new Error("Selected wallet was not found. Reopen the asset form to refresh wallets.");
  if (String(account.currency || "").toUpperCase() !== String(currency || "").toUpperCase()) {
    throw new Error("Selected wallet currency does not match the asset currency.");
  }
  const value = Number(amount || 0);
  if (value > Number(account.balance || 0) + 0.00000001) {
    const available = typeof formatReportAmount === "function"
      ? formatReportAmount(Number(account.balance || 0), account.currency)
      : `${account.currency} ${Number(account.balance || 0).toLocaleString()}`;
    throw new Error(`Insufficient wallet balance. Available: ${available}.`);
  }
  return account;
}

async function createAssetPurchaseWalletEntry(walletGroupId, asset, options = {}) {
  const value = Number(asset?.purchase_price || 0);
  if (!walletGroupId || !(value > 0)) return;
  const account = validateAssetPurchaseWallet(walletGroupId, value, asset?.currency);
  if (!account) return;
  if (typeof saveEntriesImmediately !== "function" || typeof upsertExpenseMetaInNote !== "function") {
    throw new Error("Wallet ledger service is unavailable.");
  }
  const date = String(asset?.purchase_date || new Date().toISOString().slice(0, 10));
  const expenseType = String(options.expenseType || "Asset Purchase");
  const notePrefix = String(options.notePrefix || "Asset purchase");
  const saveLabel = String(options.saveLabel || "Asset purchase wallet deduction");
  const payload = {
    ...
    action_date: date,
    notes: upsertExpenseMetaInNote(`${notePrefix} — ${asset?.name || "Asset"}`, {
      accountType: account.accountType,
      rowType: "EXPENSE",
      itemName: asset?.name || "Asset",
      expenseType
    })
  };
  await saveEntriesImmediately(payload, { label: saveLabel, awaitSync: true });
}
...
function pendingOfflineAssets(){
  if (typeof getOfflineTableRows !== "function") return [];
  return getOfflineTableRows("app_assets").map(row => ({ ...mapAssetRow(row), _syncStatus: "pending" }));
}

function pendingOfflineAssetTransactions(assetId = ""){
  if (typeof getOfflineTableRows !== "function") return [];
  const id = String(assetId || "");
  return getOfflineTableRows("app_asset_transactions")
    .filter(row => !id || String(row.asset_id || "") === id)
    .map(row => ({ ...mapAssetTxRow(row), _syncStatus: "pending" }));
}

function mergeOfflineRowsById(serverRows, pendingRows){
...
async function loadAssetsFromDatabase(options = {}) {
  ...
  if (!canUseAssetsFeature()) {
    state.assets = pendingOfflineAssets();
    state.assetTransactions = pendingOfflineAssetTransactions();
    state.assetTransactionLoadedIds = new Set(state.assets.map(asset => asset.id));
  ...
  try {
    ...
    try {
      ...
      state.assetTransactions = mergeOfflineRowsById(
        filterRowsForCurrentUser(legacyTransactions || []).map(mapAssetTxRow),
        pendingOfflineAssetTransactions()
      );
      state.assetTransactionLoadedIds = new Set(assetRows.map(asset => asset.id));
    }
    state.assets = mergeOfflineRowsById(assetRows, pendingOfflineAssets());
...
async function ensureAssetTransactionsLoaded(assetId, options = {}) {
  ...
  if (root && assetUi.selectedId === id) {
    root.innerHTML = '<div class="asset-detail-loading" role="status" aria-live="polite"><i class="fa-solid fa-spinner btn-loader" aria-hidden="true"></i><span>Loading asset details...</span></div>';
  }
  const usePendingOnly = () => {
    const pending = pendingOfflineAssetTransactions(id);
    const existing = (state.assetTransactions || []).filter(tx => String(tx.asset_id || "") === id);
    const mapped = mergeOfflineRowsById(existing, pending);
    state.assetTransactions = (state.assetTransactions || []).filter(tx => String(tx.asset_id || "") !== id).concat(mapped);
  ...
  if (navigator.onLine === false) return usePendingOnly();
  try {
    const rows = await supabase(
      `app_asset_transactions?select=*&asset_id=eq.${encodeURIComponent(id)}&is_deleted=eq.false${ownerIdQuery()}&order=tx_date.desc`
    );
    const mapped = mergeOfflineRowsById(
      filterRowsForCurrentUser(rows || []).map(mapAssetTxRow),
      pendingOfflineAssetTransactions(id)
    );
    ...
    if (typeof isConnectivityFailure === "function" && isConnectivityFailure(error)) return usePendingOnly();
    throw error;
  }
}
...
function closeAllAssetCardMenus(){
  document.querySelectorAll(".asset-card-dropdown.open").forEach(panel => {
    ...
    panel.style.left = "";
    panel.style.right = "";
    panel.classList.remove("asset-card-dropdown-portal");
    const home = panel.__assetMenuHome;
    if (home?.isConnected) home.appendChild(panel);
    panel.__assetMenuHome = null;
  });
...
function openAssetCardMenu(trigger){
  ...
  const selector = id ? `[data-asset-menu-panel="${CSS.escape(id)}"]` : ".asset-card-dropdown";
  const panel = wrap?.querySelector(selector) || document.querySelector(selector);
  if (!panel || !trigger || !wrap) return;
  const alreadyOpen = panel.classList.contains("open");
  closeAllAssetCardMenus();
  if (alreadyOpen) return;
  panel.__assetMenuHome = wrap;
  document.body.appendChild(panel);
  panel.classList.add("asset-card-dropdown-portal");
  panel.hidden = false;
...
function bindRenderedAssetCardInteractions(root = document.getElementById("assetsList")) {
  if (!root) return;
  const openAssetSafely = id => {
    const assetId = String(id || "");
    if (!assetId) return;
    Promise.resolve(openAssetDetail(assetId)).catch(err => {
      console.error("Could not open asset details:", err);
      try { alert(err?.message || "Could not open this asset."); } catch (_) {}
    });
  };
  ...
  root.querySelectorAll("[data-asset-open]").forEach(btn => {
    btn.onclick = event => {
      ...
      openAssetSafely(btn.getAttribute("data-asset-open"));
    };
  });
  root.querySelectorAll("[data-asset-menu-trigger]").forEach(btn => {
    btn.onclick = event => {
      event.preventDefault();
      event.stopPropagation();
      openAssetCardMenu(btn);
    };
  });
  root.querySelectorAll("[data-asset-card-action]").forEach(btn => {
    btn.onclick = event => {
      event.preventDefault();
      event.stopPropagation();
      handleAssetCardAction(btn);
    };
  });
  root.querySelectorAll(".asset-card[data-asset-id]").forEach(card => {
    card.onclick = event => {
      if (event.defaultPrevented) return;
      if (event.target.closest("button,a,input,select,textarea,label,.menu-wrap,.menu-dropdown")) return;
      openAssetSafely(card.getAttribute("data-asset-id"));
    };
    card.tabIndex = 0;
    card.setAttribute("role", "button");
    card.setAttribute("aria-label", `Open ${card.querySelector(".asset-card-title")?.textContent?.trim() || "asset"}`);
...
function renderAssetsList() {
  const listEl = document.getElementById("assetsList");
  if (!listEl) return;
  const hasPendingLocalAssets = (state.assets || []).some(asset => asset?._syncStatus === "pending" || (asset?.id && typeof hasPendingOfflineEntity === "function" && hasPendingOfflineEntity(asset.id)));
  if (!canUseAssetsFeature() && !hasPendingLocalAssets) {
  ...
  bindRenderedAssetCardInteractions(listEl);
}
...
function fillAssetForm(asset) {
  ...
  const currencySelect = document.getElementById("assetFormCurrency");
  if (currencySelect) {
    currencySelect.innerHTML = assetCurrencyOptionsHtml(asset?.currency || "AED");
    currencySelect.value = asset?.currency || "AED";
    if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(currencySelect);
  }
  ...
  const walletWrap = document.getElementById("assetFormWalletWrap");
  const walletSelect = document.getElementById("assetFormWallet");
  if (walletWrap) walletWrap.classList.toggle("hide", isEdit);
  if (walletSelect) walletSelect.innerHTML = `<option value="">Skip wallet deduction</option>`;
  if (!isEdit) populateAssetPurchaseWalletSelect({ preserveSelection: false });
...
async function openAssetFormModal(assetId) {
  ...
  if (!asset) await refreshAssetPurchaseWallets({ load: true, preserveSelection: true });
}
...
async function saveAssetForm() {
  ...
  if (!typeResolved.ok) { alert(typeResolved.error || "Select a valid asset type."); return; }
  ...
  const creating = !(assetUi.formMode === "edit" && assetUi.editingAssetId);
  const purchaseWalletId = creating ? String(document.getElementById("assetFormWallet")?.value || "").trim() : "";
  if (purchaseWalletId) {
    try { validateAssetPurchaseWallet(purchaseWalletId, purchase_price, currency); }
  ...
  const keepLocal = () => {
    if (!creating || typeof queueOfflineTableInsert !== "function" || !queueOfflineTableInsert("app_assets", payload, "Asset", { entityIds: [payload.id] })) return false;
    const local = { ...mapAssetRow(payload), _syncStatus: "pending" };
    state.assets = mergeOfflineRowsById(state.assets, [local]);
    state.assetsLoaded = true;
  ...
  try {
    if (creating && navigator.onLine === false) {
      if (!keepLocal()) throw new Error("The asset could not be stored locally.");
      if (purchaseWalletId) await createAssetPurchaseWalletEntry(purchaseWalletId, payload);
      if (asset_type === "other" && asset_type_other) rememberAssetCustomType(asset_type_other);
      closeModal("assetFormModal");
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset “${name}” saved locally. Use Save to sync when internet returns.`, "success");
      return;
    }
    if (!creating) {
      ...
      if (purchaseWalletId) {
        try {
          await createAssetPurchaseWalletEntry(purchaseWalletId, payload);
        } catch (walletErr) {
          console.error("Asset saved but purchase wallet deduction failed.", walletErr);
          alert(`Asset was saved, but the wallet deduction failed: ${walletErr?.message || walletErr}. Please add the matching wallet expense manually if required.`);
        }
      }
    }
    if (asset_type === "other" && asset_type_other) rememberAssetCustomType(asset_type_other);
    ...
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`${creating ? "Asset created" : "Asset updated"}: ${name}.`, "success");
  } catch (err) {
    if (creating && typeof isConnectivityFailure === "function" && isConnectivityFailure(err) && keepLocal()) {
      if (purchaseWalletId) {
        try { await createAssetPurchaseWalletEntry(purchaseWalletId, payload); } catch (walletErr) { console.warn("Offline asset wallet deduction queue failed:", walletErr); }
      }
      if (asset_type === "other" && asset_type_other) rememberAssetCustomType(asset_type_other);
      closeModal("assetFormModal");
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset “${name}” saved locally. Use Save to sync when internet returns.`, "success");
    ...
    console.error(err);
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset failed: ${err.message || err}`, "error");
...
function renderAssetDetail(assetId) {
  ...
  const assetTitleEl = document.getElementById("assetDetailTitle");
  const assetAiOrigin = (typeof isTriplemAiOrigin === "function" && isTriplemAiOrigin("assets", asset?.id)) || asset?.meta?.created_by_ai === true || String(asset?.meta?.created_by_ai || "").toLowerCase() === "true";
  if (assetTitleEl) assetTitleEl.innerHTML = `${escapeHtml(asset.name)} ${assetAiOrigin && typeof aiOriginBadgeHtml === "function" ? aiOriginBadgeHtml("AI") : ""}`;
      ...
      <div class="asset-detail-actions">
        <div class="menu-wrap asset-detail-reports-menu">
          <button type="button" class="btn ghost tiny asset-detail-reports-btn" id="assetDetailReportsBtn" title="Download reports" aria-haspopup="menu" aria-expanded="false">
            <i class="fa-solid fa-file-pdf" aria-hidden="true"></i> Reports
          </button>
...
function assetChartBaseOptions({ text, muted, legend = true, maxTicksX } = {}) {
  ...
  const chartTheme = typeof sectionDetailsThemeColors === "function" ? sectionDetailsThemeColors() : null;
  const line = chartTheme?.grid || getComputedStyle(document.documentElement).getPropertyValue("--chart-grid").trim() || getComputedStyle(document.documentElement).getPropertyValue("--line").trim();
...
function paintAssetCharts(asset, txs, reportOpts) {
  ...
  createAssetChart(document.getElementById("assetChartExpenseMix"), {
    ...
    data: {
      ...
      datasets: [{
        data: mixValues,
        backgroundColor: [warning, danger, accentPalette[1] || primary, accentPalette[4] || muted, primary, accentPalette[2] || warning, themed?.black || accentPalette[4] || themed?.text],
        borderColor: themed?.doughnutBorder || getComputedStyle(document.documentElement).getPropertyValue("--surface").trim(),
        borderWidth: 2
      }]
    },
    options: {
...
async function saveAssetTxForm() {
  ...
  if (!resolved.ok) { alert(resolved.error || "Select a valid transaction type."); return; }
  ...
  const existing = assetUi.editingTxId ? getAssetTransactions(assetId).find(t => t.id === assetUi.editingTxId) : null;
  const meta = { ...(existing?.meta && typeof existing.meta === "object" ? existing.meta : {}) };
  if (tx_type === "other_expense" && tx_label) meta.tx_label = tx_label; else delete meta.tx_label;
  const payload = {
    asset_id: assetId,
    owner_id: currentOwnerId(),
    tx_type,
  ...
  const creating = !assetUi.editingTxId;
  if (creating) {
    payload.id = crypto.randomUUID();
    payload.is_deleted = false;
    payload.created_at = new Date().toISOString();
  ...
  const keepLocal = () => {
    if (!creating || typeof queueOfflineTableInsert !== "function" || !queueOfflineTableInsert("app_asset_transactions", payload, "Asset transaction", { entityIds: [payload.id] })) return false;
    const local = { ...mapAssetTxRow(payload), _syncStatus: "pending" };
    state.assetTransactions = mergeOfflineRowsById(state.assetTransactions, [local]);
    if (!(state.assetTransactionLoadedIds instanceof Set)) state.assetTransactionLoadedIds = new Set();
    state.assetTransactionLoadedIds.add(assetId);
    renderAssetsList();
    if (assetUi.selectedId === assetId) renderAssetDetail(assetId);
    return true;
  };
  ...
  try {
    if (creating && navigator.onLine === false) {
      if (!keepLocal()) throw new Error("The asset transaction could not be stored locally.");
      if (tx_type === "other_expense" && tx_label) rememberAssetCustomExpenseLabel(tx_label);
      closeModal("assetTxModal");
      if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction saved locally. Use Save to sync when internet returns.`, "success");
      return;
    }
    if (!creating) {
      await supabase(`app_asset_transactions?id=eq.${encodeURIComponent(assetUi.editingTxId)}${ownerIdQuery()}`, { method: "PATCH", body: JSON.stringify(payload) });
    } else {
    ...
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction ${creating ? "created" : "updated"} successfully.`, "success");
  } catch (err) {
    if (creating && typeof isConnectivityFailure === "function" && isConnectivityFailure(err) && keepLocal()) {
      if (tx_type === "other_expense" && tx_label) rememberAssetCustomExpenseLabel(tx_label);
      closeModal("assetTxModal");
    ...
    console.error(err);
    // Backward-compatible fallback if tx_label column is not migrated yet.
    const msg = String(err?.message || err || "");
    if (/tx_label|PGRST204|Could not find/i.test(msg) && payload.tx_label != null) {
      try {
        const fallback = { ...payload };
        delete fallback.tx_label;
        if (!creating) await supabase(`app_asset_transactions?id=eq.${encodeURIComponent(assetUi.editingTxId)}${ownerIdQuery()}`, { method: "PATCH", body: JSON.stringify(fallback) });
        else await supabase("app_asset_transactions", { method: "POST", body: JSON.stringify(fallback) });
        if (tx_type === "other_expense" && tx_label) rememberAssetCustomExpenseLabel(tx_label);
        invalidateAssetTransactions(assetId);
        closeModal("assetTxModal");
        await loadAssetsFromDatabase({ force: true });
        ...
        if (creating && typeof isConnectivityFailure === "function" && isConnectivityFailure(err2) && keepLocal()) {
          closeModal("assetTxModal");
          if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction saved locally. Use Save to sync when internet returns.`, "success");
          return;
        }
        console.error(err2);
        if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction failed: ${err2.message || err2}`, "error");
        alert("Failed to save transaction: " + (err2.message || err2));
        return;
      }
    }
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction failed: ${err.message || err}`, "error");
...
async function deleteAssetTransaction(txId) {
  ...
  const tx = (state.assetTransactions || []).find(row => String(row.id) === String(txId));
  ...
  try {
    ...
    const assetId = assetUi.selectedId || tx?.asset_id || null;
    if (assetId) invalidateAssetTransactions(assetId);
    if (typeof invalidateDashboardSummary === "function") invalidateDashboardSummary({ refreshIfVisible: true });
    await loadAssetsFromDatabase({ force: true });
    if (assetId) await openAssetDetail(assetId);
    if (typeof showEntryConfirmation === "function") {
      const amountText = tx ? `${money(Number(tx.amount || 0), getAssetById(tx.asset_id)?.currency || "AED")}` : "Asset transaction";
      showEntryConfirmation(`${amountText} transaction deleted successfully.`, "success");
    }
  } catch (err) {
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset transaction delete failed: ${err?.message || err}`, "error");
...
async function saveAssetSaleForm() {
  ...
  try {
    ...
    await loadAssetsFromDatabase({ force: true });
    if (typeof showEntryConfirmation === "function") {
      showEntryConfirmation(`Asset updated: ${asset.name} is now ${status}.`, "success");
    }
  } catch (err) {
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset update failed: ${err?.message || err}`, "error");
...
async function deleteAsset(assetId) {
  ...
  try {
    ...
    closeModal("assetDetailModal");
    await loadAssetsFromDatabase({ force: true });
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset deleted: ${asset.name}.`, "success");
  } catch (err) {
    if (typeof showEntryConfirmation === "function") showEntryConfirmation(`Asset delete failed: ${err?.message || err}`, "error");
  ...
  createSectionDetailsChart(document.getElementById("sectionDetailsChart2"), {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: [colors.black || colors.palette?.[4] || colors.text, colors.danger, colors.sky, colors.success, t.net >= 0 ? colors.success : colors.danger],
        borderRadius: 8
      }]
    },
    options: {
...
function handleAssetCardAction(actionBtn) {
  ...
  else if (action === "add-tx") { assetUi.selectedId = assetId; openAssetTxModal(null); }
  else if (action === "edit") openAssetFormModal(assetId);
  else if (action === "sell") { assetUi.selectedId = assetId; openAssetSaleModal(); }
  else if (action === "pdf-full" && typeof downloadSingleAssetPDF === "function") downloadSingleAssetPDF(assetId).catch(err => alert(err?.message || err));
  else if (action === "pdf-revenue" && typeof downloadSingleAssetRevenuePDF === "function") downloadSingleAssetRevenuePDF(assetId).catch(err => alert(err?.message || err));
  else if (action === "pdf-expenses" && typeof downloadSingleAssetExpensesPDF === "function") downloadSingleAssetExpensesPDF(assetId).catch(err => alert(err?.message || err));
  else if (action === "delete") deleteAsset(assetId);
}

function assetsBindUI() {
  if (assetUi.bound) return;
  assetUi.bound = true;
  ...
  document.getElementById("assetFormCurrency")?.addEventListener("change", () => {
    if (assetUi.formMode === "create") refreshAssetPurchaseWallets({ load: false, preserveSelection: false });
  });
  ...
  document.getElementById("assetStatusFilter")?.addEventListener("change", e => {
  ...
  document.getElementById("assetsList")?.addEventListener("click", e => {
    ...
    if (actionBtn) { e.preventDefault(); e.stopPropagation(); handleAssetCardAction(actionBtn); return; }
  ...
  document.addEventListener("click", e => {
    const portalAction = e.target.closest(".asset-card-dropdown-portal [data-asset-card-action]");
    if (portalAction) { e.preventDefault(); e.stopPropagation(); handleAssetCardAction(portalAction); return; }
    if (!e.target.closest(".asset-card-menu,.asset-card-dropdown")) closeAllAssetCardMenus();
  });
  ...
  if (curSel) {
    curSel.innerHTML = assetCurrencyOptionsHtml(curSel.value || "AED");
    if (typeof syncCurrencySelectFonts === "function") syncCurrencySelectFonts(curSel);
  }
  populateAssetPurchaseWalletSelect({ preserveSelection: false });
}

if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", assetsBindUI, { once: true });
else assetsBindUI();
