/* Triplem VIP Live Chat semantic AI — local neural intent understanding · Build 157.
 *
...
(() => {
  ...
  const INTENT_EXAMPLES = [
    ...
    ["accounting", "What is the Accounting ERP and double-entry bookkeeping workspace?"],
    ["accounting", "How does Triplem VIP Accounting work?"],
    ["chart_accounts", "How does the Chart of Accounts work?"],
    ["journals", "How do journal entries, debits and credits work?"],
    ["receivables_payables", "How do accounts receivable, accounts payable, customer receipts and supplier payments work?"],
    ["bank_reconciliation", "Can I import a bank statement and reconcile transactions?"],
    ["tax_accounting", "Does Accounting support VAT and corporate tax estimates?"],
    ["accounting_reports", "Does Accounting have Trial Balance, Profit and Loss, Balance Sheet, Cash Flow and General Ledger reports?"],
    ["accounting_controls", "What are accounting audit trail, dimensions and Books Lock Date?"],
  ...
  function lexicalBoosts(question) {
    ...
    if (/\b(accounting|erp|double entry|double-entry|bookkeeping|books)\b/.test(q)) add("accounting", 0.18);
    if (/\b(chart of accounts|coa|general account|ledger account)\b/.test(q)) add("chart_accounts", 0.20);
    if (/\b(journal|journal entry|debit|credit|balanced entry|reversal)\b/.test(q)) add("journals", 0.18);
    if (/\b(receivable|receivables|payable|payables|ar aging|ap aging|customer receipt|supplier payment)\b/.test(q)) add("receivables_payables", 0.20);
    if (/\b(bank reconciliation|reconcile|reconciliation|bank statement|statement import)\b/.test(q)) add("bank_reconciliation", 0.21);
    if (/\b(vat|input vat|output vat|corporate tax|tax code|tax estimate)\b/.test(q)) add("tax_accounting", 0.18);
    if (/\b(trial balance|profit and loss|p&l|balance sheet|cash flow|general ledger|aging report)\b/.test(q)) add("accounting_reports", 0.22);
    if (/\b(books lock|lock date|closed period|audit trail|cost centre|cost center|dimension|project|branch)\b/.test(q)) add("accounting_controls", 0.18);
  ...
  async function analyzeWithBrowserLanguageModel(question, messages) {
    ...
    const billingCurrencies = window.TriplemCurrencyRegistry?.billingCurrencies?.().map(row => row.code) || [];
    const currencyEnum = [...billingCurrencies, "unknown"].join("|");
    const prompt = [
      `Allowed intents: ${allowlist}.`,
      `Recent visitor context: ${context || "none"}.`,
      `Current visitor message: ${clean(question)}.`,
      `Return exactly one JSON object: {"intents":[{"intent":"...","score":0.0}],"question_kind":"explain|howto|pricing|capability|benefit|confirmation|direct","plan_period":"monthly|yearly|unknown","currency":"${currencyEnum}","confidence":0.0}.`,
    ...
    const parsedCurrency = clean(parsed.currency).toUpperCase();
    const currency = billingCurrencies.includes(parsedCurrency) ? parsedCurrency : "unknown";
    return {
