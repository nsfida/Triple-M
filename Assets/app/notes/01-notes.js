...
function wireAdminCredentialControls(root){
  ...
  root.querySelectorAll("[data-toggle-pw]").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const el = root.querySelector(`#${CSS.escape?.(btn.getAttribute("data-toggle-pw")) || btn.getAttribute("data-toggle-pw")}`)
        || document.getElementById(btn.getAttribute("data-toggle-pw"));
      if (!el) return;
      ...
      if (currentlyHidden) {
        ...
        if (typeof setPasswordEyeState === "function") setPasswordEyeState(btn, true);
      } else {
        el.type = "password";
        el.value = raw ? (el.classList.contains("admin-cred-pin") ? "•".repeat(Math.min(6, Math.max(4, raw.length))) : "••••••••") : "";
        el.dataset.showing = "0";
        if (typeof setPasswordEyeState === "function") setPasswordEyeState(btn, false);
      }
    });
  });
}
...
async function openAdminCredentialsOverlay(user){
  ...
  const shell=(body)=>`<div class="modal-backdrop" data-admin-cred-close></div><div class="modal-dialog admin-credentials-dialog" role="dialog" aria-modal="true" aria-labelledby="adminCredTitle"><div class="modal-head admin-credentials-head"><div><p class="admin-credentials-kicker"><i class="fa-solid fa-lock-open" aria-hidden="true"></i> Secured credentials</p><h3 id="adminCredTitle">${escapeHtml(user.display_name||user.username)}</h3><p>@${escapeHtml(user.username)} · protected administrator access</p></div><button type="button" class="btn ghost tiny" data-admin-cred-close aria-label="Close">✕</button></div><div class="modal-body admin-credentials-body">${body}</div></div>`;
  modal.innerHTML=shell(`<div class="empty admin-themed-loading"><i class="fa-solid fa-spinner btn-loader"></i> Decrypting secured credentials…</div>`);
  ...
  try{
    const result=unwrapRpcJson(await supabaseRpc("app_admin_get_user_credentials",{p_user_id:user.id}))||{};
    const viewUser={...user,admin_visible_password:result.password||"",admin_visible_smart_pin:result.smart_pin||""};
    const unavailable=result.password_hash_only && !result.password_available
      ? `<p class="admin-credentials-note admin-credentials-note--info"><i class="fa-solid fa-shield-halved"></i> This account's current password predates the encrypted credential vault and exists only as a one-way hash, so it cannot be reconstructed. It will become securely viewable after the user's next password change or an administrator password reset.</p>` : "";
    modal.innerHTML=shell(`${adminCredentialBlock(viewUser)}${unavailable}<p class="admin-credentials-note"><i class="fa-solid fa-lock"></i> New credentials are stored in the protected encrypted vault and decrypted only on demand for the main protected administrator.</p>`);
    modal.querySelectorAll("[data-admin-cred-close]").forEach(el=>el.onclick=close);
    wireAdminCredentialControls(modal);
  }catch(err){
    modal.innerHTML=shell(`<div class="empty admin-themed-loading">Could not open secured credentials. ${escapeHtml(err.message||String(err))}</div>`);
    modal.querySelectorAll("[data-admin-cred-close]").forEach(el=>el.onclick=close);
  }
}
