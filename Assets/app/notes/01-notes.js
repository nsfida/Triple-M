...
async function renderNotes(searchTerm = '') {
  ...
  filteredNotes.forEach(note => {
    ...
    const preview = String(note.content || "").trim().replace(/\s+/g, " ").slice(0, 280);
    const noteEl = document.createElement('div');
    noteEl.className = `card note-grid-card${hasReminder ? " has-reminder" : ""}`;
    ...
    noteEl.innerHTML = `
      ...
      <span class="note-grid-preview">${escapeHtml(preview || "Empty note")}</span>
      ${hasReminder ? '<span class="note-grid-reminder" aria-hidden="true"><i class="fa-solid fa-bell"></i></span>' : ""}
    `;
...
function renderNoteDetail(noteId){
  ...
  if (metaEl) {
    const aiOrigin = (typeof isTriplemAiOrigin === "function" && isTriplemAiOrigin("notes", note?.id)) || note?.meta?.created_by_ai === true || String(note?.meta?.created_by_ai || "").toLowerCase() === "true";
    if (aiOrigin && typeof aiOriginBadgeHtml === "function") metaEl.innerHTML = `${escapeHtml(meta)} ${aiOriginBadgeHtml("AI")}`;
...
const ADMIN_CURRENCY_OPTIONS = (window.TriplemCurrencyRegistry?.codes?.() || SUPPORTED_CURRENCIES || []).slice();
window.addEventListener("triplem:currency-registry", () => { const next = window.TriplemCurrencyRegistry?.codes?.() || []; if (next.length) ADMIN_CURRENCY_OPTIONS.splice(0, ADMIN_CURRENCY_OPTIONS.length, ...next); });
...
function userAllowedTabs(user){
  const includeExplicitAccountingAccess = (tabs) => {
    const normalized = Array.from(new Set((tabs || []).map(t => String(t).toLowerCase()).filter(Boolean)));
    const accountingViewAllowed = (user?.permissions || []).some(p =>
      String(p?.module || "").toLowerCase() === "accounting"
      && String(p?.action || "").toLowerCase() === "view"
      && p?.allowed === true
    );
    if (accountingViewAllowed && !normalized.includes("accounting")) normalized.push("accounting");
    return normalized;
  };
  const fromField = user?.allowed_tabs;
  if (Array.isArray(fromField) && fromField.length) {
    return includeExplicitAccountingAccess(fromField);
  }
  const fromSettings = user?.settings?.Tabs;
  if (Array.isArray(fromSettings) && fromSettings.length) {
    return includeExplicitAccountingAccess(fromSettings);
  }
  // Derive from permissions view flags when no legacy tab assignment exists.
