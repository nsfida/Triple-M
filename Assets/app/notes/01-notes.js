...
async function saveNote(){
  ...
  try {
    ...
    if (!isGuestMode()) await loadNotesFromDatabase({ force: true });
    form.reset();
    setNewNoteReminderVisibility();
...
window.deleteNote = async function(noteId, options = {}) {
  if (!options.skipConfirm && !confirm('Are you sure you want to delete this note?')) return;
...
function openAdminCredentialsOverlay(user){
  let modal = document.getElementById("adminCredentialsModal");
  if (!modal) {
    modal = document.createElement("div");
    modal.id = "adminCredentialsModal";
    modal.className = "modal hide";
    document.body.appendChild(modal);
  }
  modal.innerHTML = `
    <div class="modal-backdrop" data-admin-cred-close></div>
    <div class="modal-dialog admin-credentials-dialog" role="dialog" aria-modal="true" aria-labelledby="adminCredTitle">
      <div class="modal-head admin-credentials-head">
        <div>
          <p class="admin-credentials-kicker"><i class="fa-solid fa-lock-open" aria-hidden="true"></i> Secured credentials</p>
          <h3 id="adminCredTitle">${escapeHtml(user.display_name || user.username)}</h3>
          <p>@${escapeHtml(user.username)} · reveal only when needed</p>
        </div>
        <button type="button" class="btn ghost tiny" data-admin-cred-close aria-label="Close">✕</button>
      </div>
      <div class="modal-body admin-credentials-body">
        ${adminCredentialBlock(user)}
        <p class="admin-credentials-note">Passwords &amp; Smart Pins are admin-visible secrets. Close this panel when finished.</p>
      </div>
    </div>`;
  const close = () => {
    modal.classList.add("hide");
