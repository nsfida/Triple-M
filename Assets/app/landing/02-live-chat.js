...
(() => {
  ...
  function openPanel() {
    ...
    if (session) {
      ...
      if (mode === "human" || mode === "legacy_human") requestAnimationFrame(() => els.reply?.focus({ preventScroll: true }));
    } else {
      showStartView();
  ...
  function updateReplyState() {
    ...
    const canType = !closed && (mode === "human" || mode === "legacy_human");
    if (els.replyForm) els.replyForm.classList.toggle("hide", !canType);
    if (els.reply) {
      els.reply.disabled = !canType;
      els.reply.placeholder = closed ? "This chat has ended" : canType ? "Write a message to your support Agent…" : "Typing unlocks when an Agent accepts the chat";
    }
  ...
  async function sendReply(event) {
    ...
    if (activeMode !== "human" && activeMode !== "legacy_human") {
