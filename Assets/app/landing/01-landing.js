...
function landingLocalizedMoneyFromText(rawText){
  ...
  const tokens = [...new Set((registry?.all?.() || []).flatMap(currency => [currency.code,currency.symbol,...(currency.aliases || [])]).filter(Boolean))]
    .sort((a,b) => b.length - a.length).map(escapeRegex);
  if (!tokens.length) return "";
  const match = raw.match(new RegExp(`^([+−-]\\s*)?(${tokens.join("|")})\\s*([0-9][0-9,]*(?:\\.[0-9]+)?)(k)?$`, "i"));
  if (!match) return "";
  const prefix = match[1] || "";
  const sourceCurrency = registry?.normalize?.(match[2]) || normalizeCurrencyCode(match[2]);
  if (!registry?.get?.(sourceCurrency)) return "";
  ...
  const maximumFractionDigits = suffixK ? Math.min(2, currencyDecimals(target)) : currencyDecimals(target);
...
function applyLandingRegionalCurrency(){
  ...
  document.querySelectorAll(".pricing-rate-list").forEach(list => {
    const plan = list.closest(".pricing-plan-offer-yearly") ? "yearly" : "monthly";
    const period = plan === "yearly" ? "/ year" : "/ month";
    list.innerHTML = `<li><span class="pricing-rate-code">${escapeHtml(currency)}</span><span class="pricing-rate-amount">${regionalMoneyHtml(publicPlanPrice(plan, currency), currency)}</span><span class="pricing-rate-period">${period}</span></li>`;
  });

  const regionalDefinition = window.TriplemCurrencyRegistry?.get?.(currency);
  document.querySelectorAll(".about-section .pricing-grid").forEach(grid => {
    const cards = Array.from(grid.querySelectorAll(":scope > .pricing-card"));
    if (!cards.length) return;
    const activeCard = cards[0];
    cards.forEach((card,index) => { card.hidden = index !== 0; });
    const plan = /yearly|per year/i.test(grid.closest(".about-section")?.textContent || "") ? "yearly" : "monthly";
    const heading = activeCard.querySelector("h5");
    if (heading) heading.textContent = regionalDefinition?.name || currency;
    const visual = activeCard.firstElementChild;
    if (visual && visual.tagName === "DIV") visual.innerHTML = currencySymbolHtml(currency);
    const amount = Array.from(activeCard.querySelectorAll("div")).find(el => el.children.length === 0 && /[0-9]/.test(el.textContent || ""));
    if (amount) amount.innerHTML = regionalMoneyHtml(publicPlanPrice(plan, currency), currency);
    const periodText = activeCard.querySelector("p");
    if (periodText) periodText.textContent = plan === "yearly" ? "per year" : "per month";
  });
  ...
  const currencyCodePattern = (window.TriplemCurrencyRegistry?.codes?.() || []).map(code => String(code).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|");
  shortLabels.forEach(el => {
    if (!el.dataset.regionalOriginalLabel) el.dataset.regionalOriginalLabel = el.textContent || "";
    el.textContent = currencyCodePattern ? el.dataset.regionalOriginalLabel.replace(new RegExp(`\\b(?:${currencyCodePattern})\\b`, "g"), currency) : el.dataset.regionalOriginalLabel;
  });
