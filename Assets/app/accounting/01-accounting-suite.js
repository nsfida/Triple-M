...
(function (global) {
  ...
  function renderAccountingView() {
    ...
    switch (stateA.view) {
      ...
      case "sales": return renderDocuments("sales");
      case "purchases": return renderDocuments("purchases");
      ...
      default: return renderAccountingOverview();
    }
  }
  ...
  function accountingKpis() {
    ...
    const receivable = ar ? accountBalance(ar) : stateA.documents.filter(d => RECEIVABLE_TYPES.has(d.doc_type)).reduce((s, d) => s + num(d.balance_amount), 0);
    const payable = ap ? accountBalance(ap) : stateA.documents.filter(d => PAYABLE_TYPES.has(d.doc_type)).reduce((s, d) => s + num(d.balance_amount), 0);
  ...
  function renderAccountingOverview() {
    ...
    setContextActions(`<button class="btn ghost tiny" data-acct-action="settings"><i class="fa-solid fa-gear"></i> Accounting Settings</button>`);
    body.innerHTML = `
  ...
  function renderAccounts() {
    const body = $("#accountingWorkspaceBody");
    const rows = stateA.accounts.filter(a => searchMatch([a.code, a.name, a.account_type, a.account_subtype, a.currency]));
    setContextActions(`<button class="btn primary tiny" data-acct-action="new-account"><i class="fa-solid fa-plus"></i> Account</button>`);
    body.innerHTML = `
      <article class="accounting-card">
        <div class="accounting-card-head"><div><small>GENERAL LEDGER</small><h4>Chart of Accounts</h4><p>${stateA.accounts.length} accounts · system defaults remain protected from deletion.</p></div></div>
        <div class="accounting-table-wrap"><table class="accounting-table"><thead><tr><th>Code</th><th>Account</th><th>Type</th><th>Currency</th><th class="num">Balance</th><th></th></tr></thead><tbody>
          ${rows.map(a => `<tr><td><code>${esc(a.code)}</code></td><td><strong>${esc(a.name)}</strong><small>${esc(statusText(a.account_subtype || "General"))}${a.is_cash_account ? " · Cash/Bank" : ""}</small></td><td><span class="accounting-type-pill ${esc(a.account_type)}">${esc(statusText(a.account_type))}</span></td><td>${currencySymbol(a.currency)} ${esc(a.currency)}</td><td class="num"><strong>${moneyA(accountBalance(a), baseCurrency())}</strong></td><td class="actions"><button class="icon-btn ghost" data-acct-action="edit-account" data-id="${esc(a.id)}" title="Edit"><i class="fa-solid fa-pen"></i></button>${!a.is_system ? `<button class="icon-btn ghost danger" data-acct-action="disable-account" data-id="${esc(a.id)}" title="Disable"><i class="fa-solid fa-ban"></i></button>` : ""}</td></tr>`).join("") || `<tr><td colspan="6">${emptyState("No matching accounts")}</td></tr>`}
        </tbody></table></div>
      </article>`;
  }
  ...
  function journalRowHtml(j, compact = false) {
    ...
    return `<div class="accounting-list-row${compact ? " compact" : ""}">
      ...
      <div class="accounting-list-actions"><button class="icon-btn ghost" data-acct-action="view-journal" data-id="${esc(j.id)}" title="View"><i class="fa-solid fa-eye"></i></button>${j.status === "draft" && can("edit") ? `<button class="icon-btn ghost" data-acct-action="edit-journal" data-id="${esc(j.id)}" title="Edit"><i class="fa-solid fa-pen"></i></button><button class="icon-btn ghost" data-acct-action="post-journal" data-id="${esc(j.id)}" title="Post"><i class="fa-solid fa-circle-check"></i></button>` : ""}${j.status === "posted" && can("delete") ? `<button class="icon-btn ghost danger" data-acct-action="void-journal" data-id="${esc(j.id)}" title="Void by reversal"><i class="fa-solid fa-rotate-left"></i></button>` : ""}</div>
    </div>`;
  }
  ...
  function renderJournals() {
    ...
    setContextActions(`<button class="btn primary tiny" data-acct-action="new-journal"><i class="fa-solid fa-plus"></i> Journal</button>`);
  ...
  function documentRowHtml(d) {
    ...
    const canPay = (RECEIVABLE_TYPES.has(d.doc_type) || PAYABLE_TYPES.has(d.doc_type)) && num(d.balance_amount) > 0 && ["issued", "partial", "overdue"].includes(effective);
  ...
  function renderDocuments(kind) {
    ...
    setContextActions(`<button class="btn primary tiny" data-acct-action="new-${kind}-document"><i class="fa-solid fa-plus"></i> New ${kind === "sales" ? "Sale" : "Purchase"}</button>`);
    body.innerHTML = `
  ...
  function renderContacts() {
    const body = $("#accountingWorkspaceBody");
    const rows = stateA.contacts.filter(c => searchMatch([c.name, c.company_name, c.contact_type, c.email, c.phone, c.tax_registration_number]));
    setContextActions(`<button class="btn primary tiny" data-acct-action="new-contact"><i class="fa-solid fa-user-plus"></i> Contact</button>`);
    body.innerHTML = `<article class="accounting-card"><div class="accounting-card-head"><div><small>PARTIES</small><h4>Customers & Suppliers</h4><p>Reusable contact records for invoices, bills, payments and statements.</p></div></div><div class="accounting-contact-grid">${rows.map(c => {
      const docs = stateA.documents.filter(d => d.contact_id === c.id);
      const open = docs.reduce((s, d) => s + (num(d.balance_amount) * num(d.fx_rate || 1)), 0);
      return `<article class="accounting-contact-card"><div class="accounting-contact-avatar">${esc((c.name || "?").slice(0, 1).toUpperCase())}</div><div class="accounting-contact-info"><strong>${esc(c.name)}</strong><span>${esc(c.company_name || statusText(c.contact_type))}</span><small>${[c.email, c.phone].filter(Boolean).map(esc).join(" · ") || "No contact details"}</small></div><div class="accounting-contact-meta"><span>${docs.length} docs</span><strong>${moneyA(open)}</strong></div><div class="accounting-contact-actions"><button class="icon-btn ghost" data-acct-action="edit-contact" data-id="${esc(c.id)}"><i class="fa-solid fa-pen"></i></button></div></article>`;
  ...
  function renderReconciliation() {
    ...
    setContextActions(`<button class="btn ghost tiny" data-acct-action="import-bank"><i class="fa-solid fa-file-import"></i> Import</button><button class="btn primary tiny" data-acct-action="new-bank-transaction"><i class="fa-solid fa-plus"></i> Bank Row</button>`);
    body.innerHTML = `<article class="accounting-card"><div class="accounting-card-head"><div><small>BANK CONTROL</small><h4>Payment Reconciliation</h4><p>${pending} unreconciled · import CSV/XLSX statements or enter rows manually.</p></div></div>${rows.length ? `<div class="accounting-reconcile-list">${rows.map(tx => {
  ...
  function taxSummary(from = stateA.reportFrom, to = stateA.reportTo) {
    ...
    for (const doc of stateA.documents.filter(docOk)) {
      const sign = ["credit_note", "debit_note"].includes(doc.doc_type) ? -1 : 1;
  ...
  function currentProfit() {
    const k = accountingKpis();
    return round8(k.revenue - k.expense);
  }
  ...
  function renderTax() {
    ...
    setContextActions(`<button class="btn ghost tiny" data-acct-action="settings"><i class="fa-solid fa-gear"></i> Tax Settings</button><button class="btn primary tiny" data-acct-action="tax-pdf"><i class="fa-solid fa-file-pdf"></i> PDF</button>`);
    body.innerHTML = `
      ...
      <div class="accounting-overview-grid">
        <article class="accounting-card"><div class="accounting-card-head"><div><small>VAT ANALYSIS</small><h4>Tax Codes</h4></div></div><div class="accounting-table-wrap"><table class="accounting-table"><thead><tr><th>Code</th><th class="num">Taxable Base</th><th class="num">Tax</th></tr></thead><tbody>${t.byCode.map(r => `<tr><td><strong>${esc(r.code)}</strong></td><td class="num">${moneyA(r.taxable)}</td><td class="num">${moneyA(r.tax)}</td></tr>`).join("") || `<tr><td colspan="3">No posted taxable documents.</td></tr>`}</tbody></table></div></article>
        <article class="accounting-card"><div class="accounting-card-head"><div><small>MANAGEMENT ESTIMATE</small><h4>Corporate Tax</h4></div></div><div class="accounting-tax-estimate"><div><span>Accounting profit</span><strong>${moneyA(profit)}</strong></div><div><span>Configured threshold</span><strong>${moneyA(threshold)}</strong></div><div><span>Configured rate</span><strong>${rate.toFixed(2)}%</strong></div><div class="total"><span>Estimated amount</span><strong>${moneyA(estimate)}</strong></div><p>This is a management estimate from accounting books, not an electronic tax filing or legal determination.</p></div></article>
      </div>`;
  ...
  function renderDimensions() {
    const body = $("#accountingWorkspaceBody");
    const rows = stateA.dimensions.filter(d => searchMatch([d.dimension_type,d.code,d.name,d.notes]));
    setContextActions(`<button class="btn primary tiny" data-acct-action="new-dimension"><i class="fa-solid fa-plus"></i> Dimension</button>`);
    body.innerHTML = `<article class="accounting-card"><div class="accounting-card-head"><div><small>ANALYTICAL ACCOUNTING</small><h4>Dimensions</h4><p>Cost centres, departments, projects and branches can be tagged on journal and document lines.</p></div></div><div class="accounting-dimension-grid">${rows.map(d => `<article class="accounting-dimension-card"><div class="accounting-dimension-icon"><i class="${d.dimension_type === "cost_center" ? "fa-solid fa-bullseye" : d.dimension_type === "department" ? "fa-solid fa-people-group" : d.dimension_type === "branch" ? "fa-solid fa-code-branch" : "fa-solid fa-diagram-project"}"></i></div><div><small>${esc(statusText(d.dimension_type))}</small><strong>${esc(d.code)} · ${esc(d.name)}</strong><span>${esc(d.notes || "Active analytical dimension")}</span></div><button class="icon-btn ghost" data-acct-action="edit-dimension" data-id="${esc(d.id)}"><i class="fa-solid fa-pen"></i></button></article>`).join("") || emptyState("No dimensions")}</div></article>`;
  }
  ...
  function openBankTransactionModal() {
    ...
    openAccountingModal({title:"Add Bank Transaction",subtitle:"Statement row for reconciliation; this does not create a journal by itself.",body:`<form id="acctBankForm" class="accounting-form"><div class="accounting-form-grid"><label>Bank Account<select class="input" name="bank_account_id"><option value="">Any bank account</option>${cash.map(a=>`<option value="${esc(a.id)}">${esc(a.code)} · ${esc(a.name)}</option>`).join("")}</select></label><label>Date<input class="input" type="date" name="transaction_date" value="${today()}" required></label><label>Amount<input class="input" type="number" step="0.00000001" name="amount" required placeholder="Positive receipt, negative payment"></label><label>Currency<select class="input" name="currency">${currencyOptions(baseCurrency())}</select></label><label class="accounting-form-wide">Description<input class="input" name="description" required></label><label class="accounting-form-wide">Reference<input class="input" name="reference"></label></div></form>`,actions:`<button class="btn ghost" data-acct-modal-close>Cancel</button><button class="btn primary" id="acctBankSave">Add Row</button>`,onOpen(modal){$("#acctBankSave",modal).onclick=async()=>{const f=$("#acctBankForm",modal);if(!f.reportValidity())return;const fd=new FormData(f);const payload={owner_id:currentOwner(),bank_account_id:fd.get("bank_account_id")||null,transaction_date:fd.get("transaction_date"),description:String(fd.get("description")||"").trim(),reference:String(fd.get("reference")||"").trim()||null,amount:num(fd.get("amount")),currency:fd.get("currency"),source:"manual"};try{await global.supabase("accounting_bank_transactions",{method:"POST",body:JSON.stringify(payload)});closeAccountingModal();await loadAccountingWorkspace({force:true});toast("Bank row added.");}catch(err){toast(err.message||"Bank row could not be added.","error");}};}});
  }
  ...
  async function handleAccountingAction(action, el) {
    switch(action){
      ...
      case "disable-account": return disableAccount(el.dataset.id);
