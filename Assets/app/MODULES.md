...
5. `auth/03-two-factor.js` — Authenticator App TOTP enrollment/login/recovery UI
6. `core/04-entries-api.js`
...
- `auth/03-two-factor.js` — Authenticator App 2FA setup, recovery-code management, and pre-session verification UI.
- `notifications/01-web-push.js` — device/visitor Web Push controls and Push Center.

## Production delivery hardening (2026-09-16)

The maintained CSS source files remain independent and editable. To avoid loading
all late feature styles as separate network requests, `index.html` loads
`Assets/style/app.feature.bundle.css` after `app.bundle.css`. The feature bundle
is generated in the exact former cascade order; no source stylesheet is deleted.

After editing any feature stylesheet listed in
`Assets/style/app.feature.bundle.manifest.json`, run:
...
`npm run verify` rebuilds the deterministic feature bundle, audits frontend
resource integrity/security invariants, and runs the complete regression suite.
Runtime scripts at the bottom of `index.html` use `defer` so download can proceed
without parser blocking while preserving declaration order. Static `/Assets/`
requests receive a version-aware service-worker cache; API, authentication and
financial-data requests are deliberately excluded from that cache.
