...
function btcToggleWalletType(type) {
  ...
  controls.forEach(control => {
    ...
    if (control.button) {
      ...
      control.button.classList.toggle("is-active", control.key === mode);
    }
  });
  const label = document.getElementById("btcWalletAccessLabel");
  if (label) label.textContent = ({ full:"WIF wallet", watch:"Watch-only", seed:"Seed phrase", brain:"Brain wallet", hex:"Hex private key" })[mode] || "Wallet access";
  document.querySelector('[data-entry-menu-panel="btc-wallet-access"]')?.classList.remove("show", "open");
  document.getElementById("btcWalletAccessMenuBtn")?.setAttribute("aria-expanded","false");
}
