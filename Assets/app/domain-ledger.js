/**
 * Domain-table adapters for Triplem VIP.
...
(function (global) {
  ...
  function normalizeExpenseDetailsInternal(value) {
    const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
    const out = {};
    for (const [key, raw] of Object.entries(source)) {
      if (!/^[a-zA-Z][a-zA-Z0-9_]{0,47}$/.test(String(key || "")) || raw == null) continue;
      if (typeof raw === "boolean") { out[key] = raw; continue; }
      if (typeof raw === "number" && Number.isFinite(raw)) { out[key] = raw; continue; }
      const text = String(raw).trim();
      if (text) out[key] = text.slice(0, 1000);
    }
    return out;
  }

  function normalizeExpenseAccountDetailsInternal(value) {
    const out = normalizeExpenseDetailsInternal(value);
  ...
  function encodeExpenseDetailsInternal(value) {
    const details = normalizeExpenseDetailsInternal(value);
    if (!Object.keys(details).length) return "";
    try { return encodeURIComponent(JSON.stringify(details)); } catch (_) { return ""; }
  }

  function encodeExpenseAccountDetailsInternal(value) {
    const details = normalizeExpenseAccountDetailsInternal(value);
    if (!Object.keys(details).length) return "";
    try { return encodeURIComponent(JSON.stringify(details)); } catch (_) { return ""; }
  }

  function decodeExpenseDetailsInternal(value) {
    const text = String(value || "").trim();
    if (!text) return {};
    try { return normalizeExpenseDetailsInternal(JSON.parse(decodeURIComponent(text))); } catch (_) { return {}; }
  }

  function ensureExpenseNotes(notes, { accountType, rowType, itemName, expenseType, details, accountDetails }) {
    let n = String(notes || "");
    ...
    const encoded = encodeExpenseDetailsInternal(details);
    if (encoded && !/\[XDET:/i.test(n)) n += ` [XDET:${encoded}]`;
    const encodedAccount = encodeExpenseAccountDetailsInternal(accountDetails);
    if (encodedAccount && !/\[ADET:/i.test(n)) n += ` [ADET:${encodedAccount}]`;
    return n.trim();
  }
  ...
  function installmentPlanFromDomain(plan) {
    return markDomain({
      ...
      direction: String(plan.plan_type || "bought").toLowerCase() === "sold" ? "given" : "taken",
      installment_plan_type: String(plan.plan_type || "bought").toLowerCase() === "sold" ? "sold" : "bought",
      installment_details: plan.details && typeof plan.details === "object" ? plan.details : {},
      entry_kind: "principal",
  ...
  function installmentPaymentFromDomain(p) {
    return markDomain({
      id: p.id,
      group_id: p.group_id,
      direction: String(p.plan_type || "bought").toLowerCase() === "sold" ? "given" : "taken",
      installment_plan_type: String(p.plan_type || "bought").toLowerCase() === "sold" ? "sold" : "bought",
      entry_kind: p.payment_kind,
      person_name: p.person_name,
      currency: p.currency,
      principal_amount: null,
  ...
  function expenseAccountFromDomain(a) {
    let notes = a.notes || "";
    notes = ensureExpenseNotes(notes, { accountType: a.account_type || "Bank Account", rowType: "ACCOUNT", accountDetails: a.account_details || {} });
    ...
    const metaObj = a.meta && typeof a.meta === "object" ? a.meta : {};
    const sortOrder = Number(metaObj.sort_order);
    if (Number.isFinite(sortOrder) && !/\[WSORT:/i.test(notes)) {
      notes += ` [WSORT:${Math.round(sortOrder)}]`;
    }
  ...
  function appNoteFromDomain(n) {
    ...
    let stored = {};
    let meta = {};
    try { stored = JSON.parse(n.notes || "{}"); } catch (_) {}
    try { meta = typeof n.meta === "string" ? JSON.parse(n.meta || "{}") : (n.meta || {}); } catch (_) {}
    const title = String(meta.title || stored.title || "").trim()
      || String(content).split(/\r?\n/).map(line => line.trim()).find(Boolean)
      || "Untitled note";
    const notes = JSON.stringify({ title: title.slice(0, 120), content, rowType: "NOTE" });
    return markDomain({
      id: n.id,
      group_id: n.id,
      direction: "taken",
  ...
  function expenseDetailsFromNotesInternal(notes) {
    return decodeExpenseDetailsInternal(metaValue(notes, "XDET"));
  }

  function expenseAccountDetailsFromNotesInternal(notes) {
    return normalizeExpenseAccountDetailsInternal(decodeExpenseDetailsInternal(metaValue(notes, "ADET")));
  }
  ...
  function domainInsertPayload(entry) {
    ...
    if (section === "expenses") {
      ...
      if (etype === "ACCOUNT" || entry.entry_kind === "principal") {
        const sortFromNotes = Number(metaValue(notes, "WSORT"));
        const sortFromMeta = Number(entry?.meta?.sort_order);
        const sortOrder = Number.isFinite(sortFromNotes)
          ? Math.round(sortFromNotes)
          : (Number.isFinite(sortFromMeta) ? Math.round(sortFromMeta) : null);
        return {
          ...
          body: {
            ...
            account_details: expenseAccountDetailsFromNotesInternal(notes),
            meta: {
              etype: "ACCOUNT",
      ...
      if (etype === "TOPUP") {
        return {
          ...
          body: {
            ...
            notes,
            details: expenseDetailsFromNotesInternal(notes),
            meta: { etype: "TOPUP" },
            is_deleted: !!deleted,
            created_at: entry.created_at
          }
      ...
      return {
        ...
        body: {
          ...
          account_name: entry.person_name,
          currency: entry.currency,
          item_name: metaValue(notes, "ITEM") || (String(metaValue(notes, "XTYPE") || "").toLowerCase() === "transfer" ? "" : (entry.person_name || "Expense")),
      ...
      if (entry.entry_kind === "principal") {
        ...
        return {
          ...
          body: {
            ...
            down_payment_amount: instMeta.downPayment || 0,
            financed_amount: instMeta.financedAmount,
            frequency: instMeta.frequency || "monthly",
            plan_type: String(entry.installment_plan_type || (entry.direction === "given" ? "sold" : "bought")).toLowerCase() === "sold" ? "sold" : "bought",
            details: entry.installment_details && typeof entry.installment_details === "object" ? entry.installment_details : {},
            notes,
            meta: {
              scheduleStart: instMeta.startDate,
              lastAmount: instMeta.lastAmount,
              count: instMeta.count,
              downPayment: instMeta.downPayment || 0,
              financedAmount: instMeta.financedAmount
            },
            is_deleted: !!deleted,
            created_at: entry.created_at
          }
      ...
      return {
        ...
        body: {
          ...
          meta: {
            ...
            paymentType: (() => {
              const m = String(notes || "").match(/\[IPTYPE:([^\]]*)\]/i);
              return m ? m[1].trim() : "";
            })()
          },
          is_deleted: !!deleted,
  ...
  function mapLedgerPatchToDomain(entry, patchBody) {
    ...
    if (table === DOMAIN.loans || table === DOMAIN.installment_plans) {
      ...
      if (table === DOMAIN.installment_plans) {
        if (patchBody.installment_plan_type != null) body.plan_type = String(patchBody.installment_plan_type).toLowerCase() === "sold" ? "sold" : "bought";
        if (patchBody.installment_details != null) body.details = patchBody.installment_details && typeof patchBody.installment_details === "object" ? patchBody.installment_details : {};
      }
    }
    if (table === DOMAIN.loan_payments || table === DOMAIN.installment_payments) {
      if (table === DOMAIN.installment_payments && patchBody.installment_plan_type != null) body.plan_type = String(patchBody.installment_plan_type).toLowerCase() === "sold" ? "sold" : "bought";
    ...
    if (table === DOMAIN.expense_topups) {
      ...
      if (patchBody.currency != null) body.currency = patchBody.currency;
      if (notes !== undefined) body.details = expenseDetailsFromNotesInternal(notes);
    }
    if (table === DOMAIN.expense_entries) {
      ...
      if (notes !== undefined) {
        ...
        body.details = expenseDetailsFromNotesInternal(notes);
      }
    }
