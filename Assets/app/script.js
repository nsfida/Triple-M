...
async function saveEncryptedSessionCredential(credential){
  ...
  localStorage.setItem(SESSION_ENCRYPTED_STORAGE_KEY, JSON.stringify({
    version: 4,
    iv: bytesToBase64(iv),
...
function openTrialSignupModal(){
  ...
  modal.querySelector("#trialSignupSave").onclick = async () => {
    ...
    try {
      ...
      const ok = await completeAuthenticatedUnlock(user, sessionToken, { remember: true });
      if (!ok) {
        if (els.lockError) {
...
async function completeAuthenticatedUnlock(user, sessionToken, { remember = true } = {}){
  runtimeConfig = getEmbeddedSupabaseConfig();
  ...
  if (remember && state.sessionToken) {
    try {
      ...
      console.warn("Could not save encrypted session for auto-login.", storeErr);
    }
  }
...
async function attemptUnlock(options = {}){
  ...
  try{
    ...
    if (rememberedCredential){
      ...
      const result = await supabaseRpc("app_login", {
        p_username: safeUser,
        p_password: password,
        p_user_agent: navigator.userAgent || "",
        p_ip: null
