...
const APP_PERMISSION_MODULES = [
  ...
  "bitcoin","notes","assets","accounting","insurance","customers","reports","pdf_export","currency_settings",
...
function refreshDefaultTaxSettings(){
  Object.keys(DEFAULT_TAX_SETTINGS).forEach(currency => delete DEFAULT_TAX_SETTINGS[currency]);
  SUPPORTED_CURRENCIES.forEach(currency => {
    const configured = window.TriplemCurrencyRegistry?.taxDefault?.(currency) || {};
    DEFAULT_TAX_SETTINGS[currency] = {
      rate: Number.isFinite(Number(configured.rate)) ? Number(configured.rate) : 0,
      mode: String(configured.mode || TAX_MODE_ADD).toUpperCase() === TAX_MODE_INCLUDE ? TAX_MODE_INCLUDE : TAX_MODE_ADD
    };
  });
}
refreshDefaultTaxSettings();
window.addEventListener("triplem:currency-registry", refreshDefaultTaxSettings);
