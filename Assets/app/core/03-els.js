...
const els = {
  ...
  expenseRecordsViewFilter: document.getElementById("expenseRecordsViewFilter"),
  ...
  noteCreateForm: document.getElementById("noteCreateForm"),
  noteInput: document.getElementById("noteCreateText"),
  saveNoteBtn: document.getElementById("noteCreateSaveBtn"),
...
function resetLazyDataState({ clearEntries = false } = {}){
  ...
  state.assetTransactionLoadedIds = new Set();
  state.assetTransactionOffsets = {};
  state.assetTransactionHasMore = {};
  state.assetTransactionPageSize = 250;
...
function expenseLazyActivityQueryKey(){
  ...
  const bounds = typeof expenseActivityQueryBounds === "function"
    ...
    : (typeof expenseHistoryRangeBounds === "function" ? expenseHistoryRangeBounds() : { from: "", to: "", mode: "history" });
  const rangeKey = bounds.mode === "search"
    ? `search:${bounds.from || ""}:${bounds.to || ""}`
...
function getUserAccessFlags(user = state.sessionUser){
  if (!user) {
    return {
      ...
      payment_approval_pending: false,
      subscription_request_id: null,
      subscription_request_context: null,
      subscription_billing_period: null,
      subscription_proposed_expires_at: null,
      subscription_promotion_months: 0
    };
  }
  ...
  if (user.period_expired != null || user.grace_active != null || user.has_access_period != null || user.lock_active != null) {
    ...
    return {
      ...
      payment_approval_pending: user.payment_approval_pending === true || user.payment_approval_pending === "true",
      subscription_request_id: user.subscription_request_id || null,
      subscription_request_context: user.subscription_request_context || null,
      subscription_billing_period: user.subscription_billing_period || null,
      subscription_proposed_expires_at: user.subscription_proposed_expires_at || null,
      subscription_promotion_months: Number(user.subscription_promotion_months) || 0,
  ...
  const paymentApprovalPending = user.payment_approval_pending === true
    || user.payment_approval_pending === "true"
    || settings.PaymentApprovalPending === true;
  const pendingContext = user.subscription_request_context || null;
  const pendingSignup = paymentApprovalPending && pendingContext === "signup";
  const plan = pendingSignup ? "full" : String(user.access_plan || "full").toLowerCase();
  const pendingExpiry = user.subscription_proposed_expires_at || null;
  const expiresAt = (pendingSignup && pendingExpiry ? pendingExpiry : user.trial_expires_at)
    ? new Date(pendingSignup && pendingExpiry ? pendingExpiry : user.trial_expires_at)
    : null;
  const now = Date.now();
  ...
  return {
    ...
    access_last_extended_at: user.access_last_extended_at || null,
    access_last_extended_until: user.access_last_extended_until || null,
    payment_approval_pending: paymentApprovalPending,
    subscription_request_id: user.subscription_request_id || null,
    subscription_request_context: pendingContext,
    subscription_billing_period: user.subscription_billing_period || null,
    subscription_proposed_expires_at: pendingExpiry,
    subscription_promotion_months: Number(user.subscription_promotion_months) || 0,
    data_access_allowed: pendingSignup || unlimited || periodActive || graceActive || user.data_access_allowed === true
  };
}
...
function showTrialExpiredOverlay(){
  ...
  if (message) {
    message.textContent = flags.lock_active
      ? `Grace ended after ${formatTrialExpiry(flags.trial_expires_at)}. The workspace is locked for one day. Open Plan & Subscription below to submit a monthly or yearly renewal. Auto-disable is on ${formatTrialExpiry(flags.access_disable_at)}.`
      : `Expired on ${formatTrialExpiry(flags.trial_expires_at)}. Your workspace is locked. Open Plan & Subscription below to submit a monthly or yearly renewal payment.`;
  }
...
function updateAccessBanner(){
  ...
  const showPendingPayment = !showGuest && state.unlocked && flags.payment_approval_pending === true;
  ...
  els.guestModeBanner.classList.toggle("hide", !(showGuest || showPendingPayment || showApproaching));
  ...
  els.guestModeBanner.classList.toggle("payment-pending-banner", !!showPendingPayment);
  ...
  if (showPendingPayment) {
    const period = flags.subscription_billing_period === "yearly" ? "Pro Yearly" : "Pro Monthly";
    const proposed = flags.subscription_proposed_expires_at ? ` Proposed expiry ${formatTrialExpiry(flags.subscription_proposed_expires_at)}.` : "";
    if (els.accessBannerTitle) els.accessBannerTitle.textContent = "Pro payment pending approval";
    if (els.accessBannerMessage) els.accessBannerMessage.textContent = `${period} payment receipt is awaiting administrator verification.${proposed}`;
    updateWeakPasswordBanner();
    return;
  }
  ...
  if (showApproaching) {
    ...
    if (els.accessBannerMessage) {
      els.accessBannerMessage.textContent = `${daysLabel}. Expires ${formatTrialExpiry(flags.trial_expires_at)}. Renew from Plan & Subscription.`;
    }
  }
  updateWeakPasswordBanner();
}
...
/* Triplem VIP shared confirmation dialog — theme-aware replacement for destructive browser confirms. */
...
function ensureAppConfirmDialog(){
  ...
  modal.innerHTML = `
    ...
    <div class="modal-dialog app-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="appConfirmTitle" aria-describedby="appConfirmMessage">
      <div class="app-confirm-glint" aria-hidden="true"></div>
      <div class="app-confirm-head">
        <span class="app-confirm-icon" aria-hidden="true"><i class="fa-solid fa-trash-can"></i></span>
        <div class="app-confirm-copy">
          <p class="app-confirm-kicker">Confirmation required</p>
          <h3 id="appConfirmTitle">Delete entry?</h3>
        </div>
      </div>
      <div class="modal-body app-confirm-body">
        <p id="appConfirmMessage" class="app-confirm-message"></p>
        <div class="app-confirm-note"><i class="fa-solid fa-shield-halved" aria-hidden="true"></i><span id="appConfirmNote">Review this action before continuing.</span></div>
        <div class="modal-footer app-confirm-actions">
          <button type="button" class="btn ghost app-confirm-action-btn" id="appConfirmCancelBtn" aria-label="Cancel" title="Cancel">
            <span class="app-confirm-action-symbol" aria-hidden="true">&times;</span><span class="app-confirm-action-label">Cancel</span>
          </button>
          <button type="button" class="btn danger app-confirm-action-btn" id="appConfirmOkBtn" aria-label="Delete" title="Delete">
            <span class="app-confirm-action-symbol" aria-hidden="true">&#10003;</span><span class="app-confirm-action-label">Delete</span>
          </button>
        </div>
      </div>
    </div>`;
...
function appConfirm(options = {}){
  ...
  const task = () => new Promise(resolve => {
    const modal = ensureAppConfirmDialog();
    ...
    const backdrop = modal.querySelector("[data-app-confirm-cancel]");
    const icon = modal.querySelector(".app-confirm-icon i");
    const previousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const previousOverflow = document.body.style.overflow;

    const accent = request.accent === "primary" ? "primary" : "danger";
    title.textContent = request.title || (accent === "danger" ? "Delete entry?" : "Confirm action");
    message.textContent = String(request.message || "Are you sure you want to continue?");
    note.textContent = request.note || (accent === "danger" ? "Deleted records may be moved to the recycle bin where supported; permanent deletions cannot be undone." : "Review this action before continuing.");
    okBtn.className = `btn ${accent === "danger" ? "danger" : "primary"} app-confirm-action-btn`;
    const confirmLabel = request.confirmLabel || (accent === "danger" ? "Delete" : "Confirm");
    const okLabel = okBtn.querySelector(".app-confirm-action-label");
    if (okLabel) okLabel.textContent = confirmLabel;
    okBtn.setAttribute("aria-label", confirmLabel);
    okBtn.setAttribute("title", confirmLabel);
    const cancelLabel = cancelBtn.querySelector(".app-confirm-action-label");
    if (cancelLabel) cancelLabel.textContent = "Cancel";
    cancelBtn.setAttribute("aria-label", "Cancel");
    cancelBtn.setAttribute("title", "Cancel");
    icon.className = request.icon || (accent === "danger" ? "fa-solid fa-trash-can" : "fa-solid fa-circle-check");
    modal.classList.toggle("is-primary", accent === "primary");
    modal.classList.remove("hide");
    modal.setAttribute("aria-hidden", "false");
    document.body.style.overflow = "hidden";
  ...
  const queued = appConfirmQueue.then(task, task);
  appConfirmQueue = queued.catch(() => false);
  return queued;
}

function appConfirmDelete(message, options = {}){
  return appConfirm({
    ...options,
    message,
    accent: "danger",
...
window.appConfirmDelete = appConfirmDelete;
