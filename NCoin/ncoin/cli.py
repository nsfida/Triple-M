...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if args.out:
        Path(args.out).write_text(json.dumps(tx.to_dict(), indent=2), encoding="utf-8")
        print(f"Wrote transaction {tx.txid()} to {args.out}")
    else:
        print(json.dumps(tx.to_dict(), indent=2))
    if args.broadcast:
        host, port = parse_peer(args.broadcast)
        asyncio.run(broadcast_tx(host, port, tx))
        print(f"Broadcast transaction {tx.txid()} to {host}:{port}")
