...
it("147 removes every other trusted browser atomically while preserving the verified current browser", () => {
  ...
  assert.match(index, /40-account-security-session-refinement\.css\?v=20260907-mobile-close151/);
...
it("151 lets the mobile Security Center actually close despite its ID-level display rule", () => {
  const root = path.join(__dirname, "..");
  const security = fs.readFileSync(path.join(root, "Assets/app/auth/05-account-security.js"), "utf8");
  const css = fs.readFileSync(path.join(root, "Assets/style/40-account-security-session-refinement.css"), "utf8");

  assert.match(security, /modal\.classList\.add\("hide"\)/);
  assert.match(security, /data-account-security-close/);
  assert.match(css, /#accountSecurityCenterModal\.account-security-center-modal\.modal\.hide[\s\S]*display:none!important/);
  assert.match(css, /#accountSecurityCenterModal\.account-security-center-modal\.modal\[aria-hidden="true"\][\s\S]*pointer-events:none!important/);
});
...
it("Accounting ERP core is additive, owner-isolated, immutable after posting, and closed-period aware", () => {
  const root = path.join(__dirname, "..");
  const migration = fs.readFileSync(path.join(root, "migrations/149_full_accounting_erp_core.sql"), "utf8");
  const topLevel = migration.replace(/\$\$[\s\S]*?\$\$/g, "").replace(/--.*$/gm, "");

  assert.match(migration, /create table if not exists public\.accounting_accounts/);
  assert.match(migration, /create table if not exists public\.accounting_journals/);
  assert.match(migration, /create table if not exists public\.accounting_documents/);
  assert.match(migration, /create table if not exists public\.accounting_bank_transactions/);
  assert.match(migration, /accounting_validate_references/);
  assert.match(migration, /Accounting owner mismatch/);
  assert.match(migration, /Posted or void journals are immutable/);
  assert.match(migration, /Issued accounting documents are immutable/);
  assert.match(migration, /app_accounting_assert_open_period/);
  assert.match(migration, /books_lock_date date/);
  assert.match(migration, /credit_applied_amount/);
  assert.match(migration, /import_fingerprint/);
  assert.match(migration, /app_accounting_unmatch_bank_transaction/);
  assert.match(migration, /app_accounting_cancel_document/);
  assert.match(migration, /app_admin_backup_table_list/);
  assert.match(migration, /VAT15/);
  assert.match(migration, /default_tax_code text not null default 'NONE'/);
  const backup = fs.readFileSync(path.join(root, "Assets/app/lib/admin-backup.js"), "utf8");
  assert.match(backup, /"accounting_journals"/);
  assert.match(backup, /"accounting_documents"/);
  assert.match(backup, /"accounting_audit_log"/);
  assert.match(migration, /grant select on public\.accounting_journals to anon,authenticated/);
  assert.match(migration, /grant select on public\.accounting_documents to anon,authenticated/);
  assert.doesNotMatch(migration, /grant select,insert,update,delete on public\.accounting_journals/i);
  assert.doesNotMatch(migration, /grant select,insert,update,delete on public\.accounting_documents/i);
  assert.doesNotMatch(topLevel, /\b(?:drop\s+table|truncate\s+table|delete\s+from\s+public\.(?:expense_|goods_|app_assets|loans|installment_|bitcoin_|app_notes)|update\s+public\.(?:expense_|goods_|app_assets|loans|installment_|bitcoin_|app_notes))\b/i);
});

it("150 restores Accounting backups atomically without erasing Accounting for legacy snapshots", () => {
  const root = path.join(__dirname, "..");
  const migration = fs.readFileSync(path.join(root, "migrations/150_accounting_backup_restore_integrity.sql"), "utf8");

  assert.match(migration, /create or replace function public\.app_admin_import_full_backup\(p_payload jsonb\)/);
  assert.match(migration, /has_accounting boolean := false/);
  assert.match(migration, /Accounting backup is incomplete\. Missing or invalid table/);
  assert.match(migration, /if has_accounting then[\s\S]*accounting_clear_order/);
  assert.match(migration, /accountingPreservedFromCurrentDatabase/);
  assert.match(migration, /overriding system value/);
  assert.match(migration, /app_accounting_assert_restored_integrity/);
  assert.match(migration, /unbalanced posted journal/);
  assert.match(migration, /if public\.app_import_is_authorized\(\) then[\s\S]*new\.owner_id := expected_owner/);
  assert.match(migration, /A restore should reproduce historical audit rows/);
  assert.match(migration, /perform setval\(seq_name::regclass,max_audit_id,true\)/);
  assert.doesNotMatch(migration.replace(/\$\$[\s\S]*?\$\$/g, ""), /\b(?:truncate\s+table|drop\s+table)\b/i);
});

it("Accounting workspace ships complete lifecycle, reconciliation, reporting, permissions, PDF references, and responsive theming", () => {
  const root = path.join(__dirname, "..");
  const accounting = fs.readFileSync(path.join(root, "Assets/app/accounting/01-accounting-suite.js"), "utf8");
  const css = fs.readFileSync(path.join(root, "Assets/style/41-accounting-suite.css"), "utf8");
  const nav = fs.readFileSync(path.join(root, "Assets/app/ui/01-render-navigation.js"), "utf8");
  const perms = fs.readFileSync(path.join(root, "Assets/app/lib/permissions.js"), "utf8");
  const config = fs.readFileSync(path.join(root, "Assets/app/core/01-config-constants.js"), "utf8");
  const notes = fs.readFileSync(path.join(root, "Assets/app/notes/01-notes.js"), "utf8");
  const index = fs.readFileSync(path.join(root, "index.html"), "utf8");

  assert.match(accounting, /Chart of Accounts/);
  assert.match(accounting, /app_accounting_post_journal/);
  assert.match(accounting, /app_accounting_post_document/);
  assert.match(accounting, /app_accounting_record_payment/);
  assert.match(accounting, /app_accounting_cancel_document/);
  assert.match(accounting, /app_accounting_unmatch_bank_transaction/);
  assert.match(accounting, /bankImportFingerprint/);
  assert.match(accounting, /Supply Date/);
  assert.match(accounting, /Document UUID/);
  assert.match(accounting, /Current Earnings \(Unclosed\)/);
  assert.match(accounting, /Dimension Profitability/);
  assert.match(accounting, /Bank Reconciliation Status/);
  assert.match(accounting, /Customer Balances/);
  assert.match(accounting, /Supplier Balances/);
  assert.match(accounting, /Books locked through/);
  assert.match(accounting, /new global\.QRCode/);
  assert.match(css, /@media \(max-width:760px\)/);
  assert.match(css, /accounting-modal-dialog/);
  assert.match(css, /scrollbar-width:none/);
  assert.match(nav, /accounting:\s*"accounting"/);
  assert.match(nav, /loadAccountingWorkspace/);
  assert.match(perms, /accounting/);
  assert.match(config, /"accounting"/);
  assert.match(notes, /accountingViewAllowed/);
  assert.match(index, /41-accounting-suite\.css/);
  assert.match(index, /01-accounting-suite\.js/);
});

it("Accounting reports render currency markup safely and Admin can explicitly gate Accounting access", () => {
  const root = path.join(__dirname, "..");
  const accounting = fs.readFileSync(path.join(root, "Assets/app/accounting/01-accounting-suite.js"), "utf8");
  const admin = fs.readFileSync(path.join(root, "Assets/app/admin/01-admin.js"), "utf8");
  const css = fs.readFileSync(path.join(root, "Assets/style/41-accounting-suite.css"), "utf8");

  assert.match(accounting, /TRUSTED_REPORT_MONEY_RE/);
  assert.match(accounting, /reportCellHtml\(c\)/);
  assert.match(accounting, /reportMoneyIsZero/);
  assert.match(accounting, /exportRows = \(rows \|\| \[\]\)\.map/);
  assert.match(accounting, /userHasPermission\("accounting", "view"\)/);
  assert.match(admin, /Accounting feature/);
  assert.match(admin, /applyAdminAccountingAccessToTabs/);
  assert.match(admin, /ADMIN_TAB_OPTIONS\.filter\(opt => opt\.id !== "accounting"\)/);
  assert.match(admin, /\["dashboard", "expenses", "loans", "notes"\]/);
  assert.match(css, /@media \(min-width:1100px\)/);
  assert.match(css, /admin-accounting-access-block/);
});

it("157 expands Aziz product knowledge without weakening handoff, regional pricing, or visitor-chat readability", () => {
  const root = path.join(__dirname, "..");
  const edge = fs.readFileSync(path.join(root, "supabase/functions/aziz-live-support/index.ts"), "utf8");
  const landing = fs.readFileSync(path.join(root, "Assets/app/landing/02-live-chat.js"), "utf8");
  const semantic = fs.readFileSync(path.join(root, "Assets/app/ai/01-live-chat-semantic-ai.js"), "utf8");
  const css = fs.readFileSync(path.join(root, "Assets/style/32-aziz-chat-agent-polish.css"), "utf8");
  const index = fs.readFileSync(path.join(root, "index.html"), "utf8");

  assert.match(edge, /ACCOUNTING ERP/);
  assert.match(edge, /Chart of Accounts/);
  assert.match(edge, /Accounts Receivable/);
  assert.match(edge, /Bank Reconciliation/);
  assert.match(edge, /Books Lock Date/);
  assert.match(edge, /operational Expenses\/Wallets, Inventory, Assets and Loans modules remain intentionally separate/);
  assert.match(edge, /Notes and reminders|NOTES AND REMINDERS/);
  assert.match(edge, /BITCOIN/);

  const fastHandoff = edge.indexOf("if (explicitHumanRequest(message))");
  const rateLimit = edge.indexOf("const ipHash = await sha256Hex");
  assert.ok(fastHandoff > 0 && rateLimit > fastHandoff, "Explicit human handoff must remain ahead of Aziz throttling");
  assert.match(edge, /regionalBillingCurrency\(visitorCountryCode/);
  assert.match(edge, /regionalCatalog\(facts\.catalog, visitorBillingCurrency\)/);
  assert.match(edge, /const exactPricing = pricingAnswer\(message, visitorCatalog, visitorBillingCurrency\)/);
  assert.match(edge, /AUTHORITATIVE LIVE FACTS/);

  assert.match(semantic, /\["accounting", "What is the Accounting ERP/);
  assert.match(semantic, /"bank_reconciliation"/);
  assert.match(landing, /"accounting","chart_accounts","journals","receivables_payables","bank_reconciliation"/);

  assert.match(css, /width:min\(470px,calc\(100vw - 76px\)\)!important/);
  assert.match(css, /font-size:\.79rem!important/);
  assert.match(css, /font-size:\.86rem!important/);
  assert.match(index, /32-aziz-chat-agent-polish\.css\?v=20260905-v153&knowledge=157/);
...
it("Delivered source contains no prohibited competitor branding", () => {
  ...
  const extensions = new Set([".html",".css",".js",".ts",".sql",".md",".json"]);
...
it('164 adds a read-only permission-scoped Dashboard Financial Timeline with responsive themed UI', () => {
  ...
  const readFile = rel => fs.readFileSync(path.join(root, rel), 'utf8');
  const index = readFile('index.html');
  const feature = readFile('Assets/app/dashboard/01-financial-timeline.js');
  const css = readFile('Assets/style/50-dashboard-financial-timeline.css');
  const nav = readFile('Assets/app/ui/01-render-navigation.js');
  const migration = readFile('migrations/164_dashboard_financial_timeline.sql');

  assert.match(index, /data-dashboard-main-view="overview"/);
  assert.match(index, /data-dashboard-main-view="timeline"/);
  assert.match(index, /id="financialTimelineRoot"/);
  assert.match(index, /01-financial-timeline\.js/);
  assert.match(index, /50-dashboard-financial-timeline\.css/);

  assert.match(feature, /app_get_my_financial_timeline/);
  assert.match(feature, /Financial Timeline/);
  assert.match(feature, /All activity/);
  assert.match(feature, /All currencies/);
  assert.match(feature, /Search activity/);
  assert.match(feature, /Load more/);
  assert.match(feature, /netByCurrency/);
  assert.match(feature, /window\.openTriplemAiRecord/);
  assert.match(feature, /timeline-day/);
  assert.match(nav, /prepareFinancialTimelineDashboard/);

  assert.match(migration, /create or replace function public\.app_get_my_financial_timeline/);
  assert.match(migration, /public\.app_data_owner_id\(\)/);
  assert.match(migration, /public\.app_has_permission\('dashboard', 'view'\)/);
  assert.match(migration, /public\.app_has_permission\('expenses', 'view'\)/);
  assert.match(migration, /public\.app_has_permission\('loans','view'\)/);
  assert.match(migration, /public\.app_has_permission\('installments','view'\)/);
  assert.match(migration, /public\.app_has_permission\('accounting','view'\)/);
  assert.match(migration, /'transfer'.*'loan given'.*'installment payment'.*'inventory purchase'.*'asset purchase'/s);
  assert.doesNotMatch(migration, /\b(update|delete|insert|truncate)\s+(?:into\s+)?public\.(?:loans|loan_payments|installment_plans|installment_payments|expense_entries|expense_topups|app_assets|app_asset_transactions|goods_items|goods_sales|accounting_documents|accounting_payments)\b/i);

  assert.match(css, /var\(--primary\)/);
  assert.match(css, /var\(--panel\)/);
  assert.match(css, /@media\(max-width:600px\)/);
  assert.match(css, /timeline-summary-grid/);
  assert.match(css, /timeline-event-card/);
});

it('SEO refresh covers Financial Timeline, Bought/Sold installments, current expense semantics and readable Triplem AI privacy cards', () => {
  const root = path.join(__dirname, '..');
  const readFile = rel => fs.readFileSync(path.join(root, rel), 'utf8');
  const timeline = readFile('seo/financial-timeline.html');
  const installments = readFile('seo/installment-plans.html');
  const ai = readFile('seo/triplem-ai.html');
  const expenses = readFile('seo/expense-tracking.html');
  const css = readFile('Assets/style/42-public-accounting-seo.css');
  const robots = readFile('robots.txt');
  const llms = readFile('llms.txt');
  const llmsFull = readFile('llms-full.txt');
  const sitemap = readFile('sitemap.xml');

  assert.match(timeline, /Financial Timeline Dashboard/);
  assert.match(timeline, /read-only/i);
  assert.match(timeline, /Internal Transfer/i);
  assert.match(timeline, /Is Financial Timeline the General Ledger\?/i);
  assert.match(installments, /Installment Plans Bought/i);
  assert.match(installments, /Installment Plans Sold/i);
  assert.match(installments, /operational receivable/i);
  assert.match(installments, /guarantor/i);
  assert.match(expenses, /pure expenses/i);
  assert.match(expenses, /Receiving/);
  assert.match(expenses, /Internal Transfers/);
  assert.match(ai, /Precision retrieval/);
  assert.match(ai, /repayment-adjusted/i);
  assert.match(ai, /Expenses &amp; Receiving/);
  assert.match(css, /\.security-dark-section \.security-rail-card\s*\{[\s\S]*?color:\s*#f4f7ff/);
  assert.match(css, /\.security-dark-section \.security-rail-card strong\s*\{\s*color:\s*#fff;?\s*\}/);
  assert.match(css, /\.architecture-page \.security-dark-section \.security-prose p,[\s\S]*?color:\s*rgba\(245,248,255,\.78\)/);
  assert.match(css, /\.architecture-page \.security-dark-section \.security-prose p:first-child\s*\{[\s\S]*?color:\s*rgba\(255,255,255,\.92\)/);
  ...
  assert.match(sitemap, /https:\/\/triplem\.vip\/seo\/insurance-management\.html/);
  assert.match(sitemap, /https:\/\/triplem\.vip\/seo\/inventory-management\.html/);
  assert.match(sitemap, /https:\/\/triplem\.vip\/seo\/excel-to-triplem-vip\.html/);
  assert.match(sitemap, /https:\/\/triplem\.vip\/seo\/windows-desktop-app\.html/);

  const seoPages = fs.readdirSync(path.join(root, 'seo')).filter(name => name.endsWith('.html'));
  assert.equal(seoPages.length, 32);
  const authorityPages = new Set(['multi-currency-accounting.html', 'month-end-close.html', 'finance-workflow-playbook.html', 'editorial-policy.html']);
  for (const page of seoPages) {
    const html = readFile(path.join('seo', page));
    assert.match(html, /42-public-accounting-seo\.css\?v=(?:20260910-security-parity006|20260911-seo-geo-authority007)/, `${page} should use the unified SEO visual stylesheet`);
    if (!authorityPages.has(page)) assert.match(html, /id="global-currency-support"/, `${page} should retain international currency availability`);
...
it('167 publishes current international SEO and answer-engine discovery assets without exposing implementation paths as content', () => {
  ...
  assert.match(sitemap, /<loc>https:\/\/triplem\.vip\/seo\/accounting-software\.html<\/loc>\s*<lastmod>2026-09-21<\/lastmod>/);
...
it('168 hardens SEO/GEO with authority content, localization, structured-data cleanup and deployment automation', () => {
  const root = path.join(__dirname, '..');
  const read = rel => fs.readFileSync(path.join(root, rel), 'utf8');
  const pkg = JSON.parse(read('package.json'));
  const sitemap = read('sitemap.xml');
  ...
  for (const slug of ['multi-currency-accounting','month-end-close','finance-workflow-playbook','editorial-policy']) {
    assert.equal(fs.existsSync(path.join(root,'seo',`${slug}.html`)), true);
    assert.equal(fs.existsSync(path.join(root,'seo',`${slug}.md`)), true);
    assert.match(read(path.join('seo',`${slug}.html`)), /TechArticle/);
  }

  const regionPages = fs.readdirSync(path.join(root,'regions')).filter(n => n.endsWith('.html'));
  assert.equal(regionPages.length, 13);
  for (const page of regionPages) {
    const html = read(path.join('regions',page));
  ...
  assert.match(read('regions/uae-ar.html'), /<html[^>]*lang="ar-AE"[^>]*dir="rtl"|<html[^>]*dir="rtl"[^>]*lang="ar-AE"/);
  assert.match(read('regions/pakistan-ur.html'), /<html[^>]*lang="ur-PK"[^>]*dir="rtl"|<html[^>]*dir="rtl"[^>]*lang="ur-PK"/);
  assert.match(read('regions/india-hi.html'), /<html[^>]*lang="hi-IN"[^>]*dir="ltr"|<html[^>]*dir="ltr"[^>]*lang="hi-IN"/);
  assert.match(read('regions/belgium-fr.html'), /<html[^>]*lang="fr-BE"[^>]*dir="ltr"|<html[^>]*dir="ltr"[^>]*lang="fr-BE"/);
  assert.match(read('regions/belgium-nl.html'), /<html[^>]*lang="nl-BE"[^>]*dir="ltr"|<html[^>]*dir="ltr"[^>]*lang="nl-BE"/);

  for (const page of fs.readdirSync(path.join(root,'seo')).filter(n => n.endsWith('.html'))) {
    assert.doesNotMatch(read(path.join('seo',page)), /"@type"\s*:\s*"FAQPage"/);
  }

  assert.match(robots, /User-agent: Claude-SearchBot/);
  assert.match(robots, /User-agent: Claude-User/);
  assert.match(robots, /User-agent: Perplexity-User/);
  assert.doesNotMatch(read('index.html'), /SpeakableSpecification/);
  assert.match(read('Founder/index.html'), /rel="canonical"/);
  assert.match(robots, /Allow: \/regions\//);
  assert.match(robots, /Disallow: \/regions\/\*\.md\$/);
  assert.match(sitemap, /https:\/\/triplem\.vip\/regions\/india\.html/);
  assert.match(sitemap, /hreflang="hi-IN"/);
  assert.equal(pkg.scripts['seo:prepare'], 'node scripts/build_sitemap.js && node scripts/audit_seo.js');
  assert.equal(pkg.scripts['seo:postdeploy'], 'node scripts/seo_postdeploy.js');
  for (const file of ['build_sitemap.js','audit_seo.js','audit_live_seo.js','seo_postdeploy.js']) {
    assert.equal(fs.existsSync(path.join(root,'scripts',file)), true);
  }
  assert.equal(fs.existsSync(path.join(root,'docs','SEO_GEO_OPERATIONS.md')), true);
  assert.equal(fs.existsSync(path.join(root,'.github','workflows','seo-quality.yml')), true);
